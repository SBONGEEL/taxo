...
**ب) `tsc` أخضرُ بذاكرةٍ مخبَّأة**: `AppVersion` مستعملٌ في
`src/api/endpoints.ts` بلا استيراد **في تطبيقَي الراكب والكبتن معاً**
(`admin-panel` وحدَه يستورده). **وسترَه `tsconfig.app.tsbuildinfo`**: رآه
`tsc -b` محدَّثاً فتخطّى الفحص. **واستثناءُ `*.tsbuildinfo` من النسخ هو الذي
كشفه** — بناءٌ من الصفر في لينكس أعطى
`error TS2304: Cannot find name 'AppVersion'`.

> **والدرسُ ليس عن لينكس**: **بناءٌ من شجرةٍ نظيفةٍ يقيس ما لا يقيسه بناءٌ فوق
> ذاكرة.** وأيُّ «أخضرَ» يعتمد ملفَّ حالةٍ غيرَ متعقَّبٍ يُقرأ خُضرةً لجهازٍ
> واحد، لا للشيفرة.
