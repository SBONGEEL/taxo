...
## حادثُ ٢٠٢٦-٠٩-٢٠ — Redis قُلب إلى تابعٍ من الخارج، والخلفيةُ كانت مفتوحةً على منفذٍ عارٍ

**الأثرُ المقيس**: من `02:17:01Z` إلى `07:28:14Z` — **٥ ساعاتٍ و١١ دقيقة** —
كان Redis **للقراءة فقط**، فمات العاملُ (**١٢٩٤ إعادةَ تشغيل**، خروجٌ ١) وعجز
المجدولُ عن وضع أيِّ مهمّة، **وسقط الدخولُ نفسُه**: المصادقةُ تكتب مفاتيحَ
الحدِّ في Redis **قبل** أن تفحص كلمةَ المرور، فكانت كلُّ محاولةٍ ٥٠٠. وقُرئ
ذلك في سجلِّ الخلفية: **١٠ سطور `POST /api/v1/auth/login … 500`** و**١١
`ReadOnlyError`** — لا استنتاجاً.

**والجذر**: **المنافذُ المنشورة في compose تتجاوز `ufw`.** قواعدُها تمرّ
بسلسلة `DOCKER-USER` قبل أن يراها الجدار، **فـ`default deny (incoming)` لا
يحكمها** — وكان `ufw status` يقول «حجب» بينما القياسُ من خارج الشبكة يقول
«مفتوح». وRedis كان بلا كلمةِ مرورٍ و`protected-mode no`.

### ما فُعل، بترتيبه

١. **الشبكةُ أوّلاً ثمّ الخدمة** — ولو عُكس لأعاد المهاجمُ القلبَ فوراً.
٢. `SLAVEOF NO ONE` ⇐ `role:master`، والكتابةُ عادت (فحصُ `SET/GET/DEL`).
٣. إعادةُ العامل والمجدول ⇐ خضراوان، والمهامُّ تُنفَّذ (`sweep_stop_waiting`
   نجحت في ٣٫٣٢ث).
٤. **رموزُ التجديد دُوِّرت**: ثلاثةٌ (لا اثنان) حُذفت ⇐ صفر.
٥. **والدائمُ في الشجرة لا في الجدار**: قواعدُ الشبكة **لا تنجو من إقلاع**،
   فالإصلاحُ الحقيقيُّ أن تُنزع المنافذُ من `compose` — وهو ما وقع.

### وما لم يتركه المهاجم

**لا وحدةَ مُحمَّلة** (`MODULE LIST` فارغ) · **و`dir`/`dbfilename` لم
يُحوَّلا** إلى `~/.ssh` أو `cron` · **ولا مفتاحَ SSH جديد** (الملفُّ بتاريخ
٢٠٢٦-٠٨-٢٢، قبل الحادث) · **ولا `crontab`** · **ولا مُعدِّنَ في أعلى المعالج**
· **ولا مستخدمَ ACL**. ومزامنتُه لم تكتمل أصلاً — خادمُه كان يرفض الاتصال —
**فلا شيءَ من عنده بلغ الجدول**، ومفاتيحُنا سليمةٌ كما عُدَّت قبلُ وبعدُ.

**لكنّ المنفذَ كان مفتوحاً بلا مصادقة**، فيُفترض أن **كلَّ ما في Redis كان
مقروءاً** لمن وجده: رموزُ التجديد وحالُ الحدِّ وحضورُ الكباتن. **ولذلك
دُوِّرت الرموز.**

### الدروسُ التي تبقى بعد أن يُنسى الحادث

- **حارسٌ يقول «محجوب» ولم يُقَس من خارج الشبكة ليس حارساً.** `ufw status`
  كان صادقاً في نفسِه وكاذباً عن الواقع.
- **والقياسُ من الخادم نفسِه لا يقيس ما يراه العالم**: اتصالٌ يبدأ محلياً
  يسلك طريقاً آخرَ ولا يمرّ بحيث توضع قواعدُ المنع — **فيُقرأ المنفذُ
  مفتوحاً وهو مغلق، أو العكس**. المِجَسُّ من خارج الشبكة هو الحَكَم.
- **وكلمةُ مرورٍ و`protected-mode` لا يُضبط أحدُهما وحدَه**: الثاني بلا
  الأولى يقطع الخلفيةَ عن Redis.
- **وفحصُ صحّةٍ لا يصادق يصير سقوطاً يومَ تُضبط المصادقة** — فيقف
  `depends_on: service_healthy` والمكدَّسُ لا يقلع وكلُّ شيءٍ «سليم».
- **و١٢٩٤ إعادةَ تشغيلٍ لم تقل لأحدٍ شيئاً.** لا مراقبةَ ولا تنبيه — وهي
  الفجوةُ نفسُها التي كُتبت في تقرير تتبّع الأعطال قبل الحادث بيومين.

> **وتفصيلُ من فعلها (عنوانُه ومنفذُه) خارج هذه الشجرة بقصد**: المستودعُ
> **عامّ**، وسجلُّه يُقرأ من كلِّ أحد. وهو في
> `/home/taxo/secrets/incident-2026-09-20.md` على الخادم (٦٠٠).

<!--/جديد-->

<!--جديد-->
## بندٌ مفتوح — **لا حدَّ لنطاق الخدمة عند إنشاء الرحلة** (قِيس ٢٠٢٦-٠٩-٠٩ · **أُغلق ٢٠٢٦-٠٩-١٠ — اقرأ البندَ الذي يليه**)

**ما قِيس، لا ما يُرجَّح:** في قاعدة التطوير صفُّ رحلةٍ `country_code = JO`
ووجهتُه `32.8935, 13.3669` — **طرابلس، على بعد نحو ألفي كيلومتر من عمّان**.
والخلفيةُ قبلته وسعّرته على المسافة كما هي.

**والمسارُ الذي يبلغه مستخدمٌ حقيقيّ**: بحثُ العناوين محدودٌ بـ`country=` فلا
يُخرج عنواناً خارج السوق، **لكنّ «حدّدها على الخريطة بالدبوس» لا حدَّ له** —
من يُبعد الخريطةَ ويضع دبّوساً خارج البلد يحصل على تقديرٍ وطلب.

**وما بُني اليوم**: حدُّ **العرض** في تطبيق الراكب («وجهات أخيرة» صارت تُصفّى
بـ`ride.country_code`). **وما لم يُبنَ**: حدُّ **القبول** في الخلفية.

**ولمَ لم يُبنَ هنا**: لا حدودَ سوقٍ مخزَّنةٌ في المشروع أصلاً —
`map_settings` تحمل `nearby_radius_km` و`nearby_max_count` وحدَهما، ولا جدولَ
`service_areas`. **فبناؤه ميزةٌ لها سطحُ لوحةٍ وقرارُ مالك**، لا إصلاحُ عطبٍ
جانبيّ. ويمسّ التسعير، **فلا يُبنى بلا إذن**.

**وأثرُه إن وقع**: أجرةٌ مقدَّرةٌ على ألفي كيلومتر تُعرض على راكب، وكبتنٌ يقبل
رحلةً لا يستطيع أداءها.
<!--/جديد-->

<!--جديد-->
## أُغلق البندُ أعلاه — **حدُّ نطاق الخدمة مبنيٌّ ومنشورٌ ومقيسٌ على الإنتاج** (٢٠٢٦-٠٩-١٠)

**الخبرُ الذي فوقه صار تاريخاً لا حالاً.** `backend/app/core/service_area.py`
صندوقٌ **يحوي** البلدَ لا يطابق حدودَه، ثابتٌ في الشيفرة بقرار المالك، **ولا
رقمَ مسافةٍ فيه**؛ وبابٌ واحدٌ يناديه التقديرُ والطلبُ معاً، والطرفانِ
والمحطّاتُ تُفحص كلُّها.

**ونُشر في `be381f16`** — البوّاباتُ الستُّ خضراءُ كلُّها،
CI `34431249193`، والنسخةُ المحقَّقة `/d/taxo-backups/20260910T060236Z`
(20260910T060236Z UTC · 16M).

**والقياسُ على الإنتاج بالبايتات المنشورة، من داخل الحاوية:**

| المسار | الجواب |
|---|---|
| عمّان → إربد | PASSED |
| العقبة → إربد (~٣٥٠ كم) | PASSED — **المشروعُ لا يُمنع** |
| عمّان → طرابلس | REJECTED · 422 · `outside_service_area` |
| طرابلس → عمّان | REJECTED · 422 · `outside_service_area` |
| عمّان → إربد ومحطّةٌ في طرابلس | REJECTED · 422 |

والنصُّ المعروض: **«هذه الوجهة خارج نطاق خدمتنا — اختر مكاناً داخل الأردن.»**
والصندوقُ المنشور: `MarketBox(29.18, 33.38, 34.95, 39.3)` لـ`JO`،
و`MarketBox(19.5, 33.17, 9.31, 25.15)` لـ`LY`.

> **⚠ وحدُّ هذا القياس يُقرأ معه ولا يُسكت عنه**: هو يقيس **الشيفرةَ
> المنشورة**، **ولا يقيس مسارَ HTTP حيّاً**. فـ`POST /rides/estimate` خلف
> `RiderUser`، **ورمزُ راكبٍ على الإنتاج شيءٌ لا يملكه إلا المالك**. **فـ«تُرفض
> على شاشة الراكب» لم يُقس** — وهو بندٌ ينتظر إذنه، لا بندٌ خضّرناه.

**والبندُ المفتوحُ الباقي بحدِّه**: الصندوقُ **لا يمسك نقطةً خلف الحدود
مباشرة** — ما جاور الحدَّ من سوريةَ أو فلسطينَ أو السعوديةِ داخلُه. **والدقّةُ
مضلَّعٌ يُخزَّن ويُحرَّر، وهي ميزةٌ لها سطحُ لوحةٍ وقرارُ مالك.**
<!--/جديد-->

<!--جديد-->
## بندٌ مفتوح — **٢١١ ملفاً في نسخة العمل بنهاياتٍ CRLF والفهرسُ LF** (قِيس ٢٠٢٦-٠٩-١٠)

**المقيس بـ`git ls-files --eol`، لا المُقدَّر:**

    المتتبَّعُ كلُّه         1,344
    i/lf w/lf              1,010   ← سليم
    i/lf w/crlf              200 ┐
    i/lf w/mixed              11 ┘  **٢١١ مخالفاً** (بعد استبعاد ٣ ملفّاتِ `.ps1`
                                      التي `eol=crlf` صحيحٌ لها بحكم `.gitattributes`)
    ثنائيّ (i/-text)         115
    فارغ (i/none)              5

    backend 95 · customer-app 56 · driver-app 34 · admin-panel 12 · tools 7 · design 6 · README 1
    .py 94 · .tsx 51 · .mjs 20 · .ts 15 · .xml 12 · .md 8 · .svg 4 · .css 2

**والعلّة**: إرثُ حقبةِ ويندوز قبل النقل إلى WSL — كُتبت هذه الملفّاتُ بأدواتٍ
ويندوزيةٍ **بعد** السحب. **والفهرسُ سليمٌ كلُّه LF** (`.gitattributes`:
`* text=auto eol=lf`)، **فالمستودعُ والخادمُ لا يريان شيئاً**: الخادمُ يسحب من
GitHub، وما يصله LF.

### **ودعوى «أولُ لمسةٍ تُظهر ضجيجاً في الديف» قِيست فسقطت**

جُرِّبت في مستودعٍ مرميٍّ بالقاعدة نفسِها (`* text=auto eol=lf`)، **ولم تُمسّ
شجرةُ المشروع**:

| الفعل | ما وقع فعلاً |
|---|---|
| قلبُ ملفٍّ إلى CRLF بلا تغيير محتوى | `git diff --numstat` **فارغ**، و`status` نظيفٌ من التشغيلة الثانية (الأولى تُظهر ` M` من ذاكرةِ `stat` بائتة، **وتزول وحدَها**) |
| تغييرُ **سطرٍ واحد** مع إبقاء CRLF | `numstat = 1  1` — **سطرٌ واحدٌ لا ملفٌّ كامل** |
| `git add` ثمّ `git commit` | **لا يُعيدان كتابةَ نسخةِ العمل** — تبقى `w/crlf` |
| `git checkout` يستبدل الملفَّ فعلاً | **هنا وحدَها** تُكتب LF |

**والشجرةُ اليومَ نظيفةٌ (٠) مع وجود الـ٢١١** — فالتطبيعُ يعمل، والدِّيفُ لا
يضجّ. **فمن يصف «ضجيجاً في الديف» يصف آليّةً غيرَ التي تقع.**

### **والخطرُ الحقيقيُّ في موضعين آخرين — وأحدُهما قائمٌ الآن**

**واحدٌ من الـ٢١١ هو ملفُّ بناءِ الخلفية** — وهو **الصنفُ الوحيدُ** الذي يحذّر
منه `.gitattributes` بنصّه: «سكربتاتٌ تُنفَّذ داخل حاويات لينكس — CRLF يكسر سطرَ
الـshebang». **والبناءُ على الخادم سليمٌ لأنه من git**، والخطرُ على **بناءِ جهاز
المطوّر وحدَه**.

**والثاني: قارئُ البايتات لا قارئُ الأسطر** — حارسٌ يعدّ الحروفَ أو يبصم الأسطر
يقرأ CRLF عدداً آخر. **وقد وقع مقيساً من قبل**، و`check:docs` هو الذي أمسكه
وحدَه.

**ولم يُصلَح اليوم بقرار**: تطبيعُ ٢١١ ملفاً إيداعٌ يمسّ الخلفيةَ والواجهاتِ
الثلاثَ في يومٍ فيه رفعةٌ على الإنتاج — **وهو «فعلان يمسّان سطحاً واحداً في
جولة»**. وطريقُه حين يُفعل: `git add --renormalize .` في إيداعٍ **وحدَه**،
والمجموعةُ والحرّاسُ بعده.
<!--/جديد-->

<!--جديد-->
## طُبِّعت الـ٢١١ — **وصفرُ إيداعٍ نتج عنها، ودعوى الخطر التي كتبتُها سقطت بالقياس** (٢٠٢٦-٠٩-١٠)

**البندُ الذي فوقه يبقى تاريخاً، وهذا ما وقع بعده.**

### ١) التطبيعُ لا يُنتج إيداعاً — **العطبُ في السحب لا في المستودع**

    قبل:  i/lf w/lf 1,010  ·  i/lf w/crlf 200  ·  i/lf w/mixed 11
    بعد:  i/lf w/lf 1,221  ·  i/lf w/crlf 3 (ملفّاتُ `.ps1` وحدَها، و`eol=crlf` صحيحٌ لها)
    المخالفُ الباقي: 0  ·  أُزيل 38,179 بايتَ CR  ·  **المُدرَجُ في git: 0**

**والبرهانُ بايتاً**: بصمةُ ملفِّ بناءِ الخلفية في الفهرس `f743ed82b164`
**قبل التطبيع وبعده سواء**، والحجمُ على القرص 2,518 → 2,473 (٤٥ بايتَ CR).
**فالفهرسُ كان LF طوال الوقت**، و`git add --renormalize` لا يجد ما يودعه.
**فلا إيداعَ في هذه الجولة، ولا يُصنَع إيداعٌ فارغٌ ليطابق خطّة.**

**وثِمرتُه الوحيدةُ محلّية**: كلُّ قارئِ بايتاتٍ على هذا الجهاز صار يقرأ ما
يقرؤه CI والخادم.

### ٢) **ودعوى «الخطرُ قائمٌ في ملفِّ بناءِ الخلفية» لم تثبت**

كُتبت في البند الذي فوقه استنتاجاً من نصِّ `.gitattributes` («CRLF يكسر سطرَ
الـshebang»). **والقياسُ نفاها:**

| المقيس | الجواب |
|---|---|
| `docker build --check` على نسخةِ **LF** (2,473 بايت · 0 CR) | `Check complete, no warnings found` |
| `docker build --check` على نسخةِ **CRLF** (2,518 بايت · 45 CR) | `Check complete, no warnings found` |

**فـBuildKit يبتلع CRLF ولا يشكو**، والفرقُ صفر. **وسببُ الوهم أن القاعدةَ في
`.gitattributes` كُتبت لـ`*.sh`** — والشِّبهُ بالجوار ليس دليلاً.

**وأشدُّ من ذلك**: **صفرُ ملفِّ `.sh` كان بين الـ٢١١** — الوحيدُ الذي طابق
`\.sh$|Dockerfile` هو ملفُّ البناء. **فالصنفُ الذي تحذّر منه القاعدةُ لم يكن
مصاباً أصلاً.**

**والباقي هو الخطرُ الحقيقيُّ وحدَه**: **قارئُ البايتات** — حارسٌ يعدّ الحروفَ
أو يبصم الأسطر يقرأ CRLF عدداً آخر، **وقد وقع مقيساً من قبل** وأمسكه
`check:docs`.

> **والدرسُ في الكتابة لا في النهايات**: كتبتُ «الخطرُ قائم» **مشتقّاً من نصِّ
> قاعدةٍ لا من قياس**، ونُقل ذلك إلى تقرير. **والفرقُ بين «القاعدةُ تقول» و«قِيس
> فوقع» هو الفرقُ بين خبرٍ ودعوى** — وهو ما يحرسه هذا الملفُّ في رأسه.
<!--/جديد-->

<!--جديد-->
## حدُّ النطاق **قِيس حيّاً على شاشة الراكب من هاتفٍ على الإنتاج** (المالك، ٢٠٢٦-٠٩-١٠)

**البندُ الذي كان «لم يُقس» أُغلق — والقياسُ بيدِ المالك لا بيدِ وكيل.**

**ما فعله**: وضع دبّوساً في **طرابلس** من تطبيق الراكب على الهاتف، على
**الإنتاج**.

**وما ظهر — بنصّه لا بوصفه:**

> **«هذه الوجهة خارج نطاق خدمتنا — اختر مكاناً داخل الأردن.»**

**وثلاثةٌ معه، كلُّها من شهادته:**

1. **زرُّ «اطلب الرحلة» معطَّل.**
2. **حقلُ السعر فارغ.**
3. **فلم يُبنَ رقمٌ يُردّ بعده** — وهي بعينها العلّةُ المكتوبة في
   `service_area.py`: «حارسٌ يقبل في التقدير ويرفض عند التأكيد يجعل الراكبَ
   يبني على رقمٍ ثم يُردّ، وهو أسوأُ من ألّا يُمنع».

### **والنصُّ المقروءُ على الهاتف = النصُّ الخارجُ من الحاوية، حرفاً بحرف**

القياسُ الآليُّ السابق (٢٠٢٦-٠٩-١٠، من داخل حاوية الإنتاج) أخرج:

    OutsideServiceArea · 422 · outside_service_area
    'هذه الوجهة خارج نطاق خدمتنا — اختر مكاناً داخل الأردن.'

**وهذا ما قرأه المالكُ على الشاشة.** فالسلسلةُ لم تُؤلَّف في الواجهة ولم
تُستبدَل بنصٍّ عامّ — وهي القاعدةُ المكتوبة في `ARCHITECTURE.md`: «التطبيقُ لا
يكتب عربيّتَه لخطأٍ سمّاه الخادم»، **وقد بليت مرّةً حين قرأ العملاءُ
`body.message` والخلفيةُ تكتب `detail`**. **فهذا القياسُ يشهد لها اليوم.**

### وما بقي غيرَ مقيسٍ ويُقال

- **الجانبُ اللِّيبيُّ لم يُقس** — لا حيّاً ولا من الحاوية بأكثرَ من صندوقه.
- **وحدُّ الصندوق قائمٌ كما هو**: ما جاور الحدَّ من سوريةَ أو فلسطينَ أو
  السعوديةِ **داخلَه**. **والدقّةُ مضلَّعٌ يُخزَّن ويُحرَّر، وهي ميزةٌ لها
  سطحُ لوحةٍ وقرارُ مالك، لا إصلاحُ عطب.**
<!--/جديد-->
...
<!--جديد-->
## جولةُ كليك — المرجعُ والشاشةُ وصفحةُ المدفوعات (٢٠٢٦-٠٩-٠١)

### ١) حقلُ المرجع — **لم يُمسّ، والقراءةُ هي التي قرّرت**

**السؤال**: بماذا يطابق المشرفُ اليومَ حوالةَ كليك بكبتنٍ بعينه؟

**الجواب — الحقلُ والمسار**: الحقلُ **`provider_orders.cart_id`** وهو **نفسُه
المرجعُ المعروضُ للكبتن**؛ والمسارُ `GET /api/v1/admin/cliq-claims` ←
`cliq_subscriptions.list_pending` ← `CliqClaimOut`.

**وهو كافٍ**: `cart_id` فريدٌ بقيدٍ في القاعدة، ويحلّ إلى صفٍّ واحدٍ وإلى
`user_id` واحد. **فلا يحتاج المرجعُ زيادةً.**

> **والعطبُ في العرض لا في الحقل**: `CliqClaimOut` كان يحمل `user_id` (UUID)
> **والجدولُ في اللوحة لا يعرضه أصلاً** — أعمدتُه: المطلوب · المرجع · الطلب ·
> زرّ. **فالمشرفُ يرى حوالةً ولا شيءَ يقول من.**

**والأرقامُ للسجلّ**: سقفُ **٢٥ حرفاً** حدُّ EMVCo للحقل 62/05 (وهو نفسُه طولُ
عمود `cart_id: String(25)`)، و`cart_id` **١٩ حرفاً** (بادئةٌ + `token_hex(9)`)
— **فالمتّسع ٦ أحرف**. **والاسمُ الأولُ خارجٌ بالطول قبل أن يكون خارجاً
بالخصوصية**، **ولا «رقمَ حساب» في النظام أصلاً** — لا عمود ولا مفهوم.

### ٢–٥) شاشةُ المطالبة

زرُّ نسخٍ للمرجع وللحساب **يقول «نُسخ»** · وملاحظةُ المرجع **كبرت وأُطّرت**
ومعها سطرٌ يقول لماذا · وزرُّ **«تمّ الدفع» يُفتح بعد النسخ** وتحته **«انسخ
المرجع أوّلاً»** — **ولا زرَّ مغلقٌ صامتٌ يُقرأ عطباً** · وبعد الضغط تتبدّل
الحالُ وتظهر مدّةُ المراجعة **من الحقلين** · **وضغطةٌ ثانيةٌ تُعيد الوقتَ
الأوّل ولا تصيح**.

**وفشلُ الحافظة لا يُبتلع**: متصفّحٌ يمنعها يترك المستخدمَ أمام زرٍّ لا يفعل
شيئاً — فيُقال، ويبقى النصُّ قابلاً للتحديد باليد.

### ٦) «حوّلتُ» صار فعلاً يُختم

`0067`: `provider_orders.declared_paid_at`. **و`status = created` كانت تعني
«فُتحت الشاشة» لا «حوّلتُ»** — فقائمةُ المشرف تخلط **من فتح ونسي** بـ**من دفع
وينتظر**. وبابٌ **واحدٌ** لكلِّ الأغراض: `POST /payments/cliq/{cart_id}/declare`
— ومفتاحُه `cart_id`، **وثلاثةُ أبوابٍ لثلاثة أغراضٍ تفترق أوّلَ تعديل**.

**والقائمةُ الجديدة** `GET /admin/cliq-claims/declared` — **الأقدمُ أوّلاً**
لأنه أطولُ انتظاراً. **والرفضُ بسببٍ ≥ ٨ أحرفٍ يُعرض على صاحبه** في
`failure_reason` — وهو الحقلُ الذي يعرضه التطبيقُ أصلاً، **فلا حقلَ ثانٍ
يُخترع لمعنىً قائم**.

**وصفحةُ المدفوعات `/m/payments`** — **بطاقاتٌ لا جدول**: جداولُ اللوحة
بأعمدةٍ كسريّة، وستّةُ أعمدةٍ على هاتفٍ تصير ستّةً **مقصوصة**. وتعرض الاسمَ
والرقمَ والمبلغَ والمرجعَ والغرضَ ووقتَ الضغط، وزرّي تأكيدٍ ورفض.

**والتأكيدُ يفتح حقلَ المبلغ الذي وصل** ولا يفترضه: `confirm_payment` **يرفض
التفعيلَ بأقلَّ من الثمن**، **وشاشةٌ تُرسل المطلوبَ دائماً تُلغي ذلك الحارسَ
من حيث لا يُرى**.

### والإشعارُ لحظةَ الضغط — **مبنيٌّ ويعمل نصفَ عمله اليوم**

يُرسل إلى **كلِّ مشرفٍ في سوق الطلب** بعد الإيداع. **وصفُّ صندوق الوارد يُكتب
ولو لا عقدَ FCM ولا جهازَ مسجَّل** — فالإشعارُ **قائمٌ ومقيسٌ اليوم**، والدفعُ
إلى الهاتف يصل **يومَ يسجّل غلافُ المشرف جهازَه**. **ولم يُنتظر الغلافُ ليُبنى
الإرسال.**

### والقناةُ الخامسة — **مبنيّةٌ ومحروسة، والغلافُ لم يُبنَ**

`channels.json` صار يحمل `admin-panel`: **`ly.tajora.admin`** و
`panel.tajora.ly` (وتجريبيّه `.test` و`dev-panel`)، **و`private: true`**.
**وستّةُ أزواجٍ الآن لا أربعة.**

**والحارسُ قِيس في الاتجاهين** (2026-09-01): زُوّرت حزمةٌ معرّفُها
`ly.tajora.admin` وغلافُها `panel.tajora.ly` — **زوجٌ متّسقٌ تماماً من قناةٍ
عامّة** — ووُضعت في `landing/downloads` وبيانِها. **فمرّ فحصُ الزوج أخضرَ
ومنعها فحصُ الخصوصيّة بنصّه**، ثم أُزيلت فاخضرّ كلُّ شيء.

> **ولمَ شرطٌ ثانٍ ولم يكفِ فحصُ القناة**: غلافُ المشرف حزمةُ قناةٍ **عامّة**
> — **فالفحصُ القائمُ يمرّ عليها**. والذي يمنعها أنها **ليست لأحد**.

### ⚠ وما لم يُبنَ من الجولة

**غلافُ المشرف نفسُه** (Capacitor + نكهةُ gradle + الأيقونة) **لم يُبنَ** —
بُني ما تحته: القناةُ، والحارسُ، والصفحةُ، والإشعار. **ولا يُنشر على صفحة
التحميل بحكم الحارس.**

**والمطالبةُ اليدويّةُ للراكب** (`wallet_topup_requests` بقناة كليك) **لم
تدخل صفحةَ المدفوعات بعد**: الصفحةُ تقرأ `provider_orders` وحدَها — أي
اشتراكَ الكبتن ودَينَه. **وشحنُ محفظةِ الراكب اليدويُّ جدولٌ آخر**، وتوحيدُهما
في قائمةٍ واحدةٍ عملٌ قائمٌ بذاته.
<!--/جديد-->

<!--جديد-->
### وثلاثةُ حرّاسٍ أمسكوا جولةَ كليك — والثالثُ عطبٌ حقيقيّ

**١) `test_two_doors`** أمسك `CliqClaimOut` — صارت تُنشر من أربعة أبوابٍ
وتحمل حقولاً محسوبةً، فصُنّفت `SHARED_BUILDER`.

**٢) و`test_locks_have_tests`** أمسك قفلاً بلا شاهد، **وقِيس بالحذف**:
نُزع `.with_for_update()` من `_locked_by_cart` **فمرّت الثانيةُ بلا وقوف**،
وأُعيد فاخضرّ.

**٣) والثالثُ عطبٌ شُحن لولا الحارس**: `MissingGreenlet` في تأكيد المطالبة —
`claim_row` كانت تقرأ `order.user` **بعد الإيداع**، فتُحمَّل العلاقةُ كسولاً
خارج السياق. **وأمسكه اختبارُ تزامنٍ قائمٌ** (`test_two_confirmations_of_one_
cliq_claim_activate_once`).

> **وأُصلح مرّتين لأن الأولى لم تكن إصلاحاً**: نُقل البناءُ **قبل** الإيداع،
> **فسقط الطلبان معاً** — لأن `activate_paid_order` يُفرغ ما بينهما.
> **فالعلاجُ أن يجلب البانِي الدافعَ بنفسه** (`session.get`) ولا يعتمد على
> ترتيب المستدعي: **حارسٌ يعتمد على أن يتذكّر كلُّ مستدعٍ ترتيبَه حارسٌ سقط.**

**ورابعٌ في الاختبار نفسِه**: كان يُصفّي الاستثناءات بـ`return_exceptions=True`
ثم يقارن العدّاد، **فقرأ `Counter({409: 1})`** — رقمٌ يُقرأ «سقط قفل» وهو في
الحقيقة **استثناءٌ ابتُلع**. فأُضيف سطرٌ يصيح بالاستثناء أوّلاً. **وعدٌّ يُصفّي
ما لا يفهمه يسمّي العطبَ باسم غيره.**
<!--/جديد-->

<!--جديد-->

## غلافُ المشرف — الحزمةُ الخامسة (٢٠٢٦-٠٩-٠١)

**بُني على نمط الأربع: نكهتان، وأيقونة، ومعرّفٌ خاصٌّ لا يُنشر.**

| ما قِيس | القيمة |
|---|---|
| المعرّف · القناة العامّة | `ly.tajora.admin` ← `panel.tajora.ly` |
| المعرّف · القناة التجريبية | `ly.tajora.admin.test` ← `dev-panel.tajora.ly` |
| الاسمُ على الشاشة | «TAXO المشرف» · «TAXO مشرف تجريبي» |
| `versionCode` | 430 — مشتقٌّ من عدد الإيداعات كالأربع |
| Firebase داخل الحزمة | `taxo-84a5f` · `1:879288557931:android:b61a…` — **مقروءٌ من موارد الحزمة لا من سجلِّ البناء** |
| حارسُ Firebase | **مقيسٌ بالنفي**: بلا الملفّ → `FAILURE: google-services.json مفقودٌ أو فارغ في app/` |
| `POST_NOTIFICATIONS` | مُعلَنٌ في البيان، ومقروءٌ من الحزمة بـ`aapt dump badging` |

**والأيقونةُ مشتقّةٌ من الأصل لا مخترعة**: `Project Screens/ICON Admin.png`
بُني من `ICON Rider.png` — **الوسمُ نفسُه بكسلاً بكسل**، والعنوانُ الفرعيُّ
وحدَه استُبدل بـ«Admin» بمقاسه (٢٩٢×٥٠) وموضعه وتتبّعه. **وهو انتظارُ أصلٍ
مصمَّمٍ إن وُجد**، لا قرارُ هويّة.

### وثلاثةٌ قِيست في الطريق — تُكتب ولا تُصلَح إلا ما أُصلح

1. **`check:money-math` كان أحمرَ على `master` قبل هذه الجولة.** مفتاحُه
   `admin-panel/src/screens/Settings.tsx:993` وقد أزاحه إيداعان مني
   (`c2daa84` إلى ٩٩٨، ثمّ `1a42c66` إلى ١٠٠٠) — **وقلتُ في تقريرَيهما إن
   الحرّاسَ خضر ولم يكن هذا منها**. أُصلح المفتاحُ إلى `1000` بعد مراجعة
   الحساب (هو نفسُه: معاينةُ إدخالٍ لا مبلغٌ يُحصَّل). **والحارسُ فعل ما بُني
   له** — مفتاحُ `ملف:سطر` أظهر الإزاحةَ؛ **والذي سقط أنني لم أشغّله**.
2. **`@capacitor/android` انفرد بنسخةٍ أحدث**: `npm i` في اللوحة جلب `7.6.9`
   بينما التطبيقان على `7.6.8`. **وهي العائلةُ العاشرة**: عمودان لمنتَجٍ واحدٍ
   وأحدُهما يتخلّف صامتاً. **وثُبِّتت اللوحةُ بالقيمة نفسِها تماماً** (بلا `^`)
   — **والتطبيقان ما زالا `^7.6.8`**، فأوّلُ `npm i` عندهما يجلب الأحدث. ولم
   يُمَسّا: منطقُ بنائهما لا يُغيَّر بلا إذن.
3. **وكلاهما — الأربعُ والخامس — يحتاج JDK 21 لا 17**: `capacitor-android`
   يعلن `VERSION_21`، و`JAVA_HOME` على هذا الجهاز 17. **والبناءُ المحلّيُّ
   يمرّ بـ`JAVA_HOME=<Android Studio>/jbr`** — ولا شيءَ في المستودع يقول
   ذلك بعد.

### وما ينتظر جهازَ المالك

- **تحديثُ السرِّ `GOOGLE_SERVICES_B64`** بالملفِّ ذي الستّة عملاء. وحتى
  يُحدَّث: **تُبنى الأربعُ ويُتخطّى الخامسُ بسطرٍ باسمه** (`ADMIN_APK_SKIPPED`)
  — لا يسقط شيء.
- **تثبيتُ الحزمة على الهاتف** وقياسُ ثلاثة: أن الإشعارَ يصل حين يضغط أحدُهم
  «تمّ الدفع»، وأن نقرَه يفتح `/m/payments`، وأن رفضَ الإذن يُظهر السطرَ الذي
  يقول ذلك.
<!--/جديد-->

<!--جديد-->
## دفعةُ اللوحة — الأحدَ عشرَ بنداً: **أين تقف بالضبط** (٢٠٢٦-٠٩-٠٢)

**هذا القسمُ يُقرأ أوّلاً في الجلسة القادمة.** الترتيبُ الذي أقرّه المالك:
**١ ثمّ ٢ ثمّ ٤ ثمّ ٥ ثمّ ٦ ثمّ ٨ ثمّ ٩ ثمّ ١٠ ثمّ ١١ ثمّ ١٢** — **والبند ٣
يسقط لأنه مبنيّ، والبند ٧ (غرف الدردشة) مؤجَّلٌ بقرارٍ حتى تُبنى الدردشةُ
نفسُها.**
...
| البند | الحال |
|---|---|
| **١٢ — بحثُ الخادم** | ✅ **تمّ**: `q` مرشِّحاً على **إحدى عشرة قائمةً** باقية، وبيتُه `services/admin_search.py`. **و`EXISTS` لا `JOIN`** كي لا يتضاعف صفٌّ فيتغيّر ما تحويه الصفحة |
| **مكوّنُ القائمة** | ✅ **تمّ**: ثلاثةُ أوضاعِ ارتفاع، وحالتان فارغتان، وشريطُ أدواتٍ ثابت، وهيكلُ تحميلٍ لا تقفز فيه الصفحة — **٢٢ جدولاً كلُّها عليه** |
| **٣ — شحنُ المحافظ** | ✅ **مبنيٌّ سلفاً** (دفعةُ الأبواب) — يسقط من الدفعة بقرار المالك |
| **الوقفاتُ الثلاث** | ✅ **أقرّها المالك ٢٠٢٦-٠٩-٠١**: المصفوفةُ أحدَ عشرَ باباً بالافتراض القائم (`support` يقرأ ويحسم النزاعات لا أكثر)، وقائمةُ ما لا يُحذف كما وُضعت، وقاعدةُ النسخة (كلُّ حفظٍ صفٌّ جديدٌ برقمٍ أعلى، و`requires_reconsent` خانةٌ مستقلّة) |

### ما بُني ولا قارئَ له بعد — **يُوصَل مع بنده، ولا يُبنى ثانياً بجانبه**

| المكوّن | يُوصَل في |
|---|---|
| `components/ui/Picker.tsx` — مُنتقٍ يبحث ويعرض الاسمَ والرقم | **البند ١** (الملفُّ الشخصيّ) و**٢** و**٤**: كلُّ حقلٍ يشير إلى مستخدمٍ أو كبتنٍ أو مركبةٍ أو خطّة |
| `components/ui/Inputs.tsx` — `MoneyField` · `DateField` · `DurationField` | **البند ٢** و**٤** و**٨** و**٩**: المبالغُ والتواريخُ والمدد |

**وهذا هو «بابٌ بلا زرّ» بعينه إن بقي** — فمن يفتح الشجرةَ بعد شهرٍ يجدها
مبنيّةً ولا يعرف أنها تنتظر وصلاً. **ولا يُبنى مُنتقٍ ثانٍ ولا حقلُ مالٍ ثانٍ.**

### وحقلُ بحثٍ موصولٌ في شاشتين من ثلاثَ عشرة

| موصول | غيرُ موصول (الخلفيةُ جاهزةٌ وينقص الحقلُ في الشاشة) |
|---|---|
| الكباتن · الركّاب | الرحلات (فيها نموذجُ بحثٍ **قديمٌ خاصٌّ بها** يُستبدل) · التدقيق · المدفوعات · النزاعات · المالية (٣) · الاشتراكات · الحملات · الإحالات · السلف · الديون · رسومُ الإلغاء · مشترياتُ المركبات |

**و`endpoints.ts` يحمل `q` لكلِّها** — فالوصلُ سطرٌ في كلِّ شاشة لا بابٌ يُبنى.

### وعطبان قِيسا في الطريق وأُصلحا

1. **`LIKE` بلا تهريبٍ في الثلاثة القائمة** (`/admin/users` · `/admin/drivers` ·
   سجلُّ الرحلات): كان النمطُ `f"%{q}%"` عارياً — **فمن كتب `%` رأى الجدولَ
   كلَّه وظنّه نتيجةَ بحثه**، ومن كتب `_` طابق أيَّ حرف. **أمسكه اختبارٌ
   كُتب للجديد فوقع على القديم.**
2. **`q` في `/admin/rides` و`/admin/riders` كان موصولاً بحالةٍ لا يكتبها أحد**
   في شاشة الركّاب: نموذجُ بحثٍ بزرِّ «ابحث» — استُبدل بحقل الشريط.

### وما ينتظر المالك

- **البند ٢**: ماذا يحدث للمدّة المتبقية وللعمولة المجمَّدة
  (`commission_percent_at_purchase`) عند إلغاء اشتراكٍ من اللوحة؟ **تُعرض
  الخياراتُ ولا تُختار.**
- **البند ٩**: الإعلانُ يُبنى على `promo_banners` القائم — **ولا كيانٌ ثانٍ**.
- **البند ١٠**: مسوّدةُ السياسات والشروط **تُكتب لتُراجَع لا لتُنشر**،
  والجداولُ الثلاثةُ نائمةٌ منذ `0060` (SPEC §34) وينقصها ثلاثةُ أعمدة.
- **تحديثُ السرِّ `GOOGLE_SERVICES_B64`** بالملفِّ ذي الستّة عملاء — وحتى
  يُحدَّث يُتخطّى بناءُ غلاف المشرف في CI بسطرٍ باسمه.
<!--/جديد-->

<!--جديد-->
## البند ١ والبحث — **ما بُني، وما قِيس، وما لم يُقَس** (٢٠٢٦-٠٩-٠٢)

### البند ١ — الملفُّ الشخصيُّ الكامل

**بُني على الدرجين القائمين لا بجانبهما**، والمواصفةُ في `SPEC.md` §37.
**ولا بابَ جامعٌ في الخلفية** — كلُّ قسمٍ ينادي بابَ مفهومه، بالتزام §5-ج.

**وقِيس من DOM في متصفّحٍ حقيقيّ** (Edge على `127.0.0.1:5175`، لا لقطة):

| الدرج | الأقسام المرسومة |
|---|---|
| الكبتن | الحساب · المركبة · الاشتراك · المحفظة والدفتر · ما عليه (السلف والمستحقّات) · آخرُ رحلاته كبتناً · رسومُ الإلغاء — **ومعها الوثائقُ وتوثيقُ الجنس وسقفُ السلفة كما كانت** |
| الراكب | الحساب · آخرُ رحلاته راكباً · رسومُ الإلغاء — **ومعها المحفظةُ بزرِّ تجميدها كما كانت** |

**وصفرُ خطأٍ في سجل المتصفح**، **وصفرُ قسمٍ يقول «تعذّرت قراءة هذا القسم»**.
**ولا مركبةَ ولا «ما عليه» في درج الراكب** — مقيسٌ بالنفي.

**وبابان جديدان بزرَّيهما**: `GET /admin/drivers/{id}/vehicles` و
`GET /admin/users/{id}`. و`check:doors` **١٦٨** مساراً (كانت ١٦٦).

### ووصلُ البحث — **الأربعَ عشرةَ قائمةً كلُّها**

كانت اثنتين من أربعَ عشرة. والباقياتُ **اثنتا عشرة سطحاً** (لا إحدى عشرة كما
قيل في التسليم: **الدفعاتُ والنزاعاتُ شاشتان على بابٍ واحد**، والماليةُ
**حقلان لا واحد** لجدولَي السحب والشحن).

**وقِيس السلوكُ لا الشكل** على سجلّ الرحلات — وهو الوحيدُ الذي **استُبدل**
نموذجُه بزرِّ «ابحث»:

    صفوفُ السجل قبل البحث: 51 · بعد نصٍّ لا يطابق أحداً: 1
    ✓ النداءُ حمل `q` إلى الخادم   ✓ الجدولُ تقلّص   ✓ «لا نتائج لبحثك» لا «لا بيانات»

**وحقلان لم يُوصَلا بعللهما**: مطالباتُ كليك (`/admin/cliq-claims`) ومطالباتُ
السداد (`/admin/drivers/debts/claims`) — **بابهما لا يقبل `q`**، **وحقلٌ
يزيّن ولا يرشِّح أسوأُ من غيابه**.

### وثلاثةُ عطبٍ قِيست وأُصلحت — **وكلُّها من إيداع أمس**

**١) بحثٌ يقارن `users.id` بعمودٍ يشير إلى `drivers.id`** — في **الدفعات**
و**طلبات الصرف** و**رسوم الإلغاء**. **لا يرمي، ولا يعيد خطأً، بل يُسقط نصفَ
البحث صامتاً**: من بحث عن دفعاتِ كبتنٍ باسمه قرأ «لا نتائج» وهو «لا يبحث».

> **ولمَ لم يمسكه اختبارُ البحث**: «أن البحثَ يجد» كان مقيساً على **قائمةٍ
> واحدةٍ من أربعَ عشرة** بينما التطابقُ مقيسٌ على الأربعَ عشرة. **والفراغُ
> جوابٌ مشروعٌ لبحثٍ سليم** — فلا اختبارَ عامٌّ يفرّقه عن بحثٍ لا يبحث.
>
> **والعلاجُ شرطُ شكلٍ لا شرطُ نتيجة**: `admin_search._must_point_at` يقرأ
> **من المخطَّط** الجدولَ الذي يشير إليه العمود، فيصيح **عند بناء الشرط**.
> ويمسكه `test_a_term_that_matches_nobody_returns_nothing` على القوائم كلِّها
> لأنه وحدَه يمرّر `q` غيرَ فارغة. **ومعه اختباران يقيسان الوجدان** على
> الدفعات وطلبات الصرف.

**٢) `AdminAdvanceOut` بلا اسمِ صاحبها** بينما `AdminDebtOut` ينشره — فكانت
اللوحة تعرض **ثماني خاناتٍ من UUID** في عمودٍ عنوانُه «الكبتن»، **وجدولُ
المستحقّات تحته يعرض الاسم**. الشكلُ الثامن **يُرى بالعين لا بحارس**.

**٣) `METHOD_LABEL` مكتوبٌ ثلاث مرّات وافترق فعلاً**: «كوبون» في المدفوعات
و«خصم كوبون» في الرحلات والاشتراكات — **للقيمة نفسِها**. وبيتُه الآن
`admin-panel/src/lib/labels.ts`.

### وشكلُ الوثائق (§34) — سُدَّ في `0068` قبل أن يُبنى عليه

`doc_type` · `app` (**بلا مشرف**) · `min_accepted_version`، والفهرسُ الجزئيُّ
صار على الثلاثيّة. **والترحيلةُ تعدّ الصفوفَ ساعةَ التشغيل وتقف بصياحٍ مسمّى**
— **وحارسُها مقيسٌ بالنفي**: اختبارٌ ينزل إلى `0067`، **يزرع صفّاً**، ويشترط
الوقوفَ بنصِّه.

**والأرقامُ المقيسة**: `privacy_policies` · `user_policy_consents` ·
`org_profile` = **٠ · ٠ · ٠** على التطوير. **والإنتاجُ لم يُقَس** — والسببُ
تحت.

### ⚠ الخادمُ لا يُبلَغ — **مفتاحان، وكلاهما لا يعمل** (قِيس ٢٠٢٦-٠٩-٠٢)
...
**والضابطُ هو الذي حسم**: مفتاحُ النشر يصادق على المضيف والمنفذ والمستخدم
أنفسِهم — **فالشبكةُ والجدارُ والاستدعاء سليمة**. **وعرضُ المفتاح العامِّ لا
يحتاج العبارةَ أصلاً**: العبارةُ تُطلب **بعد** أن يقبل الخادمُ العامَّ ليُوقَّع
التحدي، **والخادمُ رفض قبل ذلك** — فلم تدخل العبارةُ في التبادل، و`SSH_ASKPASS`
لا يغيّر شيئاً.

**فالبصمةُ `SHA256:p9Nwx/Y58ImBoh21yvm+chqkngSIC9cbGaVRpoL7MNk` ليست في
`~taxo/.ssh/authorized_keys`.** **وكيف سقطت: لا يُعرف** — الملفُّ غيرُ مقروءٍ
من هنا، ولا أثرَ في المستودع. **وأفيه مفاتيحُ أخرى: لا يُعرف كذلك.**

**وعطبٌ ثانٍ**: `scripts/deploy-command.sh` على الخادم **بلا صلاحية تنفيذ** —
`126` («وُجد ولا يُنفَّذ»). **ومفتاحُ النشر لا يستطيع اليوم أيَّ شيءٍ على
الإطلاق**: أمرٌ من عندي (`id`) وأمرُه المفروض (`pull-release v9.9.9`) يعطيان
**المخرَجَ نفسَه حرفاً**، فالأمرُ المفروضُ يبتلع ما أُرسله قبل أن يُقرأ.

**والعلاجان يحتاجان صدفةً لا أملكها، ودورتُهما مغلقة**: الأولُ يحتاج صدفة،
والصدفةُ تحتاج المفتاحَ الذي يُصلحه الأول. **ولم يُلتفَّ عليهما**: الطريقُ
الباقي استغلالُ مسار `pull-release` ليكتب على القرص — **وذاك تجاوزُ حدٍّ أمنيٍّ
بناه المالكُ قصداً**، تمنعه المطلقةُ الثالثة.
<!--/جديد-->

<!--جديد-->
## عطبٌ جانبيٌّ في شاشة المتجر — **يُكتب ولا يُصلَح اليوم** (٢٠٢٦-٠٩-٠٢)

**قِيس بالعَرَض** وأنا أقيس ورقةَ تأكيد الحذف (البند ٤): شاشةُ «مركبات المتجر»
(`/vehicle-skins`) **تُخرج خطأين في سجلّ المتصفح**، مصدرُهما `SkinMapPreview`:
...
**وليس من عمل هذه الجولة**: لم يُمسّ أيُّ ملفِّ خريطة
(`LiveCanvas` · `SkinMapPreview` · `map-rtl`)، **والشاشتان الأخريان المقيستان
اليومَ أعطتا صفرَ خطأ** — درجُ الملفِّ الشخصيّ وورقةُ إلغاء الاشتراك.

**ولمَ يُكتب ولا يُصلَح**: قاعدةُ «الوقوفُ للضرورة» تقول إن عطباً جانبياً
**يُكتب بنداً ويُمضى** — وهو ليس من البنود التي أُمرتُ بها، **وإصلاحُ خريطةٍ
داخل جولةِ حذفٍ يخلط فعلين على سطحين في جولةٍ واحدة**.

**وما يُقاس قبل أن يُتَّهم الكود**: `AbortError: Actor removed` يقع حين
**يُفكَّك المكوّنُ قبل أن يكتمل تحميلُ ملحق RTL** — فالسؤالُ الأول «أيقع مع
معاينةٍ تبقى مفتوحة؟» لا «أالملحقُ مكسور؟».
<!--/جديد-->

<!--جديد-->
## البند ٦ — مسارُ الرحلة النشطة في ملفِّ الكبتن، **مبنيٌّ ومقيس** (٢٠٢٦-٠٩-٠٣)

**نصُّه §39٫٦، وتفصيلُه §42.** وهذا ما قِيس، **بالأرقام لا بالوصف**.

### ما بُني

`GET /admin/live/drivers/{driver_id}/ride` (`AdminUser`) + قسمُ «الرحلة
الجارية» في درج الكبتن + `RouteCanvas.tsx`. **ولا جدولَ ولا ترحيلة**: يقرأ
`drivers.current_ride_id` و`ride_route_points` ومفتاحَ الحضور في Redis.

### القياسُ على جهاز التطوير — رحلةٌ حقيقيةٌ لا بيانات مُلقَّمة

**بُنيت الرحلةُ بالأبواب نفسِها التي يمرّ بها الناس** (تسجيلُ دخولٍ · اتصال ·
بثّ · طلب · قبول · وصول · بدء · أربعُ بثّاتٍ متباعدةٍ بواحدٍ وعشرين ثانية):

    ride  f1669500…  in_progress
    route 5 نقاط ملتقَطة بنافذة العشرين ثانية · route_truncated=false
    position {lat 31.9800002183158, lng 35.88999778032303,
              seconds_since_update 1, stale false}

**والقيدُ في التدقيق بعد ثلاثة استطلاعات — واحدٌ لا ثلاثة**:

    read  driver_live  220596df-974d-4374-b3f9-228aec949396  22:32:54Z

### وقراءةُ DOM من متصفّحٍ لا `curl` — الحالان معاً

**رحلةٌ جارية** (درج «زيد السائق»): لوحُ الخريطة **٣٩٥×١٧٠ بكسل**، **ثلاثُ
علامات** بعناوينها `["نقطة الانطلاق","الوجهة","موقعُه الآن"]`، الشارتان
`["جارية","يبثّ الآن"]`، والحقائق:

    من: نقطة على الخريطة · إلى: نقطة على الخريطة
    قَبِلها: 3 سبتمبر 12:30 ص · انطلقت: 3 سبتمبر 12:30 ص
    نقاط المسار: 6 نقطة · آخر بثّ: منذ 7 ثانية

**والخطُّ مرسومٌ فعلاً** — قُرئ في اللقطة لا في التوثيق.

**ولا رحلة** (درج «هناء السائقة»): **صفرُ ألواحِ خريطة، وصفرُ أخطاءٍ في
السجلّ**، ونصُّه:

    لا رحلةَ جاريةً الآن — ولا خريطةَ بلا رحلة. وسجلُّ الرحلات تحت هذا القسم.

### وعطبٌ لغويٌّ أمسكته العينُ لا الحارس

**كانت الجملةُ «لا رحلةَ جاريةً *له* الآن»**، **وقُرئت على درج سائقةٍ معتمدة**
فبانت خطأً في العربية. **ولا حارسَ يمسكها**: `test_label_gender.py` يحرس
تسمياتِ الأخطاء لا نصوصَ الشاشات. **والعلاجُ نزعُ الضمير**، فالجملةُ تستقيم
بلا ذكر جنس — وهو أسلمُ من فرعٍ بجنسين في كلِّ نصّ.

### ⚠ وما لم يُقَس — ولا يُقرأ سلامة

1. **البصمةُ الحقيقيةُ للخريطة على هذا الجهاز**: `api.mapbox.com` **يردّ ٤٠٣
   على بلاطات الخرائط**، فالخلفيةُ تحت الخطِّ والدبابيسِ **فارغة**. **وهذا
   قائمٌ قبل هذا البند**: قِيس `/vehicle-skins` (`SkinMapPreview`، لم يُمسّ)
   فأعطى **الأخطاءَ الخمسةَ نفسَها بحرفها** ومنها ٤٠٣. **فليس من عمل هذه
   الجولة**، ولا يُقاس منه أن العلامات في مواضعها الصحيحة على الشارع.
2. **وأخطاءُ `map-rtl` ليست خاصّةً بـ`SkinMapPreview`** — وهذا خبرٌ جديدٌ
   للبند الجانبيّ المفتوح: **كلُّ خريطةٍ في اللوحة تُخرجها في وضع التطوير**،
   ومعها `applyProjectionUpdate`. **والسببُ المرجَّح `StrictMode`** — يركّب
   المكوّنَ ويفكّكه ويعيده، **فيُنادى `map.remove()` قبل أن يكتمل تحميلُ ملحق
   العربية**، وذاك نصُّ `AbortError: Actor removed` بعينه. **ويُقاس بإطفاء
   `StrictMode` أو ببناء إنتاج، ولم يُقَس** — وليس من عمل هذه الجولة.
3. **ولا هاتف**: القسمُ لوحةٌ على مكتب، ولم يُفتح على جهازٍ محمول.
4. **ولم يُقَس أن الخطَّ يُعاد بعد تبديل السمة** بالعين — المسارُ مكتوبٌ
   (`style.load`) ولم يُشغَّل مبدّلُ السمة والدرجُ مفتوح.
5. **وحدٌّ مكتوبٌ لا مقيس**: **جوابٌ ساقطٌ واحدٌ يوقف الاستطلاع** حتى يُطوى
   القسمُ ويُفتح. `ProfileSection` ترسم الخطأَ **بدل** أبنائها، فيُفكَّك
   المؤقّتُ معهم. **وتُرك كذلك بقرار**: إصلاحُه يمسّ مكوّناً تشترك فيه سبعةُ
   أقسام، **وشاشةٌ تعيد المحاولةَ صامتةً على ٤٠٣ أسوأُ من واحدةٍ تقف وتقول**.
<!--/جديد-->

<!--جديد-->
## البند ٨ — لوحةُ الإصدارات والتحديثُ الإلزاميّ، **مبنيٌّ ومقيس** (٢٠٢٦-٠٩-٠٣)

**نصُّه §39٫٨، وتفصيلُه §43.** وهذا ما قِيس، **بالأرقام لا بالوصف**.

### ما بُني

جدولُ `app_releases` (ترحيلة `0071`) · `GET /public/app-version` (عامٌّ بلا
جلسة) · أربعةُ أبوابٍ إداريةٍ على `settings.write` · شاشةُ «إصدارات التطبيقات»
تحت «النظام» · **بوّابةُ `update-gate.tsx` في التطبيقات الثلاثة** وحارسُها
`check:update-gate`.

### القياسُ على الأبواب — مجموعةٌ ثمّ إنتاج

    اختباراتُ البند: 16 خضراء
    ترحيلةٌ: downgrade 0071→0070 ثمّ upgrade — خضراء، والرأس 0071

**وعلى قاعدة التطوير بأبوابها الحقيقية:**

    بلا الإذن الثاني          → 422 «رفعُ الحدِّ يقفل … أعِد كتابة الرقم»
    برابطِ غلافِ التطبيق      → 422 «ينتهي بـ.apk لكنه يردّ صفحةَ ويب»
    برابط صفحة التحميل       → 201
    build=None → ok · 400 → ok · 385 → optional · 370 → forced
    تطبيقٌ بلا سجلّ (driver)   → ok بحقولٍ فارغة

### ⚠ والعطبُ الذي كشفه القياسُ في الطريق — **وأنشأ شرطاً ثانياً**

**وُضع في حقل الرابط `https://app.tajora.ly/downloads/taxo-rider.apk`** — وهو
**غلافُ تطبيق الراكب لا صفحةُ التحميل** — **فقُبل بـ201**. والسببُ أن العنوان
يردّ **`200` ومعه `text/html`**: تطبيقُ الويب نفسُه، أوّلُ بايتاته
`<!doctype html>` (قِيس بـ`curl -r 0-40`).

**فحالةُ الردِّ وحدَها كانت تُخضِّر رابطاً يعطي صاحبَ الهاتف صفحةً بدل
الحزمة** — وهو بعينه ما تمنعه جملةُ §39٫٨. **والمضيفُ الصحيح `taxo.tajora.ly`**
ردَّ `application/vnd.android.package-archive` **و404 لِما لا وجودَ له**.

**فأُضيف الشرطُ الثاني**، ضيّقاً بقصد: `.apk` يجيب `text/html` **يُرفض**،
**وصفحةُ تحميلٍ لا تنتهي بـ`.apk` تمرّ** — تلك تجيب HTML بحقّ والزرُّ يفتحها.

### وقراءةُ DOM من متصفّح — الشاشةُ والبوّابة

**شاشةُ اللوحة** (`/releases`): التحذيرُ يظهر لحظةَ يرتفع الحدُّ
(«يوقف كلَّ من حزمتُه أقلُّ من 395»)، **وورقةُ الإذن تسمّي التطبيقَ والرقم**،
**وزرُّ المتابعة معطَّلٌ حتى يُعاد كتابةُ الرقم** (`true` ← `false` مقيساً)،
ثمّ صار الصفُّ **«الحاكم»** وسبقه صفٌّ لم يعد كذلك. **صفرُ أخطاءٍ في السجلّ.**

**وبوّابةُ التطبيق** — قِيست بمنصّةٍ أصليّةٍ مزيَّفة
(`CapacitorCustomPlatform`) **بلا حرفٍ يتغيّر في الشيفرة**:

- **البوّابةُ تنادي البابَ فعلاً عند الإقلاع** — رُصدت النداءات.
- **وبلا رقمِ حزمةٍ تُرسل بلا `build`، والجوابُ `ok`** — **فلا يُقفل من لا
  نعرف نسختَه**، مقيساً في متصفّح.
- **والإلزاميُّ يستبدل الصفحةَ كلَّها**: نصُّه، ورابطُ التحميل بعنوانه
  الصحيح، **وصفرُ أزرارٍ في الصفحة** — لا زرَّ إغلاقٍ ولا خلفيةَ تُنقر.
- **والاختياريُّ يظهر فوق اللوحة العاملة**، و«لاحقاً» يُخفيه ويكتب كتماً
  **٦٫٠٠ ساعة بالضبط** = `reminder_hours` القادمة من اللوحة.

### ⚠ وما لم يُقَس — ولا يُقرأ سلامة

1. **لم تُبنَ حزمةُ أندرويد ولم تُفتح على هاتف.** فما قِيس أن الشيفرة تفعل ما
   وُصف **في متصفّحٍ بمنصّةٍ مزيَّفة**، **ولم يُقَس `App.getInfo()` الحقيقيُّ
   على جهاز** — وهو الموضعُ الوحيد الذي يأتي منه الرقم. **ويلزمه جولةُ هاتف.**
2. **ولم يُقَس أنّ ملحقَ Capacitor يعطي `build` عدداً صحيحاً**: التوثيقُ يقول
   `build: string`، والبوّابةُ تقرؤه بـ`Number(...)` وتردّ `null` عند الفشل —
   **فالسقوطُ آمنٌ ولكنّه غيرُ مقيس**.
3. **والقفلُ لم يُختبر على تطبيقٍ حقيقيٍّ في يد إنسان** — وهو أخطرُ فعلٍ في
   هذه اللوحة: **رقمٌ خاطئٌ يوقف الجميع دفعةً واحدة**.
4. **وجدولُ التطوير تُرك فارغاً بقصد** بعد القياس: صفٌّ باقٍ كان يفرض تحديثاً
   على حزمةِ قناةٍ تجريبيةٍ في أوّل جولةِ هاتف.
<!--/جديد-->

<!--جديد-->
## البندان ٩ و١٠ — **الإعلانُ مبنيٌّ سلفاً، والسياساتُ سُلِّمت مسوّدة** (٢٠٢٦-٠٩-٠٣)

### البند ٩ — ساقطٌ لأنه مبنيّ (§44)

**نصُّه شرطٌ واحد**: «يُبنى على `promo_banners` القائم — ولا كيانٌ ثانٍ».
**وقِيس بالأبواب لا بقراءة ملفّات**:

    لافتةٌ لجمهور الركّاب  → سطح rider: ظهرت · سطح driver: لا
    لافتةٌ لجمهور الكباتن  → سطح driver: ظهرت · سطح rider: لا
    لافتةٌ نافذتُها انقضت  → لم تظهر في أيٍّ منهما
    حذفُ لافتةٍ عُرضت     → 422، تُطفأ ولا تُحذف

**فلم يُبنَ شيءٌ ثانٍ بجانبه.** وتفصيلُ الحلقات في §44، **ومعه ما لم يُبنَ ولم
يُطلب**: عدّادُ ظهورٍ وضغطاتٍ للافتة.

### البند ١٠ — مبنيٌّ، والنصوصُ الثمانِ مسوّدة (§45)

`services/policies.py` · `routers/admin_policies.py` · شاشةُ «السياسات
والشروط» · `scripts/seed_policies.py`. **ولا ترحيلةَ جديدة** — الشكلُ سُدَّ في
`0068`.

**والقياسُ**:

    اختباراتُ البند: 13 خضراء
    بذرُ المسوّدات: 8 نسخ (سوقان × وثيقتان × تطبيقان) — كلُّها غيرُ منشورة
    وتشغيلٌ ثانٍ للبذر: 0 مكتوبة — «موجودةٌ، تُترك»

**وقراءةُ DOM من متصفّح**: ورقةُ النشر تسمّي النسخةَ والوثيقةَ والتطبيق وتقول
ما سيُنزع؛ **وزرُّ الحذف معطَّلٌ على المنشورة** (`true`)، **ويعود صالحاً بعد
السحب** (`false`)؛ والسحبُ يُرجع «لا نسخةَ منشورةً … ولا يُسأل أحدٌ عن قبولها
بعد». **صفرُ أخطاءٍ في السجلّ.**

**وبابُ السحب أُضيف ولم يكن في النصّ** — بقاعدةٍ مكتوبةٍ في هذا المستودع:
«مفتاحٌ بُني ليوقف الأذى **ولا يُطفأ** — بابٌ بلا زرّ في اتجاهٍ واحد». **ومن
نشر نصّاً في سوقٍ لم يُفتح** كان لا يملك إلا أن ينشر فوقه غيرَه.

### ⚠ وما لم يُقَس ولا يُبنى — **بقرارٍ مكتوب**

1. **لا شاشةَ قبولٍ في تطبيقٍ ولا بابَ قبول**: الوثائقُ مسوّداتٌ، **ووثيقةٌ
   غيرُ منشورةٍ لا يُسأل عنها أحد** — فبابُ قبولٍ اليومَ لا يجيب إلا «لا شيء».
   **والشكلُ جاهزٌ له** (`user_policy_consents` و`min_accepted_version`
   مقيسان)، **فما يبقى السطحُ لا القاعدة**.
2. **ولم يقرأ إنسانٌ النصوصَ الثمانِ بعد** — وهي مسوّدةُ وكيلٍ لا نصُّ محامٍ.
   **ومراجعتُها هي البند** (§39٫١٠)، **ولا تُنشر قبلها**.
3. **وقاعدةُ التطوير عليها الثمانِ كلُّها مسوّدةً** بعد القياس: نُشرت واحدةٌ
   أثناءه ثمّ **سُحبت**، فلا منشورةَ فيها الآن.
4. **ولم يُقَس السلوكُ على الإنتاج** — الجداولُ هناك فارغةٌ كما كانت، ولا
   يُشغَّل البذرُ عليها إلا بإذنٍ ومعه نسخة.
<!--/جديد-->

<!--جديد-->
## البند ١١ — التحكّمُ بالمستخدمين والتواصلُ الفرديّ، **مبنيٌّ ومقيس** (٢٠٢٦-٠٩-٠٣)

**نصُّه §39٫١١، وتفصيلُه §46.**

### ما بُني

بابان على `users.manage`: `PATCH /admin/users/{id}` (الاسمُ والبريدُ لا أكثر)
و`POST /admin/users/{id}/notify`؛ و`notifications.publish_admin_message` تنادي
`_safe_notify` القائمة؛ **وقسمُ «التحكّم والتواصل»** في الملفِّ المشترك —
**بالحظر في درج الكبتن وبلا حظرٍ في درج الراكب**؛ **وشارةُ إيقاف الحملة**
في قسم «الحساب».

### القياس

    اختباراتُ البند: 7 خضراء
    الحرّاسُ الساكنون: خضر

**وDOM من متصفّح — درجُ الكبتن**: القسمُ موجود، وحقولُه
`profile_name · profile_email · message_title · message_body · block_reason`،
**وأزرارُه الثلاثةُ معطَّلةٌ عند الفتح** (لا تعديل، ولا نصّ، ولا سبب)؛ وبعد
إرسال رسالةٍ حقيقيةٍ ظهر **«أُرسلت الرسالة إلى صندوق وارده.»** وعاد الزرُّ
معطَّلاً (الحقولُ فُرِّغت). **صفرُ أخطاءٍ في السجلّ.**

**ودرجُ الراكب**: القسمُ موجود، **وأزرارُه `["احفظ البيانات","أرسِل"]` لا
ثالثَ لها** — فلا زرَّ حظرٍ ثانٍ في درجٍ يحمله سلفاً.

### ⚠ عطبٌ جانبيٌّ قِيس ولم يُصلَح — **بقرار**

**درجُ الراكب ينادي محفظةً بلا إعلان جانبها**، فحسابٌ يحمل الدورين يردّ
**409 `wallet_owner_undecided`** وتبقى البطاقةُ على دوّارةٍ أبداً. قِيس على
«عمر الراكب» في قاعدة التطوير. **والخلفيةُ مُحقّة** — والناقصُ إعلانُ الجانب
في نداء اللوحة. **وسطحٌ آخرُ في الجولة نفسِها لا يُلمس** (§46٫٦).

### ⚠ وما لم يُقَس

1. **شارةُ إيقاف الحملة لم تُرَ بالعين**: لا حسابَ موقوفٌ بالحملة على قاعدة
   التطوير، **والحملةُ نفسُها لم تُطلق قطّ** (§32٫٧). فما قِيس أن الحقلَ
   يُقرأ ويُرسم، **لا أنه ظهر لحسابٍ موقوف**.
2. **ولم يُقَس وصولُ الرسالة إلى هاتف**: لا عقدَ FCM مفعَّلٌ على التطوير،
   **فالمقيسُ صفُّ صندوق الوارد** — وهو ما يجعل الجرسَ يعرض ما جرى لا ما نجح
   إرسالُه (قاعدةُ `_safe_notify` نفسُها).
3. **ولم يُقَس الحظرُ من درج الكبتن بالضغط** — الزرُّ رُسم ومُقيس معطَّلاً بلا
   سبب، **ولم يُحظر حسابٌ حقيقيّ**.
<!--/جديد-->
<!--جديد-->
---

## النقلُ إلى داخل WSL — ما قِيس قبل وبعد (٢٠٢٦-٠٩-٠٣)

**شجرةُ العمل صارت `~/prj/TAXO` في `Ubuntu-24.04`، و`D:\prj\TAXO` باقيةٌ لم
تُمسّ ولم يُحذف منها شيء** — **نسخٌ لا نقل**، بقرار المالك.

### الحكم — بمقياسٍ واحدٍ في الطرفين
...
**والمكسبُ أقلُّ ممّا وعدت النظرية**، **والعلّةُ أن الدعوى كانت خاطئة**:
قاعدةُ التطوير في **مجلَّدٍ مسمّى داخل آلة Docker** (`taxo-app_taxo_pgdata`)
لا على `D:` — **فـ`TRUNCATE` لكلِّ اختبارٍ لم تعبر جسرَ 9p يوماً**. الذي كان
يعبره **ربطُ `./backend` وحدَه**، وثمنُه أربعُ دقائق لا أربعون.

### وما قِيس بعد النقل — بفعلٍ لا بافتراض

- **المجموعة**: ١٣٥٦ ✅ · `EXIT=0` · صفرُ ساقط.
- **الحرّاسُ الساكنون**: `rc=0` في **١٢ ثانية** (١٨ حارساً + حرّاسُ الثلاثة).
- **بناءُ اللوحة**: `rc=0` في **٣٩ ثانية**، و`dist` ٣٫٥ م.ب.
- **`npm ci`**: ١١ + ٧ + ٦ = **٢٤ ثانية** للثلاثة.
- **git**: ٤٤٥ إيداعاً · ٤١ وسماً · ٦ فروعٍ محلّيّة · `fsck` نظيف — **مطابقٌ
  للأصل حرفاً**. **وشجرةُ العمل المرتبطةُ إلى `D:` قُلِّمت في الجديدة وحدَها.**
- **الخطّاف**: قِيس في الاتجاهين (انظر `GUARDS.md`).
- **Docker**: كلُّ رباطٍ يُحلّ إلى `/home/loly3/prj/TAXO/…`، **وصفرُ مسارِ
  مضيفٍ خارج شجرة المشروع**، و`.env.local` بصلاحية `600` يُقرأ.
- **SSH**: `SSH_OK` · `vmi3521522` · `taxo` · `rc=0` **بالمفتاح المنقول**
  (`SHA256:p9Nwx/…`، صلاحية `600`).

### ⚠ وما لم يُقَس — ولا يُقرأ سلامة

1. **لم تُبنَ حزمةُ أندرويد من داخل لينكس ولا تُبنى**: لا SDK ولا JDK في
   التوزيعة، و`local.properties` ×٣ تشير إلى SDK ويندوز.
2. **ولم تُشغَّل جولةُ Playwright**: لا `msedge` ولا Chrome ولا Chromium في
   التوزيعة، و`COMMANDS.md` يشترط Edge **المركّب**.
3. **والحاوياتُ الثلاث ما زالت تربط `D:`** — أُقلعت `db` و`redis` من الشجرة
   الجديدة وحدَهما. **ولذلك قال `check:served` «لم يُقَس» لعمودَي المحلّيِّ
   والنفق** — **وهو الحارسُ صادقٌ لا عطب**.
4. **ولم يُقَس شيءٌ على الإنتاج** غيرَ **قراءتين**: وضعُ
   `deploy-command.sh` (`-rw-rw-r--`) وبصمتا `authorized_keys`.

### وحجمُ ما نُسخ وما لم يُنسخ

**نُسخ** ٣٬٦٤١ مُدخَلاً · ١٨٢ م.ب في **٨٠ ثانية** بـ`rsync`، ومعه `.git`
كاملاً وكلُّ غيرِ المتعقَّب. **ولم يُنسخ ≈١٫٤٨ ج.ب في ≈٦٦٬٥٠٠ ملفّاً**:
`node_modules` ×٤ · `*/android/app/build` و`.gradle` · `dist` ×٣ ·
`__pycache__` و`.pytest_cache` و`.ruff_cache` و`*.tsbuildinfo`.
**وقِيس أن صفرَ ملفٍّ متعقَّبٍ يقع تحت أيٍّ من تلك الأسماء** — فالاستثناءُ لا
يُسقط مصدراً.

---

## اعتمادُ الشجرة الجديدة — الحاوياتُ والنفقُ تحوّلا، والأعمدةُ الثلاثة (٢٠٢٦-٠٩-٠٣)

**كلُّ حاويةٍ صارت تربط `/home/loly3/prj/TAXO/…`، والنفقُ معها** — مقيساً من
`docker inspect` لا من ملفّ compose. **و`check:served` أعطى الأعمدةَ الثلاثةَ
خضراءَ في التطبيقات الثلاثة، وهي أوّلُ مرّةٍ يكتمل فيها هذا القياسُ في
المشروع**: كان عمودُ النفق يقول «لم يُقَس (HTTP 530)» في كلِّ جولةٍ سابقة.

    admin-panel   المبنيّ = المحلّيّ = dev-admin.tajora.ly   assets/index-C770noDi.js
    customer-app  المبنيّ = المحلّيّ = dev-app.tajora.ly     assets/index-CVhzOZBB.js
    driver-app    المبنيّ = المحلّيّ = dev-driver.tajora.ly  assets/index-DukPcDGW.js

### وثلاثةُ أشياءَ كسرها المسارُ الجديد، وكلُّها ظهرت هنا لأوّل مرّة

**١) `${USERPROFILE}` — الافتراضُ السادس، ولم يمسكه جردُ اليوم.** ربطُ اعتماد
النفق في `docker-compose.tunnel.yml` كان `${USERPROFILE}/.cloudflared` —
**متغيّرُ بيئةٍ ويندوزيّ**، وغيابُه في لينكس يجعل المسارَ `/.cloudflared`.
**والجردُ بحث عن مساراتٍ حرفيّةٍ (`C:\`) لا عن متغيّرات**، فلم يظهر إلا حين
احتاجه النفق. **والعلاجُ متغيّرٌ مسمّى واحتياطُه سلوكُ ويندوز حرفاً**
(`${TAXO_CLOUDFLARED_DIR:-${USERPROFILE}/.cloudflared}`)، **ومقيسٌ في
الاتجاهين**: بقيمةٍ مصرَّحة → `/home/loly3/.cloudflared`، وبـ`USERPROFILE`
مصطنَع → `<هو>/.cloudflared`.

**٢) وصلاحيةُ سرٍّ حقيقيةٍ تمنع حاويةً — على ويندوز لم تكن تُقاس أصلاً.**
صورةُ `cloudflared` تعمل بمستخدم **65532**، واعتمادُ النفق `600` ومجلَّدُه
`700` بملك المستخدم `1000`. **فخرجت الحاويةُ بـ`1` قائلةً «الملفُّ غيرُ موجود»
وهو موجودٌ ومقيس.** **وعلى drvfs كلُّ شيءٍ يُعرض `777` فلا يظهر الأثر.**

> **ولا تُرخى صلاحيةُ سرٍّ لأجل حاوية**: صُرِّح مستخدمُ الحاوية بدلَها
> (`user: "${TAXO_TUNNEL_USER:-65532:65532}"`)، **والاحتياطُ مستخدمُ الصورة
> نفسُه** فلا يتغيّر شيءٌ لمن يعمل على ويندوز.

**٣) وسياسةُ `restart: "no"` هي التي أبقت النفقَ ساقطاً — لا عطبٌ فيه.**
خرج بـ`255` لحظةَ `wsl --shutdown` (`FinishedAt` يوافقها، **ولا سطرَ أخيرَ في
سجلّه** — قُتل ولم يُنهَ)، **وكلُّ إخوته `unless-stopped` فعادوا وحدَه لم
يعد**، وهو مصرَّحٌ في `docker-compose.tunnel.yml`. **فليس من عملنا، ودليلُه
مقيس.**

**وأُقلع النفقُ وحدَه في الجولة التالية (٢٠٢٦-٠٩-٠٣، ١٢:٤٢ محلّيّاً)** بـ`docker
start taxo-tunnel` — **لا بـ`compose up`**: الحاويةُ القائمةُ تحمل
`TAXO_CLOUDFLARED_DIR` و`TAXO_TUNNEL_USER` محلولَين (`/home/loly3/.cloudflared`
و`1000:1000`)، **وإعادةُ إنشائها بلا تصديرهما كانت تُرجعها إلى الاحتياطَين
الويندوزيَّين** — `${USERPROFILE}/.cloudflared` و`65532:65532` — وهما بعينهما
العطبان ١ و٢ فوق. **فالبدءُ لا الإنشاء.** وسُجِّلت أربعةُ اتّصالاتٍ في ثلاث
ثوانٍ (`connIndex=0..3`، `quic`، `pmo01`/`mrs06`/`mrs07`)، وأجاب المضيفون بعدها
`dev-app`/`dev-driver`/`dev-admin` **200**، و`dev-api` **404 على `/` و200 على
`/health` و`/api/v1/config`** — **وهو مطابقُ أصلِه حرفاً** (`localhost:8001/`
يجيب `404` أيضاً، فلا جذرَ في FastAPI)، **فلا يُقرأ الـ404 نقصاً في النفق**.

### ⚠ وخانةُ `autoMemoryReclaim` فارغةٌ لأنها لا تُقاس من داخل جلسة — **ولا تُقرأ نفياً**

**قِيس ٢٠٢٦-٠٩-٠٣**: بعد نقل `autoMemoryReclaim=gradual` من `[wsl2]` إلى
`[experimental]` في `C:\Users\loly3\.wslconfig`، **لا سبيلَ إلى إثبات اختفاء
التحذير `wsl: Unknown key 'wsl2.autoMemoryReclaim'` من داخل جلسةٍ تعمل**:
التحذيرُ يُطبع **عند إقلاع الآلة الافتراضية وحدَه** لا عند كلِّ أمر، فمن يشغّل
`wsl.exe … -- echo` على آلةٍ قائمةٍ يرى صمتاً **لا يفرّق بين «أُصلح» و«الآلةُ
كانت قائمةً سلفاً»**.

**فقياسُه يقتضي `wsl --shutdown` ثمّ بدءاً نظيفاً يُلتقط مخرجُه** — وهو فعلٌ
يقتل الآلةَ والحاوياتِ معها، **فلا يُفعل ضمناً في جولةِ قياس**، ويبقى **بيد
المالك**.

**وما يُقاس من داخل الجلسة ثلاثةٌ لا يُغني أحدُها عن الرابع:**

- **قسمُ `[wsl2]` مقروءٌ يقيناً** — `MemTotal: 6067408 kB` و`SwapTotal:
  6291456 kB` (٦ GiB حرفاً) يوافقان `memory=6GB` و`swap=6GB`. **وهذا لا يقول
  شيئاً عن `[experimental]`.**
- **وسطورُ النواة تُطبع ولا تُفسَّر**: `Free page reporting enabled` ·
  `hv_balloon: Cold memory discard hint enabled with order 9` ·
  `Max. dynamic memory size: 6144 MB`. **ولم يُقَس أنها تظهر أو تغيب بتبديل
  المفتاح**، فلا تُحمَّل شهادةً عليه.
- **وأثرُ المفتاح على الذاكرة لا تكفيه عيّنتان**: `vmmemWSL` `WorkingSet`
  ٢٬٤٢٩ م.ب ثمّ ٢٬٧١٥ م.ب بعد ٦٨ ثانيةً على آلةٍ عمرُها خمسُ دقائق —
  **ارتفاعٌ لا هبوط**، وهو لا يشهد باسترجاعٍ ولا بضدّه. **الأثرُ يُقرأ بجولةٍ
  كاملة**: حِمْلٌ يرتفع ثمّ يهبط، والقياسُ بعد الهبوط.

> **وهذا هو درسُ «حارسٌ يُشغَّل حيث لا يملك ما يقيسه يعلن ‹لم يُقس› لا ‹سليم›»
> في ثوبٍ جديد** (`GUARDS.md`): الخانةُ الفارغةُ هنا **ليست شهادةَ سلامةٍ ولا
> شهادةَ عطب** — هي إعلانُ أن الأداةَ لم تكن بيد من قاس.

### وحارسان أحمران عاشا على `master` إيداعاً كاملاً — كشفهما المسارُ الجديد

**كلاهما من `ebe87c7` (البند ٨)، وكلاهما مستورٌ بسببٍ مختلف:**

**أ) `check:sheet` أحمرُ ولم يُشغَّل**: هو في `npm run build` لتطبيق الراكب
وحدَه **وليس في `scripts/guards.sh`** — والمشروعُ لا يدفع فلا CI. **وهو عينُ
درس `check:money-math`.** `src/lib/update-gate.tsx` كان بطاقةً بلا سقفٍ ولا
منطقةِ تمرير، **و`release_notes` نصٌّ يكتبه المشرفُ بلا حدِّ طول** — **وهذه
شاشةٌ لا مخرجَ منها إلا زرُّ التحميل**، فقصُّها يقصّ المخرج. أُصلح في الثلاثة
معاً (يطابقها `check:update-gate` بايتاً)، **وأُدخل `check:sheet` في
`guards.sh`** فصار الخطّافُ يمرّ عليه.

**ب) خطأُ ترجمةٍ لم يُقَس لأن البناءَ لم يعمل**: `AppVersion` مستعملٌ في
`src/api/endpoints.ts` بلا استيراد **في تطبيقَي الراكب والكبتن معاً**
(`admin-panel` وحدَه يستورده)، وأوّلُ بناءٍ في لينكس أعطى
`error TS2304: Cannot find name 'AppVersion'`.

> **⚠ وقيل أوّلاً إن `tsconfig.app.tsbuildinfo` سترَه — والقياسُ أسقط ذلك في
> اليوم نفسِه**: الذاكرةُ `01:52:40` والمصدرُ `01:55:23` **أحدثُ منها**، فأيُّ
> تشغيلٍ تالٍ كان سيعيد الفحص؛ و`dist` بعمر أربعةِ أيامٍ يشهد أن البناءَ لم
> يعمل منذ ٣٠ آب. **فلم تستره ذاكرة، بل لم يشغّل أحدٌ شيئاً** — **وهو عينُ
> سبب `check:sheet`، فالدرسُ واحدٌ لا اثنان** (`GUARDS.md`).

### و`google-services.json` الثلاثةُ ببصمةٍ واحدة — **مقصودٌ بقول المالك**

ملفُّ المشروع الواحد يحمل عملاءَ كلِّ الحزم، **وأعطاه المالكُ هكذا**
(٢٠٢٦-٠٩-٠٣). **فلا يُسأل عنه ثانية**، ولا يُقرأ تطابقُ البصمة عطباً.
<!--/جديد-->

<!--جديد-->
## بندُ التسهيل — **الفروعُ السبعة، وثلاثةُ بنودٍ وُلدت منه** (٢٠٢٦-٠٩-٠٣)

**المواصفةُ في `SPEC.md` §47 بخمسةَ عشرَ فرعاً**، وهذا خبرُ ما وقع.

### ما بُني وقِيس

| الفرع | المقيس |
|---|---|
| **١ الاقتراحات** | ثلاثةُ منتقياتٍ مصنوعةٍ باليد استُبدلت بـ`Picker` · `level_required` صار اختياراً |
| **٢ الحقول** | المدّةُ **١٦ حقلاً** · التاريخُ **٦** · المالُ **٤** |
| **٣ الجماعيّ** | «اعتمِد ما عاينتَه» — **واحدٌ من ثلاثة، والاثنان بعلّتيهما** |
| **٤ البحثُ العامّ** | بابٌ واحدٌ · أربعةُ أصنافٍ · أربعُ وجهات · **صفرُ بابٍ ثانٍ للفتح** |
| **٥ الجمل** | **ستٌّ بسند، وسبعٌ تُسمّى ساكتة** |
| **٦ القياس** | الخمسُ: −١ · −٢ · **صفر** · −١ · −١ |
| **٧ الصلاحيات** | اختبارٌ على ما يقع — **والقوائمُ الأربعُ بلا `require_permission` اليوم** |

**والمجموعةُ الكاملة خضراءُ مرّتين**: `1356 passed` قبل الخلفية (`0:40:21`)،
ومرّةً بعدها. **وملفُّ البحث وحدَه ٩ اختباراتٍ في ٢١٫٩٤ ثانية.**

### وعطبان أمسكهما أوّلُ تشغيلٍ للاختبار الجديد — **ولم يكن ليُمسكهما قياسٌ يدويّ**

**١) `EXISTS` على `users` داخل استعلامٍ يضمّ `users`.** `user_clause` استعلامٌ
متداخل، **وضمُّ `users` نفسِه في الخارج يجعل SQLAlchemy يربط الداخلَ بالخارج
آلياً فيمحو كلَّ FROM**:

    InvalidRequestError: returned no FROM clauses due to auto-correlation

**والعلاجُ اسمٌ مستعار**: الضمُّ للعرض، والشرطُ للمطابقة. **ولا يظهر هذا في
مراجعةِ عين** — الشيفرةُ تُقرأ صحيحةً تماماً.

**٢) والمحفظةُ تحتاج سوقاً مفعَّلاً**: `feature_disabled` من `topup_wallet` بلا
`jordan_settings`/`jordan_wallet`. **فخٌّ مكتوبٌ في الاختبارات القائمة ولم
أقرأه قبل أن أكتب.**

### وثلاثةُ بنودٍ مستقلّةٍ وُلدت منه — **تُقرأ بنوداً لا نواقص**

1. **لسانٌ ثانٍ للمطالبات المؤكَّدة، أو مرشِّحُ حالٍ في اللسان** (قرارُ المالك):
   «شحنات بانتظار التأكيد» **اسمُه يصف ما يعرضه، فتوسيعُه يجعله يكذب باسمه».
   **والبحثُ العامُّ يجد ما لا تعرضه الألسنة — صفةٌ مقصودةٌ لا عطب.**
2. **حراسةُ القوائم الأربعِ بصلاحياتٍ مسمّاة**: المصفوفةُ مبنيّةٌ منذ §41
   **ولا واحدةٌ منها تحرس `/admin/users` ولا أخواتِها** — يقرؤها `support`
   كما يقرؤها `admin`.
3. **عمودُ اختيارٍ في `Table.tsx`** — يستعمله ٢٢ جدولاً، **وسطحٌ مشتركٌ لا
   يُلمس لأجل فعلٍ واحد**.
...
1. **لم تُفتح شاشةٌ في متصفّح**: لا Playwright ولا Edge في التوزيعة. **فحقلُ
   ...
   لا نقرٌ على شاشة**.
2. **ودرجُ `Picker` في الرأس لم يُقَس بصرياً**: هو `absolute` داخل رأسٍ
   `sticky` بـ`z-30`، **وحاويةُ قصٍّ لم أرَها لا تعني أنها ليست هناك** —
   ودرسُ شريط التنقّل في هذا الملفّ بعينه.
3. **ولم يُقَس شيءٌ على الإنتاج.**
<!--/جديد-->

<!--جديد-->
## تحذيرُ `autoMemoryReclaim` — **قِيس أخيراً بإقلاعٍ بارد** (٢٠٢٦-٠٩-٠٤)

**الخانةُ التي أُعلنت «لم تُقس» صارت مقيسة، والجوابُ: التحذيرُ اختفى.**

    wsl --shutdown           →  vmmemWSL: 0 · صفرُ توزيعةٍ تعمل
    إقلاعٌ باردٌ ملتقَطٌ كاملاً:  stdout = "COLD_BOOT" (10 bytes)
                                stderr = 0 bytes
    وبرهانُ برودته:            /proc/uptime = 23.90 ثانية

**ولا `wsl: Unknown key 'wsl2.autoMemoryReclaim'` في المخرج** — والمفتاحُ في
`[experimental]` يُقرأ. **وسطورُ النواة كما كانت**: `Free page reporting
enabled` · `hv_balloon: Cold memory discard hint enabled with order 9`.

### وثلاثةُ شروطٍ لزمت لبلوغ القياس — **يُكتبن لأن الجولة القادمة ستحتاجهنّ**

1. **`wsl --shutdown` وحدَه لا يُسقط الآلة إن كان Docker Desktop يعمل**:
   `docker-desktop` توزيعةٌ تعيد إحياءها فوراً. **فيُوقَف Desktop أوّلاً.**
2. **وعمرُ الآلة هو برهانُ البرودة لا شيءَ سواه**: `/proc/uptime` داخل أيِّ
   توزيعةٍ **هو عمرُ الآلة الافتراضية** (النواةُ مشتركة). **وقياسٌ على آلةٍ
   قائمةٍ يعطي صمتاً لا يفرّق بين «أُصلح» و«فاتت اللحظة».**
3. **ويُلتقط المخرجُ إلى ملفّين منفصلين** (`> out 2> err`) — دمجُهما في
   PowerShell يلفّ أسطرَ الخطأ في كائناتٍ ويخلط القراءة.

---

## جولةُ المتصفّح — **على Chrome، وثلاثةُ عطوبٍ لم يمسكها حارس** (٢٠٢٦-٠٩-٠٤)

**ويُذكر أنها Chrome لا Edge** كي لا تُقارَن بقياسٍ سابقٍ كأنهما واحد.

**ما تحقّق على الشاشة**: حقلُ البحث مرسومٌ في الرأس · **درجُه يطفو فوق رأسٍ
`sticky` بلا حاويةِ قصّ** (وهو التحفّظُ الذي كتبتُه ولم يقع) · والأصنافُ
الأربعةُ كلُّها تجد: حساب · كبتن · رحلة · **ومطالبةٌ بمرجعها** (`REF-EYE`
وُجدت بالمرجع لا بالاسم) · والقفزةُ تفتح الدرجَ ويُمحى `?open=` بعده.

**وثلاثةُ عطوبٍ من عملي أمسكها المتصفّحُ وحدَه** — تفصيلُها في `SPEC.md`
§٤٧٫١٦: حالُ الرحلة خاماً بالإنجليزية · والقفزةُ إلى كبتنٍ معتمَدٍ تقع على
لسانٍ يخفيه · ومعرّفُ الرحلة ٣٦ خانةً في درج.

**وعدُّ الضغطات من الشيفرة كان أقلَّ بواحدةٍ في العمودين** — نسي أن الحقلَ
يُنقر قبل أن يُكتب فيه، ونسي لسانَ «بانتظار الاعتماد». **والمقيسُ من الشاشة
٤ ← ٢ لكبتنٍ معتمَد، و٣ ← ٢ لمنتظِر.**

### ⚠ وتكاملُ Docker مع Ubuntu سقط في هذه الجولة — **ولم يُصلَح**

**مقيسٌ لا مظنون**: `/var/run/docker.sock` غائب · `cli-tools` **فارغٌ تماماً**
· و`/usr/bin/docker` **وصلةٌ مكسورة**. **والأخطرُ أثرُه على المركَّبات**:

    docker run -v /home/loly3/prj/TAXO:/probe alpine ls /probe
    → admin-panel          (مدخلٌ واحدٌ من ٢١)

**فحاويتا اللوحة والكبتن تقلعان بـ`/app` فارغ** وتدوران في حلقة. و`backend`
و`customer-app` نجتا لأن مركَّبيهما هُيّئا قبل السقوط.

**والعلاجُ تفعيلُ تكامل WSL لـ`Ubuntu-24.04` من إعدادات Docker Desktop** —
**ولا مفتاحَ له في `docker desktop` CLI**، **وكتابتُه في `settings-store.json`
رفضها مصنِّفُ الأذونات**. **فهو بيد المالك.**

**واللوحةُ قِيست من خارج Docker**: `npx vite` من `~/prj/TAXO/admin-panel`
بـ`VITE_API_BASE_URL=http://127.0.0.1:8001` — **وحارسُ «حزمةٌ بلا هدف» أوقفني
قبلها وهو يعمل**.
<!--/جديد-->

<!--جديد-->
## تكاملُ WSL عاد، وعطبُ المحفظة أُصلح — **جولةُ ٢٠٢٦-٠٩-٠٤ الثانية**

### التكاملُ خضراءُ بسطرين لا بواحد

    /var/run/docker.sock                                     موجود ✅
    docker run -v /home/loly3/prj/TAXO:/p alpine ls /p | wc -l   →  34
    ls ~/prj/TAXO | wc -l                                        →  34
    admin-panel: 18 = 18   ·   cloudflared: الملفّاتُ الثلاثة

**وأُعيد كلُّ شيءٍ من داخل لينكس**: ١٣ حاوية · النفقُ بـ`docker start` ·
٤ اتّصالاتٍ مسجَّلة · والمضيفون `dev-app`/`dev-driver`/`dev-admin` **٢٠٠**،
و`dev-api` **٤٠٤ على `/`** مطابقاً أصلَه و**٢٠٠ على `/health`**.

### ⚠ وأوّلُ قياسٍ بصريٍّ أعطى صفراً — **والسببُ لم يكن عطباً**

**الحاويةُ تخدم `dist` لا المصدر**: المبنيُّ من **٢٠٢٦-٠٩-٠٣ ١١:٠٣** والمصدرُ
من **٠٩-٠٤ ٠٤:٤٧** — **حزمةٌ أقدمُ من مصدرها بسبعَ عشرةَ ساعة**. **وهو الشكلُ
العاشر**، **ولولا `check:served` لَقُرئ العطبُ في الشيفرة**. وبعد البناء
تطابقت الأعمدةُ الثلاثة.

### وثلاثةُ عطوبٍ كشفها القياسُ البصريّ — **كلُّها في شيفرةٍ خضراء**

1. **«رحلة · `completed`»** — إنجليزيةٌ خامٌ على شاشةٍ عربية (§٤٧٫١٦).
2. **والقفزةُ إلى كبتنٍ معتمَدٍ تقع على لسانٍ يخفيه** — «افتح» في العنوان
   و«لا نتائج لبحثك» في الشاشة.
3. **وأزرارُ «تأكيد/رفض» على شحنةٍ مؤكَّدة** — **زرٌّ يعمل ثمّ يرتدّ بـ409**
   (§٤٨٫٢)، **كشفه المرشِّحُ الذي أُضيف لغيرِ هذا**.

### وعطبُ `wallet_owner_undecided` أُصلح — **مقيسٌ بالنقض وعلى الشاشة**

**والإضافةُ اختياريةٌ لا تبديلُ سلوك** (§52): نداءٌ بلا `wallet` يسلك ما كان
يسلكه حرفاً. وسبعةُ اختباراتٍ **أوّلُها هو الشرط**: أن غيابَ الإعلان يبقى ٤٠٩.

    درجُ «عمر الراكب» (الدوران)  →  9.223 د.أ    · صفرُ ٤٠٩
    درجُ «زيد السائق» — الدفتر    →  23.655 د.أ ومعها قيودُها

### وجردُ الحرّاس خارج البابِ الواحد — **صفرُ جديد**

    مُصرَّحٌ في package.json الأربعة   53
    يشغّلها scripts/guards.sh          21
    خارجَه                             3  — check:dist · check:served · check:target

**والثلاثةُ مسجَّلةٌ سلفاً بعللها** في `HANDOFF.md`: الأوّلان أحمران بلا بناء،
والثالثُ ٤٥ ثانيةً على حاوياتٍ قد تكون مطفأة. **و`tools/check-apk.mjs` ليس
يتيماً**: يُستدعى يدوياً برابطٍ في جولة الحزم (`node tools/check-apk.mjs
https://…`). **فلا حارسَ منسيّاً** — والسؤالُ الذي في التسليم مُجاب.

## أين تُبنى حزمُ أندرويد — **قرارٌ مُقفَل، لا يُطرح ثانيةً** (قرارُ المالك ٢٠٢٦-٠٩-٠٤)

**تُبنى في استنساخٍ مؤقّتٍ على ويندوز** — اليومَ `C:\Users\loly3\taxo-apk-build`
— يُنشأ من `~/prj/TAXO` ويُحذف بعده. **ولا يُبنى من `D:\prj\TAXO` ولا يُركَّب
SDK في التوزيعة.**

**ونصُّ المالك بعلّته:**

> «أبقِ الاستنساخ المؤقّت على ويندوز كما فعلت. العلّة أنه يحقّق ما أردته من
> الوجهين بلا كلفتهما: لا يكتب في `D:` فلا شجرتان تتباعدان، ولا يستهلك قرص
> التوزيعة، ويستعمل SDK وJDK القائمين، وadb يعمل من ويندوز بلا `usbipd`.
> وكلفته نسخة و`npm ci` تُدفع عند كل بناء حزمة، وهي أرخص من الخطرين.»

**والوجهان المرفوضان بكلفتهما المقيسة** (٢٠٢٦-٠٩-٠٤):

| | الكلفة | ما يُسقطه |
|---|---|---|
| **جلبُ الشيفرة إلى `D:`** | دقائق | **شجرتان تُبنيان وتتباعدان** — وكتابةٌ في `D:` مُنعت صراحةً |
| **SDK وJDK في التوزيعة** | ٢–٤ جيجا + منصّة 35 + تراخيص | **الهاتفُ بالـUSB يسقط**: `/dev/bus/usb` غيرُ موجود، فيلزم `usbipd-win` فوقه |

**وخطواتُه الخمسُ مقيسةٌ بتشغيلٍ حقيقيّ:**

    git clone //wsl.localhost/Ubuntu-24.04/home/loly3/prj/TAXO  <hedef>
    نسخُ ما لا يدخل git: admin-panel/android/local.properties · app/google-services.json
    npm ci داخل التطبيق
    TAXO_CHANNEL=<قناة> node tools/build-channel.mjs <app> && npx cap sync android
    JAVA_HOME=<jbr 21> ANDROID_KEYSTORE_* … sh gradlew assemble<Flavor>Release

> **و`git clone` عبر حدِّ WSL يحتاج سطراً واحداً أوّلَ مرّة**:
> `git config --global --add safe.directory //wsl.localhost/…/TAXO/.git` —
> وبدونه يرفض git المستودعَ بـ«dubious ownership» **ولا يقول أن هذا هو
> العلاج** إلا في نصِّ الخطأ.
>
> **و`.git` يلزم لا يُستثنى**: `taxoVersionCode` في `app/build.gradle`
> **يشتقّ رقمَ النسخة من تاريخ git**، ويرمي `GradleException` في استنساخٍ بلا
> تاريخ. **فنسخُ الملفّات وحدَها لا يكفي.**

## ⚠ لوحةُ الإنتاج أقدمُ من الشيفرة — **و`ly.tajora.admin` لا تصلح للقياس اليوم** (٢٠٢٦-٠٩-٠٤)

**قِيس بقراءة الحزمة المخدومة نفسِها** على `https://admin.tajora.ly`
(`assets/index-fy3fA2pB.js`، ٢٢٧٬٦٥٠ بايت) — **لا من git ولا من ظنّ**:

| النمط | إنتاج `admin.tajora.ly` | تطوير `dev-admin.tajora.ly` |
|---|---|---|
| `/m/payments` | **✗ غائب** | ✓ |
| `cliq_claim_declared` | **✗ غائب** | ✓ |
| `pushNotificationActionPerformed` | **✗ غائب** | ✓ |
| «إذن الإشعارات مرفوض» | **✗ غائب** | ✓ (في `MobilePayments-Db1NKZop.js`) |

**وشاهدٌ موجبٌ يمنع أن يُقرأ هذا عطبَ بحث**: «لوحة تحكم العمليات» و`/drivers`
و`/finance` **موجودةٌ** في حزمة الإنتاج نفسِها.

**فالأثرُ العمليّ**: `ly.tajora.admin` **تُثبَّت وتفتح ولا تصلح لقياس
الإشعارات** — الغلافُ يحمّل اللوحةَ من الشبكة (`server.url`)، **فما يظهر هو
ما يخدمه الخادمُ لا ما في الحزمة**. **وأداةُ القياس اليومَ `ly.tajora.admin.test`**
(`dev-admin.tajora.ly`) وحدَها، **حتى تُنشر اللوحةُ على الإنتاج**.

> **وهذا هو الشكلُ العاشر بحرفه**: «خضرةُ بناءٍ لا تقول شيئاً عمّا يشغّله
> المستخدم». **والحزمةُ خضراءُ في كلِّ عمود** — بُنيت ووُقّعت وثُبّتت وفتحت —
> **والشاشةُ التي تحمّلها من عمرٍ آخر.**

## وحارسٌ وُلد من ذلك — `check:hosts` (٢٠٢٦-٠٩-٠٤)

**`check-apk.mjs` كان يقارن `server.url` بالجدول ولا يسأل هل النطاقُ حيّ.**
فبقي `panel.tajora.ly` و`dev-panel.tajora.ly` مُصرَّحَين لغلاف المشرف **وهما
بلا سجلِّ DNS**، **والزوجُ متّسقٌ فالحارسُ أخضر**.

**و`tools/check-hosts.mjs` يسأل**، ويُنادى من موضعين: **`build-channel.mjs`
قبل البناء** (فالوقوفُ أرخصُ هناك) و**`check-apk.mjs` قبل البيان** — وقبله
بقصد، لأن غلافَ المشرف `private` **فلا يدخل البيانَ أصلاً**.

**ولا يُقاس بشاهدٍ خارجيّ**: تُسأل نطاقاتُ الجدول كلُّها، **وسقوطُ جميعِها
يعني سائلاً بلا شبكةٍ فيقال «لم يُقس»** — وسقوطُ بعضها مع نجاح بعضٍ هو الخبر.
**فلا يصيح على من يبني بلا إنترنت.**

**ولا يقيس أن الخادمَ يجيب**، ويقول ذلك: نطاقٌ يحلّ ويعطي ٥٠٢ يمرّ من هنا،
**وذاك عمودُ `check:served`** — وتوسيعُ الدعوى فوق النطاق هو الشكلُ الخامسَ عشر.

**ومقيسٌ في الاتجاهين ٢٠٢٦-٠٩-٠٤:**

    الحالُ الصحيحة   →  ✓ 8 نطاقاتٍ مُصرَّحةٍ لكلٍّ سجلُّ DNS
    بالنقض (أُعيد panel.tajora.ly)  →  البناءُ يقف **قبل vite**، ويسمّي
                                        النطاقَ وموضعَه: «public/admin-panel الغلاف»

## عطبُ `SkinMapPreview` — **قِيس فسقط: أثرُ وضعِ تطويرٍ لا عطبٌ في شيفرتنا** (٢٠٢٦-٠٩-٠٤)

**البندُ الجانبيُّ المفتوح منذ ٢٠٢٦-٠٩-٠٢** — خطآن في سجلّ `/vehicle-skins`:

    [map-rtl] تعذّر تحميل ملحق العربية: AbortError: Actor removed
    TypeError: Cannot read properties of undefined (reading 'applyProjectionUpdate')

**والفرضيّةُ كانت مكتوبةً وغيرَ مقيسة**: «السببُ المرجَّح `StrictMode` … ويُقاس
بإطفاء `StrictMode` أو ببناء إنتاج، **ولم يُقَس**».

**وقِيس اليومَ على بناء إنتاج** — وهو ما تشغّله الحاويةُ فعلاً
(`docker inspect` → `npx vite preview`، لا `vite dev`)، **وبعد أن خضّر
`check:served` الأعمدةَ الثلاثة** (`index-B-EfLjiG.js` في الثلاثة):

    الشاشة                /vehicle-skins  · Chrome
    الخريطةُ ركّبت فعلاً   mapboxgl-canvas = 1 · mapboxgl-map = 1 · **علامات = 5**
    أخطاءُ الصفحة          **0**  — لا map-rtl، ولا applyProjectionUpdate، ولا AbortError
    وما ظهر في السجلّ      18 رسالةً **كلُّها من إضافةِ متصفّح** (MetaMask)، لا من اللوحة

**فالبندُ يُغلق: ليس من عملنا، ولا يصل مستخدماً.** `StrictMode` في وضع التطوير
يركّب المكوّنَ ويفكّكه ويعيده، **فيُنادى `map.remove()` قبل أن يكتمل تحميلُ
ملحق العربية** — وذاك نصُّ `AbortError: Actor removed` بعينه. **وفي بناء
الإنتاج لا يقع التركيبُ المزدوج فلا يقع الخطأ.**

> **والقياسُ نفسُه كان يمكن أن يكذب**: صفرُ خطأٍ على شاشةٍ **لم تركّب خريطةً**
> صفرٌ لا يقول شيئاً. **ولذلك عُدَّت العلاماتُ قبل أن يُقرأ السجلّ** — خمسٌ،
> فالمكوّنُ عمل. **وحارسٌ يُشغَّل حيث لا يملك ما يقيسه يعلن «لم يُقس»**، وهذه
> قِيست.

**وما بقي وليس عطباً**: الخطآن **يظهران في `vite dev`** — كلفةُ راحةٍ لمن
يطوّر، لا شيءٌ يُشحن. **ولا يُصلَح بإطفاء `StrictMode`**: إطفاؤه يخفي أخطاءَ
تركيبٍ حقيقيةً بُني ليكشفها.

## البندان المستقلّان — **مبنيّان ومقيسان على الشاشة** (٢٠٢٦-٠٩-٠٤)

**والقياسُ كلُّه على الحاوية بعد بناءٍ خضّر `check:served`** — حزمة
`index-DzDBFA9h.js`، والأعمدةُ الثلاثةُ متطابقة.

### ١) حراسةُ القوائم الأربع (§٤٧٫١٨) — `24 passed`

**والنقضُ بالحذف**: أُعيد بابان إلى `StaffUser` **فسقط اختباران** بـ
`assert {200, 403} == {403}`.

### ٢) عمودُ الاختيار (§50) — **ومقيسٌ بالنقر لا بالقراءة**

    الحزمة المخدومة              index-DzDBFA9h.js  (الأعمدة الثلاثة ✓)
    مربّعات /riders               19 = رأسٌ واحد + 18 صفّاً
    وسمُ مربّع الرأس              «اختَرِ المعروضَ في هذه الصفحة»
    ضغطةُ صفٍّ واحد               «اختِرتَ 1 من 18 المعروضة» · الرأس mixed
    «اختَرِ الكلّ»                «اختِرتَ 18 من 18 المعروضة» · الرأس true
    الإلغاء                       الشريطُ يختفي · الرأس false · المربّعات باقية 18

#### ⚠ وعطبٌ أمسكه النقرُ ولم يمسكه مُترجِمٌ ولا حارس

**ثلاثُ ضغطاتٍ في دفعةِ React واحدةٍ أعطت `اختِرتَ 2 من 18`** — واحدةٌ ضاعت.
**والعلّةُ أن الحسابَ كان يقرأ `selected` من الخاصّيّة**، وهي في مُعالِج
الضغطة الثانية **قيمةُ ما قبل الأولى**؛ فتبني الثانيةُ مجموعةً من قيمةٍ
بائدةٍ وتمحو ما أضافته الأولى.

**ولا يقع بإصبعِ إنسانٍ عاديّ** — بينهما إعادةُ رسم — **لكنه يقع بضغطةٍ
مزدوجةٍ أو نقرٍ سريعٍ على لمس، ولا يصيح شيء**: يبقى صفٌّ غيرُ مختارٍ وقد ظنّ
المشرفُ أنه اختاره، **ثمّ يُرسَل إلى أقلَّ ممّا قصد**.

**والعلاجُ `Dispatch<SetStateAction<…>>`** — يُحسب من السابق لا من الخاصّيّة.
**والقياسُ بعده على الحزمة الجديدة: `اختِرتَ 3 من 18`.**

> **ولولا النقرُ لَما ظهر**: `tsc` أخضر، والحرّاسُ الواحدُ والعشرون خضر،
> **والشاشةُ ترسم كلَّ شيءٍ صحيحاً** — **والعددُ وحدَه كان يكذب**. وهو
> الشكلُ العاشرُ في ثوبٍ جديد.

### ٣) الإشعارُ الجماعيّ — **مقيسٌ إلى القاعدة لا إلى الشاشة**

    الحوار        «رسالةٌ إلى 1 حساباً»
    بعد الإرسال   «أُرسلت إلى 1 صندوقَ وارد.» · الشريطُ اختفى · صفرُ خطأ
    وفي القاعدة   user_notifications  → admin_message | قياسُ الإشعار الجماعيّ
                  admin_audit_logs    → user_message  | create | العنوانُ كما كُتب

**فالمسارُ كاملٌ لا واجهةٌ تقول «تمّ»**: صندوقُ الوارد **وصفُّ التدقيق** معاً.

### ٤) وأزرارُ «الملفّ» الثلاثة

| الشاشة | المقيس |
|---|---|
| **طلباتُ الصرف** | عمودُ «الكبتن» أوّلاً · «هناء السائقة» ومعها الزرّ · **والنقرُ يفتح `/drivers?q=%2B962790000012` والدرجُ عليها** (الحساب · المحفظة · الوثائق · الرحلات) |
| **المدفوعات** | رأسٌ «الرحلة \| الطرفان \| القناة \| المبلغ \| الحال \| التاريخ» · **64 زرَّ ملفّ** · أوّلُ صفّ «ليلى الراكبة» ومعها زرُّها |
| **النزاعات** | **العمودُ مرسوم** («المبلغ \| الطرفان \| السبب \| مرجع الحوالة \| فُتح») — **والخليّةُ لم تُقَس: صفرُ نزاعٍ مفتوحٍ في قاعدة التطوير**. والبيتُ هو بيتُ المدفوعات نفسُه (`PaymentParties`) وقد قِيس هناك |

### ⚠ وما لم يُقَس في هذه الجولة — ولا يُقرأ سلامة

1. **خليّةُ «الطرفان» في شاشة النزاعات** — لا صفَّ يعرضها اليوم.
2. **الإرسالُ الجماعيُّ إلى أكثرَ من حساب** — قِيس بواحد، **والتسلسلُ وعدّادُه
   والجملةُ ذاتُ العددين لم تُقَس على إخفاقٍ جزئيّ**.
3. **ولا هاتفَ ولا إنتاج** — القياسُ كلُّه على لوحة التطوير في Chrome.
<!--/جديد-->

<!--جديد-->
## الرفعُ إلى الإنتاج — **٤٧ إيداعاً و٨ ترحيلات، والبوّاباتُ الستُّ خضراء** (٢٠٢٦-٠٩-٠٥)

**أوّلُ رفعٍ منذ ٢٠٢٦-٠٨-٣٠**، ومن الشجرة اللينكسيّة `~/prj/TAXO`.

| البوّابة | المقيس |
|---|---|
| **١ الإعلان** | `c6caf79 ← d4f8e10` · **٤٧ التزاماً** · **٨ ترحيلات** (القاعدة `0063` والشجرة `0071`) · مالٌ **يمسّ** ومأذونٌ صراحةً · طريقُ الرجوع معلَن |
| **٢ الدفعُ وCI** | ٤٣ إيداعاً إلى `origin/master` ثمّ إيداعان · **CI أخضرُ في ثلاثة تشغيلات** — والرابع أحمرُ بعطبِ شبكةٍ لا كود |
| **٣ النسخة** | `/mnt/d/taxo-backups/20260905T002733Z` · و**نسخةُ الوثائق الحقيقيةُ منفصلة** (أدناه) |
| **٤ تحقّقُ النسخة** | أربعةُ أرشيفاتٍ تُفتح ويُقاس محتواها — **والوثائقُ كانت تكذب** (أدناه) |
| **٥ الرفع** | الخادمُ سحب `d4f8e10` من GitHub · `moved=0 patch=3` · **الواجهاتُ الثلاثُ بُنيت على الخادم** |
| **٦ التحقّق** | إيداعُ الخادم = إيداعُ CI · الترحيلةُ `0071` · ١١ حاويةً تعمل · الصحّةُ `production` · **صفرُ خطأٍ في ثلاث دقائق** |

### الأنماطُ الأربعةُ صارت موجودةً على `admin.tajora.ly` — **بشهادة العملية الحيّة**

**والقياسُ من الحزمة التي يخدمها الخادمُ الآن لا من `grep` على قرص**: تُجلب
`index.html` عبر HTTPS، ومنها الحزمةُ الرئيسة، ومنها **كلُّ قطعةٍ كسولةٍ
تذكرها** — فالنمطُ قد يسكن في قطعةٍ لا في الأصل.

| النمط | قبل | بعد |
|---|---|---|
| `/m/payments` | ✗ غائب | ✓ في `main.js` |
| `cliq_claim_declared` | ✗ غائب | ✓ في `main.js` |
| `pushNotificationActionPerformed` | ✗ غائب | ✓ في `main.js` |
| «إذن الإشعارات مرفوض» | ✗ غائب | ✓ في `chunk-MobilePayments-BEW2qzAb.js` |

    الحزمةُ المخدومة   index-fy3fA2pB.js (227,650 ب · 22 قطعة)
                    →  index-gtGmXo_h.js (244,612 ب · 26 قطعة)
    والصفوفُ بلا فرقٍ حرفاً: diff صامت، وبصمتا الصفوف واحدة e72b87d7…

**وشاهدٌ موجبٌ يمنع أن تُقرأ الخضرةُ خطأً في المِسبار**: المِسبارُ نفسُه أعطى
الأربعةَ على `dev-admin.tajora.ly` **قبل الرفع وبعده**، وأعطى صفراً على
الإنتاج قبله. **فالمقياسُ يعرف كيف يقول «موجود»، وقال «غائب» عن علم.**

### والأعمدةُ الأربعةُ للثلاثة — خضراءُ على الإنتاج

| التطبيق | المبنيُّ على الخادم | يشير إليه `index.html` | عبر النفق | **يُحمَّل في المتصفّح** |
|---|---|---|---|---|
| `admin-panel` | `index-gtGmXo_h.js` `02:35` | ✓ | ✓ | ✓ — ٣٢ عقدة، والنموذجُ مرسوم |
| `customer-app` | `index-CfEsbBEt.js` `02:30` | ✓ | ✓ | ✓ — «اطلب تاكسي في ثوانٍ» · زرُّ «دخول» |
| `driver-app` | `index-Dev39IEd.js` `02:33` | ✓ | ✓ | ✓ — «تطبيق الكبتن» · حقلان وثلاثةُ أزرار |

**وأخطاءُ الصفحات صفر** — والرسائلُ الوحيدةُ في السجلّ من إضافةِ متصفّح
(MetaMask)، لا من التطبيقات. **ولم تُفتح لوحةُ الإنتاج بحسابٍ**: بابُ الدخول
محدودُ المحاولات، وطرقُه ينفق من ميزانية المالك لا من ميزانيتنا.

### والحزمُ على `taxo.tajora.ly` — **مقيسةً بتنزيلها لا بوجودها**

| | قبل | بعد |
|---|---|---|
| الوسم · الإيداع | `v0.2.2` · `c6caf79` | **`v0.2.3` · `e358474`** |
| `versionCode` | `418` | **`464`** |
| `taxo-rider.apk` | 7,637,040 ب | **7,638,284 ب** · `sha256=e874a5a3a9e08aa8…` |
| `taxo-driver.apk` | 7,860,718 ب | **7,863,282 ب** · `sha256=3e0bff0e41fd08ad…` |

**والبصمتان محسوبتان من البايتات المنزَّلة من الصفحة العامّة**، وتطابقان
البيانَ في الاثنتين. **وبناؤها في CI لا على جهاز**: وسمُ `v*` يشعل وظيفةَ
«الحزمُ وإصدارُها» فتبني الأربعَ وتوقّعها وتنشئ الإصدار، **والخادمُ يسحب
أثرَه** — فلا حزمةَ تُرسل من قرص أحد.

> **⚠ والبيانُ يقول `e358474` والخادمُ عند `d4f8e10`** — إيداعٌ واحدٌ بينهما،
> **ومحتواه `scripts/deploy.sh` وحدَه**، ولا حزمةَ تحمله. **فالبيانُ صادقٌ
> عمّا بُنيت منه الحزم**، ولم يُنشأ وسمٌ ثانٍ لأنه كان سيعطي حزماً متطابقةَ
> المحتوى برقمِ نسخةٍ جديد — **ضجيجٌ يُقرأ إصداراً**.

## ⚠ نسخةُ الوثائق كانت خضراءَ وهي عمياء — **٣٧ كبتناً بلا نسخة** (قِيس ٢٠٢٦-٠٩-٠٥)

**المقيس، ثلاثةُ أرقامٍ لا وصف:**

    driver_documents في قاعدة الإنتاج      37 صفّاً
    backend/var/documents على قرص المضيف   0 ملفّ
    documents.tar.gz في كلِّ نسخةٍ سابقة    124 بايت — المجلَّدُ وحدَه

**والعلّةُ أن الوثائق تسكن مجلَّداً مسمّى** (`taxo_documents` مثبَّتاً على
`/app/var/documents`) **لا مسارَ مضيفٍ مربوطاً** — والبابُ كان يضغط مسارَ
المضيف. **وقُرئ من `docker inspect` لا من ملفّ compose.**

**وأخطرُ ما فيه أن الفحصَ يُخضِّره**: `tar_holds … "documents"` **يطابق اسمَ
المدخل**. **وهو بحرفه الدرسُ المكتوبُ فوقه بأسطرٍ لأجل `.env`** — «الاسمُ
يقول إن الشيءَ مذكور، والمحتوى يقول إنه هناك». **كُتب للأسرار ولم يُطبَّق على
الوثائق**، فعاش سبعةَ عشرَ يوماً يُعلن خضرةً وهو لا يقيس شيئاً.

**والفرقُ بينه وبين نسخةٍ غائبة هو الفرقُ نفسُه المكتوب في `CLAUDE.md`**:
الغائبةُ تُعرف يومَ تُطلب، **وهذه تُعلَن محقَّقة**. ولو سقط القرصُ لضاعت
**وثائقُ هويةِ ٣٧ كبتناً حقيقيّاً** ولا شيءَ يعيدها.

### والنسخةُ أُخذت، ومحقَّقةٌ بفتحها

    /mnt/d/taxo-backups/documents-20260905T000000Z/documents-volume.tar.gz
    gzip -t   سليم        الحجم 16,100,316 بايت
    الملفّات  768  (738 webp · 30 jpg)   في 373 مجلَّداً — مجلَّدٌ لكلِّ كبتن
    واستُخرج ملفٌّ حقيقيّ: 27,204 بايت · RIFF (little-endian) data, Web/P image

### والبابُ أُصلح، **ومقيسٌ في أربعة اتجاهات**

**المصدرُ صار المجلَّدَ المسمّى** — `docker compose run --rm --no-deps -T
--entrypoint tar backend` فيأخذ المجلَّدَ من تعريف الخدمة نفسِه، **بلا اسمِ
مجلَّدٍ مكتوبٍ بيد ولا صورةٍ تُسحب**. **والفحصُ صار يعدّ الملفّات ويقابلها
بصفوف الجدول**:

| الحال | الجواب |
|---|---|
| ٧٦٨ ملفّاً · ٣٧ صفّاً | **يمرّ** |
| **بالنقض**: الأرشيفُ القديمُ الفارغ · ٣٧ صفّاً | **يسقط** — وهو المطلوب |
| صفرٌ · صفر (تركيبٌ جديد) | **يمرّ** — وحارسٌ يقرأ بلوغَ الحالِ الصحيحةِ فشلاً أسوأُ من غيابه |
| ٧٦٨ ملفّاً · **رأسُ الجدول غيرُ مقروء** | **يسقط** — «لم يُقس» لا «سليم» |

## ⚠ بابُ الرفع كان على حافّة حدِّ `ufw` — **والرقمُ مشتقٌّ من حدٍّ صحيح** (قِيس ٢٠٢٦-٠٩-٠٥)

**سقطت الرفعةُ مرّتين عند الخطوة نفسِها**، والنصُّ واحد:

    ssh: connect to host 169.58.207.123 port 22: Connection refused

**والسببُ من سجلِّ الخادم**، والسطرُ يسمّي عنوانَ من يرفع:

    01:58:58 opened · 01:59:02 accepted · 01:59:09 accepted · 01:59:11 accepted
    01:59:17  kernel: [UFW LIMIT BLOCK] ... SRC=<عنوانُ من يرفع>

**والتعليقُ في `deploy.sh` كان يحمل الحدَّ الصحيحَ ثمّ يستنتج منه رقماً
خاطئاً**: «`ufw` … ستُّ محاولاتٍ في ثلاثين ثانية» ثمّ «وستُّ ثوانٍ بين اتصالين
تُبقي المعدَّلَ تحت الحدِّ **مهما طال السكربت**». **وفجوةُ ستٍّ تضع وصلاتٍ عند
٠ و٦ و١٢ و١٨ و٢٤ و٣٠ — ستٌّ في النافذة بالضبط**، و`ufw` يحجب **عند** الستِّ.

**ورقمٌ خاطئٌ مشتقٌّ من حدٍّ صحيحٍ أخطرُ من جهلٍ بالحدّ**: يُقرأ محسوباً فلا
يُراجَع. **وقد قُرئ هذا الحجبُ «انقطاعَ شبكة» في تقريرٍ سابق.**

**ولم يُمسّ `ufw` بحرف** (المطلقةُ الثالثة). **صارت الفجوةُ اثنتَي عشرةَ
ثانية**: ثلاثُ وصلاتٍ في النافذة لا ستّاً — **نصفُ الحدِّ هامشٌ، والحافّةُ
ليست هامشاً**. و`TAXO_SSH_GAP` يُصرَّح لمن يعرف حدَّ خادمِه.

### وإسنادٌ يسقط تحت `set -e` يقتل معالِجَه نفسَه

`PULL="$(_ssh …)"` بلا `|| true` — **فسقط الرفعُ بـ`rc=255` بلا سطرٍ واحدٍ يقول
ماذا وقع**، و`case` تحتَه يحمل ثلاثةَ أجوبةٍ مكتوبةً **لم تُقرأ قطّ**، منها
«يُقاس الخادمُ قبل أيِّ إعادة» — **وهي بعينها القاعدةُ التي لزمت**. **وبابٌ
يسقط بلا أثرٍ ليس باباً**، درسٌ مكتوبٌ في هذا الملفِّ ولم يكن مطبَّقاً فيه.

**وما وقع على الإنتاج في المحاولتين: لا شيء.** الخادمُ بقي `c6caf79` مقيساً،
ولا مجلَّدَ إزاحةٍ جديد — **فالوقوفُ كان قبل أوّل فعلٍ يكتب**، والإعادةُ آمنةٌ
بقياسٍ لا بظنّ.

## الإرسالُ الجماعيُّ إلى أكثرَ من حساب — **قِيس، وكشف عطباً لغويّاً** (٢٠٢٦-٠٩-٠٥)

**البندُ كان مفتوحاً بنصّه**: «قِيس بواحد، والتسلسلُ وعدّادُه والجملةُ ذاتُ
العددين لم تُقَس». **وقِيس بثلاثة على لوحة التطوير في Chrome:**

    مربّعاتُ /riders   19 = رأسٌ + 18 صفّاً
    ثلاثُ ضغطات        «اختِرتَ 3 من 18 المعروضة» · الرأس mixed · 3 صفوفٍ true
    بعد الإرسال        الشريطُ اختفى · الرأس false · الصفوف باقية 18
    user_notifications  3 صفوف admin_message — بفارق 1.42 ث و1.51 ث ⇒ **تسلسلٌ لا دفعة**
    admin_audit_logs    3 صفوف create · user_message — **صفٌّ لكلِّ حساب**

**فالوعدُ المكتوبُ في الحوار — «يُحفظ في سجل التدقيق … لكلِّ حساب» — مقيسٌ
إلى القاعدة لا إلى الشاشة.**

### ⚠ وعطبٌ لغويٌّ لم يكن قياسُ الواحد ليكشفه

    رسالةٌ إلى 3 حساباً        ← تمييزُ ٣–١٠ جمعٌ مجرور لا مفردٌ منصوب
    أُرسلت إلى 3 صندوقَ وارد.  ← والمنصوبُ المفردُ صحيحٌ من ١١ إلى ٩٩ وحدَها

**وهو صنفُ «الصحيحِ بالصدفة»**: من قرأ الجملةَ بعددٍ بين ١١ و٩٩ رآها سليمة.
**وبواحدٍ تُخفي الجملةُ عيبَها أصلاً** — ولذلك عاش مع قياسٍ أعلن الخضرة.

**والعلاجُ بيتٌ واحدٌ لا إصلاحُ سطرين**: `counted()` في `lib/utils.ts` حيث
تسكن `digits()` — **ولا في `labels.ts`** لأن رأسَه يقول إنه لتراجم التعدادات
وحدها، **وهذه حسبةٌ لا اسم**. والأشكالُ الخمسةُ تُصرَّح مع الاسم فلا تُخمَّن
عند النداء.

**ومقيسٌ على الشاشة عند أربعة حدود** (حزمة `index-DhOYzMzc.js`، والأعمدةُ
الثلاثةُ متطابقة):

    1  →  «رسالةٌ إلى حسابٍ واحد»      2  →  «رسالةٌ إلى حسابَين»
    3  →  «رسالةٌ إلى 3 حسابات»        11 →  «رسالةٌ إلى 11 حساباً»
    وبعد الإرسال إلى ثلاثة: «أُرسلت إلى 3 صناديقِ وارد.»

### ⚠ وما بقي غيرَ مقيسٍ من هذا البند — ولا يُقرأ سلامة

1. **الإخفاقُ الجزئيُّ وجملتُه ذاتُ العددين** — **لم يقع إخفاق**، والبابُ
   الوحيدُ الذي يردّ (حسابُ موظّف) **لا يظهر في قائمة الركّاب أصلاً**.
2. **و`VerificationCampaign.tsx` فيه النمطُ نفسُه ولم يُحوَّل** — «ستشمل
   `{scope_size}` حساباً»، **والعددُ ملفوفٌ بـ`<b>`**: تحويلُه ينقل العريضَ من
   الرقم إلى العبارة كلِّها، **وذاك تغييرُ شكلٍ لم يُقَس**. **يُقيَّد ولا
   يُصلَح بالتخمين** — ونطاقُه الأخطر: حملةٌ تشمل ألفاً تقول «1000 حساباً».
3. **خليّةُ «الطرفان» في شاشة النزاعات ما زالت غيرَ مقيسة** — **صفرُ دفعةِ
   كليك معلَّقةٍ في قاعدة التطوير** (٥ معلَّقاتٍ كلُّها نقدٌ أو بطاقة)، والنزاعُ
   لا يُفتح إلا على كليك. **والبيتُ هو بيتُ المدفوعات نفسُه وقد قِيس هناك** —
   لكنّ ذلك **استنتاجٌ لا قياس**، ويُقال كذلك.

## والرفعةُ الثانية — **البوّابةُ المُصلَحة قِيست في مسارها الحقيقيّ** (٢٠٢٦-٠٩-٠٥)

**`d4f8e10 → 2c3daca`**، والبوّاباتُ الستُّ خضراء. **وسطرٌ واحدٌ هو الشاهد:**

    ✓ الوثائق — يُفتح ومحتواه صحيح (16,100,316 بايت)      ← وكان 124
    حجمُ النسخة: 16M                                        ← وكان 100K

**فالبوّابةُ صارت تقيس ما ظنَّت أنها تقيسه**، ولم تُصدَّق دعوى الإصلاح حتى
مرّت في الباب نفسِه لا في تجربةٍ جانبية — **ومسارُ نشرٍ جديدٌ لا يُصدَّق حتى
يمرّ مرّتين**، وقد مرّ.

**وأُعيد قياسُ الأنماط الأربعة بعدها** على الحزمة الجديدة
`index-JzWUZ2qv.js` (245,085 ب · 26 قطعة): **الأربعةُ موجودة**، و«إذن
الإشعارات مرفوض» في `chunk-MobilePayments-Bn1bhVch.js`. **وأشكالُ التمييز
الخمسةُ في حزمة الإنتاج**: «حسابٍ واحد» · «حسابَين» · «حسابات» · «صناديقِ
وارد» · «صندوقَ وارد». **والمتصفّحُ حمّل الحزمةَ نفسَها** — ٣٢ عقدةً، حقلان
وزرّ.
<!--/جديد-->

<!--جديد-->
## موقعُ `taxo.tajora.ly` — **بُني على تصميمٍ مُسلَّم، ووُصل بما كان مبنيّاً** (٢٠٢٦-٠٩-٠٥)

**والمواصفةُ في `SPEC.md` §52.**

### أوّلاً: ما وجدتُه مبنيّاً فلم يُبنَ ثانياً

**الصفحةُ القائمةُ كانت تحمل ثلاثةَ أشياءَ نُقلت ولم تُخترع من جديد:**

| المبنيُّ سلفاً | أين صار |
|---|---|
| `source: "apk"\|"play"\|"none"` في `landing/config.js` | **حقلٌ في `site_settings`** — يُبدَّل من اللوحة ويصل الزائرَ بلا نشر |
| قراءةُ `downloads/manifest.json` (الإصدار·البصمة·التاريخ·الحجم) | **كما هي** في `site/src/site.js` — ومجرى CI لم يُمسّ بحرف |
| `GET /public/landing` (العرضُ القائم) | **كما هو**، وقد نُزعت أرقامُه (أدناه) |

**و`none` أُسقطت**: لم تُستعمل قطّ، **و«لا زرَّ بلا رابط» تغطّي حالَها** —
رابطٌ فارغٌ يعطي شارةً معطَّلةً، وذاك أوضحُ من قسمٍ يختفي بلا أثر.

### وثلاثةٌ في القائم خالفت شرطَ المالك فأُصلحت

    «تاكسي في ثوانٍ — في الأردن وليبيا»   →  الأردنُ وحدَه (شرط ٢)
    بطاقةُ عرضٍ تعرض سعراً ومشطوباً       →  الاسمُ وحدَه بلا رقم (شرط ٣)
    رقمُ دعمٍ ليبيّ +218 في config.js      →  بريدُ الدعم من الإعدادات

### الخلفية — جدولٌ وبابان وترحيلة

**`site_settings` صفٌّ واحدٌ لا صفٌّ لكلِّ سوق**، ويمنعُ الثانيَ عمودُ
`singleton` (فريدٌ عليه `CHECK`). **ولمَ جدولٌ جديد**: جداولُ الإعدادات في هذا
المستودع مكتوبةٌ بأعمدةٍ مسمّاةٍ لكلِّ ميدان، **ولا مخزنَ مفتاح-قيمةٍ عامٌّ
فيها** — فشرطُ المواصفة «إن اتّسع» **قِيس فلم يتّسع**.

**و`GET /public/site` قائمةُ سماحٍ صريحة** — يبني القاموسَ حقلاً حقلاً ولا
يُسلسل النموذج. **والفرقُ ليس أسلوباً**: قائمةُ استبعادٍ تُنسى عند أوّل عمودٍ
يُضاف، **وقائمةُ سماحٍ لا تنشر ما لم يُذكر فيها**.

**ومقيسةٌ في الاتجاهين:**

    أعمدةُ الجدول        24
    في قائمة السماح      23
    ما خرج فعلاً          23
    خرج وهو خارج القائمة  لا شيء
    في القائمة ولم يخرج   لا شيء
    محجوبٌ عمداً          singleton — قفلُ الجدول لا محتواه

**والعمولةُ والمفاتيحُ تُقرآن من مصدرهما**: النسبةُ من `commission_settings`
عبر `settings_service` — **وهو المصدرُ نفسُه الذي يُجمَّد منه
`commission_percent_at_ride`** — والمفاتيحُ من مصدر `GET /config`. **قراءةٌ لا
نسخة.**

**و`PATCH /admin/site` يختم القيمةَ قبل وبعد** عبر `audit.apply_changes`،
**وما لم يتغيّر لا يُذكر**. و«لم يُرسَل» ليست «أُرسل فارغاً»: `exclude_unset`،
**فحفظُ قسمٍ لا يمحو قسماً لم يُفتح**.

**و`GET /public/policy` خلف مفتاحين لا واحد**: `policies_public` (مطفأٌ
بقرارِ المالك حتى يراجع النصوصَ الثمانِ) **و**`is_published` على الوثيقة
نفسِها. **ومفتاحٌ واحدٌ لمعنيين يفتح أحدَهما بالآخر من حيث لا يُرى** — النشرُ
في الجدول يخصّ التطبيقات وهذا يخصّ الويب.

**والاختبارات ١٣ خضراء**، ومنها قائمةُ السماح في الاتجاهين، وأن **البابَ
العامَّ لا يكتب صفّاً**، وأن رابطَ المتجر يبدأ بـ`play.google.com` أو يبقى
فارغاً.

### الصفحة — نُفِّذ التصميمُ آلياً لا يدوياً

**والنقلُ من المصدر لا بإعادة كتابة**: `TAXO Landing.dc.html` حُوِّل بمُحوِّلٍ
يفكّ `<x-dc>` و`<helmet>` و`sc-for` و`sc-if` و`{{ }}` و`style-hover`.
**وما يُعاد كتابتُه بيدٍ ينحرف عن التصميم**، والآليُّ لا ينحرف.

    ١٣ قسماً · ٦٣٤ عقدة · ٥١ قاعدةَ تحويم · صفرُ قالبٍ متبقٍّ
    والقوائمُ الخمسُ (٤١ عنصراً) **مبنيّةٌ في HTML وقتَ البناء**

**وذاك بعينه ما أغلق فجوةَ «بلا JavaScript»**: التصميمُ كان يرسمها في
المتصفّح، **فخمسُ قوائمَ تسقط بلا سكربت**. وبناؤها وقتَ البناء يجعلها تُقرأ
بلا سكربتٍ **بحكم بنائها لا بحيلة**.

### ⚠ وعطبٌ في التصميم كشفه Lighthouse ولم يكشفه قياسي

**شاشةُ الظهور الأول كانت تُزال بـJavaScript** — غطاءٌ أسودُ على الصفحة كلِّها
حتى يصل السكربتُ ويحذفه. **وأثرُه وجهان:**

1. **`LCP = 6.7` ث** — أكبرُ عنصرٍ مرسوم (`h1`) لا يُعدّ مرسوماً تحت غطاء.
2. **وبلا JavaScript لا يزول أبداً** — صفحةٌ كاملةٌ تحت سوادٍ دائم.

**وقياسي قرأ العقدَ والنصَّ فقال «تعمل بلا JavaScript»، ولم يسأل: أتُرى؟** —
**وهو عينُ درس هذا المستودع**: «الحقلُ يُعلن ولا يقيس». وعدُّ العقدِ يقيس
الوجودَ لا الظهور.

**والعلاجُ حركةُ CSS تنتهي من نفسها** (`txSplashOut`)، وJavaScript للتخطّي
باللمس **زيادةً لا شرطاً**، وعند `prefers-reduced-motion` تختفي فوراً.
**والقياسُ بعده: `LCP` ‏6.7 → ‏2.3–2.5 ث.**

### وثانٍ كشفه القياسُ نفسُه — صورٌ تُطلب فتردّ الصفحةَ

`/assets/cars-stock/*.png` **لم تكن منسوخةً**، فيردّ الخادمُ `index.html`
بحجمه — **٩٥ ك.ب لكلِّ «صورة»**، ويقرؤها Lighthouse صوراً خارج الشاشة.
**ونقصُ ملفٍّ لا يظهر إلا في القياس**: الصفحةُ تُرسم كاملةً والشريطُ يبدو
فارغاً بلا خطأ. نُسخت وحُوّلت إلى WebP (−٨٠٪ إلى −٨٣٪) وتُحمَّل كسولاً.

### القياسُ على المقاسات الثلاثة — وبلا JavaScript

| | 360×640 | 390×844 | 412×915 | بلا JavaScript |
|---|---|---|---|---|
| تجاوزٌ أفقيّ | **0** | **0** | **0** | **0** |
| عقد | 634 | 634 | 634 | **634** |
| أقسام | 11 | 11 | 11 | **11** |
| أسئلة | 8 | 8 | 8 | **8** |
| نصٌّ يُقرأ | 5,035 | 5,035 | 5,035 | **5,035** |
| مساحاتُ لمسٍ دون 44 | **0** | **0** | **0** | **0** |

**وكانت خمساً في التصميم** (أربعةُ روابطِ تذييلٍ 23px ورابطُ الشعار 20px)،
**والعلاجُ حشوةٌ لا تكبيرُ خطّ** — الشكلُ كما رسمه التصميم والهدفُ وحدَه اتّسع.

**و`prefers-reduced-motion` في الطبقتين**: CSS يُلغي الحركةَ والانتقالَ
والتحويل، وJS يقرأ `matchMedia` ويتفرّع. **مقيسٌ وجودُه في المخرَج المبنيّ،
ولم يُقَس أثرُه بمحاكاةٍ للاستعلام** — ويُقال كذلك.

### Lighthouse — والرقمُ يُقال بمداه لا بوسيطه

**ستُّ تشغيلاتٍ على هاتفٍ محاكى (412×823، DPR 1.75):**

    الأداء        81 · 93 · 97 · 64 · 79 · 94     ← **مدىً واسع**
    الإتاحة       100 في الستّ
    الممارسات     100 في الستّ
    SEO           100 في الستّ
    LCP           2.3–2.5 ث   ·   CLS 0   ·   TBT 0–780 ملّي ثانية

**ولا يُقال «≥٩٠» ولا «دونها»**: التشتّتُ في `TBT` وحدَه (0 إلى 780)،
**والجهازُ يشغّل ١٢ حاويةً والمجموعةَ وChrome معاً في أثناء القياس** — فالرقمُ
يصف الجهازَ بقدر ما يصف الصفحة. **والقياسُ على جهازٍ هادئٍ بندٌ مفتوح.**

**وما أُصلح بالقياس لا بالظنّ**: `getTotalLength()` كان يُقرأ **لكلِّ مسارِ
أيقونةٍ عند الإقلاع** — وهو يفرض تخطيطاً. صار يُقرأ **عند ظهور الأيقونة**،
فالنتيجةُ المرئيةُ هي هي **وما تغيّر متى يُحسب لا كم**. `TBT` ‏550–780 →
‏0–610.

> **وفكرةٌ جُرّبت فأسقطها القياس**: تأجيلُ الحركة كلِّها إلى
> `requestIdleCallback` — **`TBT` ‏650 → ‏1400 و`Speed Index` ‏2.5 → ‏6.2**،
> لأن الخمولَ يقع داخل نافذة القياس فيصير مهمّةً واحدةً طويلة. **فأُعيدت كما
> رسمها التصميم**، والفكرةُ تُقيَّد مسقَطةً لا تُعاد.

### وحارسٌ للصفحة، مقيسٌ في الاتجاهين

`site/scripts/check-site.mjs` يسأل خمسةً: قالبُ مُشغِّلٍ متبقٍّ · بلدٌ غير
الأردن · رمزُ عملة · مفتاحٌ يقرؤه `site.js` ولا موضعَ له · وقائمةٌ ليست في
HTML. **ومقيسٌ بالنقض:**

| ما زُرع | الحكم |
|---|---|
| (بلا زرع) | **يمرّ** |
| «في الأردن وليبيا» | يسقط — ويسمّي «ليبيا» |
| «5 د.أ» | يسقط — ويسمّي «د.أ» |
| `{{ price }}` | يسقط — ويسمّيه |
| حذفُ سؤالٍ من HTML | يسقط — «تسقط بلا JavaScript» |

**وأمسك في أوّل تشغيلٍ فجوتين حقيقيتين**: `site.js` يقرأ `support_email`
و`privacy_email` وخمسَ أيقوناتِ تواصلٍ **ولا موضعَ لأيٍّ منها في الصفحة** —
**بابٌ بلا زرّ بوجهه المقلوب**: قارئٌ بلا مقروء.

### ⚠ وما لم يُنجَز — ولا يُقرأ سلامة

1. **اللقطاتُ الخمسَ عشرةَ لم تُلتقط** — والخاناتُ **مستطيلاتٌ محايدةٌ باسم
   الشاشة** كما اشترط المالك. **وسببُ التعذّر**: إضافةُ المتصفّح تمنع التقاطَ
   الصور على هذا الأصل (`Chrome blocked the extension from accessing this page`)،
   جُرّب مرّتين.
2. **والأربعُ والثلاثون في حزمة التصميم مرفوضةٌ كلُّها** — أدناه.
3. **شارةُ Google Play الرسمية ناقصة** — ولا تُرسم بديلاً، وحالُ `play` تعرض
   شارةً معطَّلةً حتى يصل الملفّ.
4. **ولم يُقَس شيءٌ على الإنتاج** — البناءُ والقياسُ على بيئة التطوير وحدَها.

## ⚠ لقطاتُ حزمة التصميم مرفوضةٌ كلُّها — **والأرقامُ هي البرهان** (٢٠٢٦-٠٩-٠٥)

**`screens/` الخمسَ عشرة**: `github.md` في الحزمة يعلن مصدرَها بنفسه —
`video-assets/wan-frames/{rider,captain}/*_screen.png`. **وقِيس ذلك المصدر**:
`_src/tmp/R01_screen.html` **مسرحُ HTML مكتوبٌ بيد** (720×1280، تدرّجاتٌ
وهالةُ ضوءٍ وشعار) يُصيَّر إلى PNG بـ`png.js`. **فهي واجهاتٌ مرسومةٌ لفيديو
دعائيّ لا لقطاتٌ من تطبيقٍ يعمل.**

**و`screenshots/` الأربعُ والثلاثون**، وثلاثةُ أرقامٍ تكفي:

    الأبعاد            ٩٢٤×٥٤٠ **أفقيّ** في الأربع والثلاثين — لا شكلَ هاتف
    ساعةُ شريط الحالة  9:41 — ساعةُ إعلانات آبل لا وقتَ جهاز
    الأرقام            ٤٢٬٥٠٠ · ٤٫٩ · ٧ · ١٨٬٢٥٠ — **عربية-هندية**

**والأخيرُ وحدَه قاطع**: `check:digits` حارسٌ أخضرُ اليوم يضمن أن كلَّ رقمٍ
يعرضه التطبيقان **لاتينيّ**. **فلقطةٌ من تطبيقٍ يعمل لا يمكن أن تُظهر
٤٢٬٥٠٠.**

> **وختمُ C2PA على ملفّات الحزمة لا يقرّر شيئاً**: نصُّه «Claude provided this
> file … **and may have created or modified**» — **ولا يفصل «التقط» عن
> «ولّد»**. **والفيصلُ مصدرُ اللقطة لا ختمُها** (قرارُ المالك)، وقد حُسم
> بالأبعاد والساعة والخانات.

## والشعارُ مشتقٌّ لا رسميّ — **يُسجَّل بقول المالك** (٢٠٢٦-٠٩-٠٥)

`assets/logo.png` (1233×234) هو ما استعمله التصميم، **وليس أيَّ أيقونةٍ من
التطبيقين**: `md5` يخالف الثلاثة. و`github.md` في الحزمة يقول: «استُخرج شعار
TAXO الحقيقي **من أيقونة تطبيق الراكب**».

**فهو مشتقٌّ منها لا منقولٌ عنها، ولا شعارَ موقعٍ رسميٌّ في المشروع** — وقرارُ
المالك ٢٠٢٦-٠٩-٠٥: يُستعمل كما فعل التصميم، **ويُسجَّل أنه مشتقٌّ لا رسميّ**.

## وأين يسكن الموقعُ ولماذا (٢٠٢٦-٠٩-٠٥)

    site/            المصدر — index/privacy/terms/404 · src/site.js · scripts/
    site/assets-src/ **مُدخَلاتُ التوليد** — PNG وTTF، تُودَع ولا تُنشر
    site/public/     **مخرَجُ التوليد** — WebP وwoff2، يُنشر ولا يُودَع
    landing/         **الجذرُ المخدوم** — مخرَجُ `vite build`، ويُودَع

**و`emptyOutDir: false` بقصدٍ مقيس**: `landing/downloads/` هو ما يكتب فيه
`scripts/pull-release.sh` على الخادم، **والإفراغُ الافتراضيُّ يمسحه** — فتذهب
الحزمُ المنشورةُ مع أوّل بناء، **ولا شيءَ يشكو حتى يضغط أحدٌ زرَّ التنزيل**.
**وقِيس بعد أوّل بناء: الحزمتان والبيانُ في مكانها.**

**وفصلُ المُدخَل عن المخرَج قِيس أيضاً**: `public/` يُنسخ حرفاً إلى `landing/`،
وكان فيه أصولُ PNG وTTF **لا تشير إليها الصفحةُ أصلاً**. `landing` ‏2.2 م.ب →
**‏860 ك.ب**.

**و`/api/` وكالةٌ في نجينكس لا نداءٌ عابرُ أصل**: `landing/app.js` كان ينادي
`https://stg-api.tajora.ly` من `taxo.tajora.ly`، **و`cors_origins` لا يحمل اسمَ
الصفحة** — فالمتصفّحُ يمنع قراءةَ الجواب و`catch` يبتلعه. **فسطرُ العرض لم
يظهر على الإنتاج قطّ، ولا شيءَ كان يقول ذلك.** والوكالةُ أضيقُ من توسيع
`cors_origins`.
<!--/جديد-->

<!--جديد-->
## الموقعُ — الخمسةُ التي أمرتَ بها، مقيسةً (٢٠٢٦-٠٩-٠٥، الجولةُ الثانية)

### ١ · شارةُ Google Play — نُزّلت ولم تُرسم

`https://play.google.com/intl/en_us/badges/static/images/badges/ar_badge_web_generic.png`
— **٦٤٦×٢٥٠، ١٥٬٨٢٣ بايت**، تُنسخ إلى المخرَج **كما هي بلا تحويلٍ إلى WebP**:
**تحويلُ ملفِّ علامةٍ تجاريةٍ مسٌّ له**، وحجمُها لا يستحقّ ذلك.

### ٢ · رابطا المتجر — مشتقّان لا فارغان

    play_url_rider   https://play.google.com/store/apps/details?id=ly.tajora.rider
    play_url_driver  https://play.google.com/store/apps/details?id=ly.tajora.driver
    distribution_mode  apk   ← **يبدّله المالكُ بنفسه من اللوحة**

**ولمَ افتراضٌ لا فراغ**: يومَ يُبدَّل الوضعُ **يجد المشرفُ الرابطَ جاهزاً**
بدل شارةٍ معطَّلةٍ يظنّها عطباً. **والحالُ تبقى `apk`** — إعدادٌ محضَّرٌ ليس
إعلاناً.

### والحالتان مقيستان بالضغط من الشاشة (الشكلُ التاسع)

| | `apk` | `play` |
|---|---|---|
| الزرّ | «تنزيل APK الراكب» → `/downloads/taxo-rider.apk` | **شارةُ Google الرسمية** → `play.google.com/…id=ly.tajora.rider` |
| بيانُ الحزمة | `1.0 · 7ecf178d · 29 أغسطس 2026 · 4.6 م.ب` | **صفر** |
| خطواتُ التثبيت | ٣ | **٠** |
| سطرُ «أقدم من v0.2.0» | موجود | **غائب** |
| كلمةُ APK في الصفحة | ٢ | **٠** |
| سطرُ العلامة التجارية | مخفيّ | **ظاهر** |

**والتبديلُ من اللوحة وحدَها** — `PATCH /admin/site` ثمّ إعادةُ تحميلٍ، بلا نشر.

### ٣ · مخرَجُ البناء خرج من الشجرة، والخادمُ يبنيه

**قرارُ المالك**: «يبنيه الخادمُ كما يبني الثلاثة — سطرٌ في `deploy.sh` — ولا
نُودع مخرَج بناء في الشجرة».

    for a in customer-app driver-app admin-panel site   ← كانت ثلاثةً
    BUILT=4                                              ← وكان ٣

**ومقيسٌ في `node:22-alpine` قبل أن يُكتب السطر**: `npm ci` ثمّ `npm run build`
داخل الصورة نفسِها التي يبني بها الخادمُ الثلاثة — **و`sharp` و`ttf2woff2`
بُنيا على musl**، والمخرَجُ أربعةُ ملفّاتِ HTML وأصولُها. **وسطرٌ لم يُقَس في
هذا الباب يسقط على الإنتاج وحدَه.**

**و`landing/` لم يعد يُودَع منه إلا ملفّان**: `nginx.conf` و`spa.conf` —
**مركَّبان في `docker-compose.prod-tunnel.yml`** (والثاني في التطبيقات الثلاثة
أيضاً)، **فهما مصدرٌ لا مخرَج**. وحُذف من الإيداع ٢٦ ملفَّ بناء.

### ⚠ وعطبٌ من هذا الفصل نفسِه، كشفه القياس

**حين صار `site/public/` مخرَجاً متجاهَلاً، بقي فيه ثلاثةُ ملفّاتٍ لا يولّدها
أحد**: `favicon.png` و`car.svg` و`logo.svg`. **فاستنساخٌ جديدٌ كان يبني الموقعَ
بلا أيقونةٍ ولا سيارة**، **ولا شيءَ يشكو**: الصفحةُ تُرسم والأيقونةُ لا تظهر.

**والقاعدةُ التي يخرج بها**: حين يصير مجلَّدٌ مخرَجاً، **يُجرد ما فيه ملفّاً
ملفّاً** — «هل يولّده أحد؟» — **ولا يُفترض أن كلَّ ما فيه مولَّد**. وقِيس
بحذف المخرَج كلِّه وإعادةِ بنائه من المُدخَلات وحدَها.

### ٤ · اللقطات — ثمانٍ حقيقيةٌ وسبعُ خاناتٍ محايدة

**Playwright يقود Chrome المركَّب** (لا تنزيلَ متصفّح، كما يوثّق
`COMMANDS.md`)، على `390×844` بـ`DPR 3`، **بحسابات البذرة**.

    rider-home · rider-wallet · rider-rides
    captain-home · captain-earnings · captain-subscription · captain-documents · captain-garage

**والمُركَّبُ منها في الصفحة ستٌّ** (ما يطابق اسمُه خانتَه)، **وسبعُ خاناتٍ
تبقى مستطيلاتٍ محايدةً باسم الشاشة**.

### ⚠ وثلاثةُ أخطاءٍ في قياسي أنا، كلُّها من نوعٍ واحد

**١) التُقطت خمسُ لقطاتٍ من خادمٍ أجنبيّ.** `localhost:5173` **ليس منفذَ
حاويتنا**: تطبيقُ الراكب ينشر على `5176` (`CUSTOMER_APP_PORT`)، و`5173` يحمله
خادمٌ آخرُ على ويندوز. **واللونُ الأصفرُ وحدَه كشفه** — `ARCHITECTURE.md` يقول
إن الأصفرَ زال من التطبيق. **وهو فخُّ `localhost` المكتوبُ في `COMMANDS.md`
بعينه**، وقعتُ فيه بعد أن قرأتُه.

**٢) «شاشةٌ غيرُ فارغة» ليست «الشاشةَ المطلوبة».** حارسي الأوّلُ عدّ النصَّ
والعقد، **فمرَّت شاشةُ ترحيبٍ واحدةٌ خمسَ مرّاتٍ بخمسة أسماء** — **وتساوي
الأرقام (٢٥٧ حرفاً و٨٦ عقدةً في الخمس) هو ما كشفها**.

**٣) وعلامةٌ تطابق الترويسةَ تمرّ على شاشةٍ تُحمِّل.** «المحفظة» عنوانٌ يُرسم
قبل وصول البيانات، **فحُفظت دوّارةُ تحميلٍ باسم محفظة** — ورأيتُها بالعين لا
بالرقم.

**والحارسُ صار ثلاثةَ شروط**: **الحزمةُ المخدومةُ تطابق `dist`** · **وعلامةٌ
تخصّ الشاشةَ** · **وزوالُ الدوّارة**. **وطولُ النصِّ ليس فيصلاً** — شاشةُ
الخريطة ٧٩ حرفاً وهي مكتملة، **فرفضُها بالطول رفضُ سليم**.

### وحدُّ الدخول ضربني ثلاث مرّات

`429` على حسابات البذرة. **والمفتاحُ `ratelimit:login:*` لا `login:*`** — كنتُ
أمسح نمطاً لا وجودَ له **فأظنّه مُسح**، وهو حارسٌ صامتٌ في أداتي. **والعلاجُ
ذاكرةُ توكنٍ على القرص**: تشغيلٌ لا يعيد الدخول.

### ٥ · Lighthouse — بلغت الشرط

| | قبل الإصلاح | بعده |
|---|---|---|
| الأداء | `81·93·97·64·79·94` (مدىً واسع) | **`90 · 94 · 93`** — وسيطُه **٩٣** |
| LCP | 2.3–4.1 ث | **2.5–2.9 ث** |
| الإتاحة · الممارسات · SEO | ١٠٠ | **١٠٠** |

**وما أصلحه القياسُ لا الظنّ:**

    الشعار    ١٢٣٬٨٤٥ → ٦٬٦٩٨ بايت   — كان يُشحن بعرض ١٢٣٣ ويُرسم بـ١٠٤
    favicon   ١٣٩٬٨٢٣ → ٣٬٥٦١ بايت   — **أثقلُ موردٍ في الصفحة كان أيقونةَ تبويب**

**وصورةٌ أكبرُ من موضعها ليست جودةً، هي انتظار** — وقد كانت **هي عنصرَ LCP**.

### مالٌ في لقطةٍ معروضة — **القاعدةُ حُسمت، فلا تُستأنف** (قرارُ المالك ٢٠٢٦-٠٩-٠٥)

**سُئل في لقطة `rider-wallet` (رصيدٌ بالدينار على شاشةٍ حقيقية)، فجاء الحكمُ
قاعدةً لا جواباً لسؤال:**

> «شرطي يمنع **ما يعِد المستخدمَ بمال** — أسعارُ اشتراكٍ ومبالغُ عروضٍ
> ومكافآت. **ورصيدُ محفظةٍ عرضُ ميزةٍ لا وعد.**»

**فالفيصلُ ليس «أفي الصورة رقمُ مال؟» بل «أيُقرأ الرقمُ وعداً؟»** — والفرقُ
هو ما جعل السؤالَ يُطرح أصلاً: منعُ كلِّ رقمِ مالٍ كان سيُسقط شاشةَ المحفظة
كلَّها، **وهي ميزةٌ مبنيّةٌ منشورةٌ يريد الزائرُ رؤيتَها**.

**وشرطان يلزمان كلَّ لقطةٍ فيها مال، ويُقاسان قبل قبولها:**

| الشرط | لمَ | مقيسٌ على المقبولتين |
|---|---|---|
| **١) الرقمُ من حساب بذرةٍ لا من حسابٍ حقيقيّ** | رصيدُ إنسانٍ حقيقيٍّ على صفحةٍ عامّةٍ تسريبٌ ماليّ، **ولو كان اسمُه محجوباً** | `rider-wallet` = `9.223 د.أ` من حساب البذرة |
| **٢) ورقمٌ محايدٌ لا يُقرأ ربحاً متوقَّعاً** | رقمٌ في **أرباح الكبتن** يصير إعلانَ دخلٍ ضمنيّاً — **وهو وعدٌ بمال بعينه** | `captain-earnings` = `0.000 د.أ` وصفرُ رحلاتٍ منجزة |

**ولمَ سطرُ «لا في لقطة أرباح الكبتن» منصوصٌ بذاته**: هو **الموضعُ الذي
ينقلب فيه العرضُ وعداً**. محفظةٌ فيها رصيدٌ تقول «هنا تحفظ مالَك»؛ **وأرباحٌ
فيها رقمٌ تقول «هذا ما ستكسب»** — والثانيةُ دعوى دخلٍ لا ميزة. **فالشاشةُ
التي عليها الرقمُ جزءٌ من الحكم، لا الرقمُ وحدَه.**

**والقاعدةُ تُطبَّق على كلِّ لقطةٍ فيها مالٌ من اليوم**، وكُتبت هنا **كي لا
تُستأنف في كلِّ جولة** — وهو ما أمر به المالكُ نصّاً.
<!--/جديد-->

<!--جديد-->
## عطبُ تخطيط اللوحة على الهاتف — **قِيس بالعناصر فسقط ١٤٦ ← ٠** (٢٠٢٦-٠٩-٠٥)

**البندُ كان مفتوحاً بوصفٍ لا بقياس**: «رأيتُه بعيني على جهازي». **ووصفٌ لا
يقول أيَّ عنصرٍ ولا كم بكسلاً، ولا يُعرف بعده أزال العطبُ أم بقي.**

### أوّلاً: صار قياساً — `tools/check-mobile/check-mobile.mjs`

**أربعةُ مقاساتٍ × عشرةُ مساراتٍ = ٤٠ فتحة**، وستّةُ أسئلةٍ لكلٍّ منها **لها
جوابٌ رقميّ**: `scrollWidth > clientWidth` · عنصرٌ خارجَ إطار العرض ·
مساحةُ لمسٍ دون ٤٤ · تقاطعُ عنصرين تفاعليّين · محتوىً تحت شريطٍ ثابت ·
**وأثرُ لوحة المفاتيح** (تُحاكى بقصِّ الارتفاع ٣٠٠px بعد تركيز حقل، **فما
يقفز عند فتحها يقفز هنا**).

### والمسار: ١٤٦ ← ٦٦ ← ٩ ← ٢ ← **٠**

| ما قِيس | العلّة | البيت |
|---|---|---|
| **تمريرٌ أفقيٌّ ٤٦٣ في شاشةٍ ٣٦٠** على ٩ مساراتٍ من ١٠ | صفُّ الترويسة `flex` **بلا التفافٍ وبارتفاعٍ ثابت** | `Shell.tsx` |
| **الزرّان ٢٢×٣٤ لا ٣٤×٣٤** | **لم يخرجا بل انضغطا** — والصنفُ يقول `size-34` **بينما القياسُ ٢٢** | `Shell.tsx` |
| **حقلُ البحث ٣٢px عرضاً** | `w-side max-w-full` تقيس إلى **الحاوية** لا إلى الشاشة | `GlobalSearch.tsx` |
| **شرائحُ الترشيح ٣٤px** في ٦ شاشات | صنفٌ واحدٌ يخدمها كلَّها | `Table.tsx` |
| **«فعّله الآن» ٦٤×٢٩** على كلِّ شاشة | **الفعلُ الوحيدُ في شريط التذكير** | `Shell.tsx` |
| **مربّعُ الاختيار ٢١px** | صفٌّ بلا حدٍّ أدنى | `ui/Field.tsx` |
| **تقاطعٌ ٨×١٧ في `/rides`** | **ثمانيةُ أعمدةٍ كسريّةٍ في ٣٦٠px** ← نحوُ ٣٠px للعمود، **وزرُّ «الملفّ» وحدَه ٢٧** | `Table.tsx` |
| **٧٥٨ عند فتح لوحة المفاتيح** | البطاقةُ **نمت إلى محتواها** بعد أن صار لابنتها حدٌّ أدنى | `Table.tsx` |

**وأربعةُ بيوتٍ لا أربعةٌ وعشرون**: `Shell` · `Table` · `GlobalSearch` ·
`Field`. **وإصلاحُ الحقل لا البانِي كان سيترك اثنتين وعشرين شاشةً كما هي.**

### والأخيران هما الدرسُ فيها

**تقاطعُ ٨×١٧ لم يكن عطبَ زرّين**: الخليةُ **لا تسع محتواها ولو قُصَّ الاسمُ
إلى صفر** (٣٠px موضعاً و٢٧px زرّاً و٦px فاصلاً). **فالعَرَضُ تقاطعٌ والعلّةُ
عمودٌ مستحيل** — وعلاجُ العَرَض (قصُّ الزرّ) كان **يخفي العلّة ويبقيها**.

**والعلاجُ فتح عطباً ثانياً في الجولة نفسِها**: حدٌّ أدنى ٤٤rem للصفوف
**جعل البطاقةَ تنمو إلى محتواها** فدفعت الصفحةَ إلى ٧٥٨ — **والتمريرُ الأفقيُّ
لا يعمل ما لم يُؤذَن للحاوية أن تضيق دون محتواها** (`min-w-0`). **ولم يظهر
إلا تحت لوحة المفاتيح**: الفحصُ الذي أُضيف آخِرَ شيءٍ هو الذي أمسكه.

### ⚠ والحارسُ نفسُه اخترع عطباً في أوّل تشغيل

**بلّغ عن أربعة عناوينَ «خارجَ الإطار ١١٨px» في مصفوفة الصلاحيات — وهي سليمةٌ
بالبنية**: داخل `overflow-x-auto` بحدٍّ أدنى مصرَّح، **تُمرَّر في صندوقها ولا
تدفع الصفحة**. **وحارسٌ يخترع عطباً أغلى من واحدٍ يفوته** — يُطفأ، فيسقط معه
ما يمسكه حقاً. **فصار يستثني ما تحته حاويةُ تمريرٍ أفقيّ**.

### وما لا يقيسه، مكتوبٌ فيه

**خلايا الجداول التي تُمرَّر أفقياً تُعدّ وتُقال ولا تُسقط** (٨ · ٤٤ · ٩٦ ·
١٢٣ بحسب الشاشة): **رفعُ خليةِ مصفوفةٍ إلى ٤٤ إعادةُ تصميمٍ لا إصلاحُ تخطيط**.
**والرقمُ يبقى مرئياً كي لا يصير الصمتُ عُرفاً.**

**ولا يقيس هاتفاً حقيقياً** — متصفّحُ مكتبٍ بمقاسِ هاتفٍ و`hasTouch`،
**ولوحةُ المفاتيح محاكاةٌ بالقصّ لا لوحةٌ تُفتح**.

### ومقيسٌ في الاتجاهين قبل أن يُصدَّق

`--negate` **يحقن العطبَ المقيس نفسَه** — ترويسةٌ `flex-wrap: nowrap` وأزرارٌ
`34px` — **ويسقط الحارسُ إن لم يصح**. **فبابٌ يصمت دائماً ليس باباً.**

### ولا نسخةَ هاتفٍ منفصلة

**كلُّ ما تغيّر نقاطُ توقّفٍ على المكوّن نفسِه** — `sm:` وحدَها — **ولا مسارَ
ولا منطقَ ولا نداءَ**. و`/m/payments` وحدَها شاشةُ هاتفٍ مستقلّةٌ **وكانت
خضراءَ في المقاسات الأربعة قبل الإصلاح وبعده**: شاهدُ ضبطٍ يقول إن القياسَ
يفرّق بين المعطوب والسليم.

### وقصاصةُ الشاشة صارت مُعمَلةً لا مكتوبة

`safe-top` و`safe-bottom` في `index.css` **نسخةً من `driver-app`** لا اجتهاداً
ثانياً. **ومعها `viewport-fit=cover`**: بدونه تعود `env(safe-area-inset-*)`
**صفراً دائماً** — فالصنفُ يُكتب ويُقرأ حراسةً **وهو لا يفعل شيئاً**، وهو
«شرطٌ لا يتحقّق أبداً يُقرأ حراسةً وهو تعطيل» بعينه.

### ⚠ وما يبقى شرطاً بشرياً

**الغلافُ العامُّ يقرأ من `admin.tajora.ly`** — **فالإصلاحُ لا يبلغ هاتفَ
المالك إلا برفعة**، ويُقاس هناك **بعدها لا قبلها**.
<!--/جديد-->

<!--جديد-->
## مهلةٌ قتلت مجموعةً خضراء — **والرقمُ كان محسوباً على بياناتٍ بليت** (٢٠٢٦-٠٩-٠٥)

**البوّابةُ الثانيةُ سقطت بنصِّها**: `✗ CI أحمرُ (cancelled)`. **ولم يكن فيها
اختبارٌ ساقط.**
...
| `f7d90f5` | ١٧د ٠٥ث | ✓ |
| `745310e` | ١٩د ٠٣ث | ✓ |
| `b2c51d8` | **فوق ٢٥** بين ستٍّ متوازية · **١٩د ١٣ث وحدَه** | ✓ بعد الإعادة |
| `757a7e9` | **٢٣د ٤١ث** | ✓ — **بـ٧٩ ثانيةً وحدَها تحت الحدّ** |
| `d9da0a5` | **فوق ٢٥ مرّتين، والثانيةُ وهو وحدَه** | ✗ |
| المجموعةُ عند الرأس `0fac708` | ١٩د ٣٩ث | ✓ |

### والحاسمُ أن الجامدَ لم يكن ساقطاً

سجلُّ الوظيفة: **١٬٠٨٤ نقطةً ناجحةً، وصفرُ `F`، وصفرُ سطرِ `FAILED`** —
والوحيدُ الذي فيه كلمةُ `FAILED` هو **نصُّ أمر `grep` نفسِه**. **فالمجموعةُ
كانت تمرّ ونفد وقتُها.**

**ومهلةٌ تقتل مجموعةً خضراءَ تُقرأ حمرةً فتوقف رفعاً بلا عطب** — وهو **بلاغٌ
كاذبٌ في ثوب ميزانية**، من عائلة «حارسٌ يخترع عطباً أغلى من واحدٍ يفوته».

### والرقمُ لم يكن موضوعاً، كان محسوباً — ثمّ بلي

**التعليقُ نفسُه كان يحمل حجّتَه**: «الأبطأُ مقيساً ١٦د ٣٣ث… والتوازي يكلّف
دقيقةً ونصفاً لا أضعافاً». **وشِقّاه سقطا معاً على الشيفرة نفسِها**: المدى
اليومَ **تسعُ دقائقَ لا دقيقةٌ ونصف**، والأبطأُ **٢٣د ٤١ث لا ١٦د ٣٣ث**.

**وهو الشكلُ المكتوبُ في `CLAUDE.md` بعينه** — «رقمٌ يُكتب في نثرٍ يبلى، ثم
يُنقل بعد شهرٍ على أنه خبر» — **ووقع هنا على رقمٍ يحرس بوّابة**، لا على رقمِ
حجمِ ملفّ. **فبلاؤه لا يُقرأ خطأً في وصف، يُقرأ رفعاً موقوفاً.**

### والعلاجُ شقّان لأن العلّة شقّان

- **`timeout-minutes: 25 ← 40`** — لأن `d9da0a5` جُمد **وهو وحدَه**، فالباقي
  **تفاوتُ عتادٍ لا يُشترى بحدٍّ أضيق**.
- **`max-parallel: 4`** — لأن `b2c51d8` مرّ في ١٩د ١٣ث وحدَه بعد أن جُمد بين
  ستّ: **التوازي يكلّف نحوَ ستِّ دقائق**. **والدفعةُ تكبر بعدد الإيداعات، فما
  عليه القياسُ لم يكن محدوداً أصلاً** — وهذا هو سببُ بلاء الرقم.

### وحجّةُ الحصّة انقلبت بالقياس

**التعليقُ كان يرفض ٤٥ لأنها «تبيع خمسَ عشرةَ دقيقةً من الحصّة في كلِّ
عالق».** **والحدُّ الضيّقُ كلّف اليومَ نحوَ ٧٥ دقيقة** — قتلتان وإعادتان —
**وأوقف الرفعَ مرّتين**. **والخمسَ عشرةَ لا تُدفع إلا في عالقٍ حقيقيّ وهو
نادر. فالأرخصُ هو الأوسع** — ولم يكن ذلك معروفاً حتى قِيس.
<!--/جديد-->

<!--جديد-->
## البوّاباتُ الستُّ خضراء والموقعُ مكسور — **الشكلُ العاشر عند الحافّة** (٢٠٢٦-٠٩-٠٥)

**رُفع `861405d` وخضّرت البوّاباتُ الستُّ كلُّها**: الإيداعُ والترحيلةُ
والحاوياتُ والصحّةُ وصفرُ خطأٍ في السجلّ. **ثمّ سُئلت الصفحةُ نفسُها فسقطت.**

| ما سُئل | الجواب |
|---|---|
| `taxo.tajora.ly/` | ✓ الحزمةُ `index-2C1RDgF8.js` = ما بناه الخادم |
| `taxo.tajora.ly/privacy` | **٤٠٤ — ١٥٣ بايتاً**، والملفُّ تحته **١٠٬٩٨٥** |
| `taxo.tajora.ly/terms` | **٤٠٤** |
| `error_page` داخل الحاوية | **صفر** |
| `location /api/` داخل الحاوية | **صفر** |

**فكلُّ ما قِيس كان صحيحاً، وما لم يُقَس هو ما سقط.**

### علّتان لا واحدة

**١) `try_files $uri $uri/ =404` لا يجرّب `.html` أبداً.** طلبُ `/privacy`
يبحث عن ملفٍّ اسمُه `privacy` ثمّ مجلَّد، **ولا يبلغ `privacy.html` قطّ**.
**والتعليقُ فوقَ السطر كان يقول «صفحتان مبنيّتان تُقرآن بلا JavaScript»** —
**صادقٌ في البناء، كاذبٌ في الخدمة**، وهو «توثيقٌ يصف ما ليس كذلك» بعينه.

**٢) وربطُ ملفٍّ مفردٍ يمسك عقدتَه لا مسارَه.** `landing/nginx.conf` مربوطٌ
ملفّاً مفرداً؛ **و`git pull` يكتب ملفّاً بعقدةٍ جديدة، والحاويةُ تبقى على
التي وُلدت عليها**. فوكيلُ `/api/` وصفحةُ ٤٠٤ كُتبا على القرص **ولم ترَهما
الحاويةُ**، **والموقعُ يُخدَم بقيَمه المخبوزة**: لا إعداداتِ موقعٍ ولا سطرَ
عرض — **ولا رسالةَ خطأٍ في أيِّ مكان**.

### ⚠ والعلّةُ الثانيةُ كانت مكتوبةً في الملفِّ نفسِه منذ ٢٠٢٦-٠٨-٢٥

`deploy.sh` يحمل التشخيصَ حرفاً: «`up -d` لا يعيد حاويةً لم يتغيّر **تعريفُها**
— فيبقى القديمُ حيّاً في الذاكرة والجديدُ على القرص، **ولا يشكو شيء**».
**وعُولج `cloudflared` وحدَه.**

**فالدرسُ لم يُعمَّم**: التطبيقاتُ الثلاثةُ تربط `spa.conf` **منذ البداية**،
ثمّ جاء الموقعُ بملفٍّ رابعٍ من الشكل نفسِه. **ودرسٌ يُعالَج في موضعه وحدَه
ينتظر موضعاً ثانياً** — وهو «المعالجةُ بالبانِي الواحد لا بالحقل» في ثوبِ
حاوية. **فصارت القائمةُ تُذكر كلُّها لا التي تُذكَر**:

    CONF_MOUNTED='cloudflared landing customer-app driver-app admin-panel'

### والحارسُ: الحافّةُ تُسأل عمّا وعدت به لا عن حياتها

**«أتعمل الحاوية؟» لا يجيب «أيصل المسارُ إلى ملفّه؟»** — وأضيف إلى البوّابة
السادسة سؤالان **يُقاسان بالطول لا بالرمز**: صفحةُ ٤٠٤ تردّ `200` حين تُوجَّه،
**والرمزُ يقول «وصل شيء» لا «وصل الصحيح»**.

**ومقيسٌ في الاتجاه الآخر على عطبٍ حقيقيٍّ لا مزروع**: شُغِّل على الإنتاج
**والعطبُ حيٌّ بعد**، فصاح في الثلاثة — `/privacy` ١٥٣ بايتاً، `/terms` ١٥٣،
و`/api/landing` يردّ صفحةَ ٤٠٤ من nginx. **وهو أقوى من القياس على عطبٍ
مزروع**: العطبُ هنا لم يُصنع ليُمسَك.
<!--/جديد-->

<!--جديد-->
## البند ١٠ اكتمل — **النشرُ كشف ثلاثةَ عطوبٍ كان المفتاحُ المطفأُ يسترها** (٢٠٢٦-٠٩-٠٥)

**اعتمد المالكُ النصوصَ فنُشرت**، وبذلك **حلّ اليومُ الذي جدولته المواصفةُ
بنصِّها** (§45): «لا شاشةَ قبولٍ في تطبيقٍ… **ويُبنى يومَ يعتمد المالكُ
النصوص**، ومعه شاشتان في التطبيقين».

### أربعٌ لا ثمانٍ — والإنتاجُ كان فارغاً

| السوق | التطبيق | النوع | النسخة | الحروف |
|---|---|---|---|---|
| JO | rider | `privacy_policy` | 1 | 2,414 |
| JO | rider | `terms_of_use` | 1 | 1,429 |
| JO | driver | `privacy_policy` | 1 | 1,649 |
| JO | driver | `terms_of_use` | 1 | 1,468 |

**والثمانِ صارت أربعاً بأمر المالك** («ليبيا لا تُذكر»). **والإنتاج كان صفرَ
وثيقةٍ وصفرَ بيانِ جهةٍ وصفرَ صفِّ إعدادات** — فالبذرُ لم يُشغَّل عليه قطّ،
**والصفحةُ كانت تُخدَم بقيَمها الافتراضية ولا شيءَ يقول ذلك**.

### و`requires_reconsent` لا أثرَ لها في أوّل نشرة

**قرارُ المالك**: «هذه أوّلُ نشرةٍ ولا موافقةَ سابقة، فلا معنى لإعادة طلب
قبول». **والقياسُ لا يخالفه، ويضيف**: `min_accepted_version = 1` **في
الحالين** — لأن `_stamp_min_accepted` بلا منشورةٍ سابقةٍ ترجع نسخةَ الصفِّ
نفسِه. **فالمفتاحُ يغيّر شيئاً في النشرة الثانية لا الأولى.**

### ⚠ وثلاثةُ عطوبٍ لم تظهر إلا لحظةَ الإشعال

**١) `data-doc="terms_of_service"` والتعدادُ `terms_of_use`.** الباب يردّ ٤٢٢
**و`catch` في الصفحة يبتلعه صامتاً**، فتبقى «لم تُنشر بعد» **والوثيقةُ
منشورة**. **وما ستره أن الميزةَ كانت مطفأة**: الصفحتان تقولان الشيءَ نفسَه،
**فالخاطئةُ تبدو كالصحيحة**. **وعطبٌ يستتر خلف مفتاحٍ مطفأ يعيش حتى يُشعَل.**
**وحارسُه**: `check-site` يقرأ `PolicyDocType` من `enums.py` نفسِه، **مقيسٌ في
الاتجاهين**.

**٢) والصفحتان تدّعيان حالاً لا تعرفانها.** النصُّ الثابتُ يقول «لم تُنشر بعد»
**قبل أن يُسأل الباب** — فلمّا نُشرت **بقيت تكذب على من لا JavaScript عنده**.
**والصفحةُ لا تعرف الحالَ قبل أن تسأل**: صارت تقول «جارٍ التحميل»، **والحكمُ
بعد الجواب** بثلاث حالاتٍ لا واحدة (نصّ · لم تُنشر · تعذّر)، ومعها `<noscript>`
يقول الثمنَ ويعطي بديلاً.

**٣) وبوّابةُ القبول لم تكن مبنيّةً أصلاً.** `user_policy_consents` **صفرُ
كاتبٍ في الخلفية كلِّها** — الموديلُ وعدُّ القراءةِ فقط — **وشاشتا التسجيل
فيهما جملةٌ بلا رابطٍ ولا نصٍّ ولا صفّ**: «بالمتابعة أنت توافق على شروط
الاستخدام». **فهي موافقةٌ مفترَضةٌ لا معطاة**، **ويومَ يُسأل «أوافق هذا على
النسخة الأولى؟» لا جواب**.

### وما بُني، ومقيسٌ بفعلٍ حقيقيّ

    GET /public/policies/required?country_code&app   ← بلا مصادقة، ولا يمرّ بمفتاح الموقع
    POST /auth/register  ← accepted_policy_ids: معرّفاتٌ لا `true`

**والمعرّفاتُ لا علامةٌ منطقية**: `true` تقول «وافق» ولا تقول **على ماذا**،
**فتستنتج الخلفيةُ «المنشورَ الآن»** — **ونشرةٌ تقع بين عرض النصِّ وإرساله
تكتب موافقةً على نصٍّ لم يقرأه أحد**.

**وبابُ التطبيق مستقلٌّ عن `policies_public`**: ذاك مفتاحُ **الموقع**، وهذا
سؤالُ **التطبيق**. **وربطُهما بمفتاحٍ واحدٍ يجعل إخفاءَ صفحةِ ويبٍ يُسقط
بوّابةَ القبول في التطبيقين** — أثرٌ لا يقصده من يطفئ مفتاحَ موقع.

| ما قِيس | النتيجة |
|---|---|
| تسجيلٌ بلا موافقة | **٤٢٢** «لا يُنشأ حساب قبل الموافقة على: سياسة الخصوصية، شروط الاستخدام.» |
| موافقةٌ ناقصة | **٤٢٢** — **يسمّي الناقصَ وحدَه** |
| موافقةٌ كاملة | **٢٠١**، وصفّان في `user_policy_consents` بالمستخدم والتاريخ ورقم النسخة |
| وثائقُ كبتنٍ لتسجيل راكب | **٤٢٢** — لكلِّ تطبيقٍ نصُّه (§34-٢) |
| تكرارُ الموافقة | صفٌّ واحدٌ لا اثنان |

**والفحصُ قبل إنشاء الصفّ لا بعده** — كما الرقمُ تماماً، **فلا يبقى حسابٌ
نصفُ منشأٍ لموافقةٍ لم تُعطَ**. **والكتابةُ في معاملة التسجيل نفسِها.**

### ⚠ واختبارٌ بهامش ٥٪ أسقط المجموعةَ مرّتين

`test_broadcasts_during_the_ride_become_route_points` ينام **١٫٠٥ ثانية**
والنافذةُ **مفتاحُ Redis بعمر ثانية** — **وRedis يُنهي المفاتيحَ كسولاً لا في
اللحظة**. سقط `2 == 3` **مرّتين في مجموعةٍ كاملةٍ على جهازٍ محمَّل، ومرّ وحدَه
في كلِّ مرّة**. **وهامشُ ٥٪ ليس هامشاً** — صار ١٫٣، وثمنُه ٠٫٧٥ ثانية.

**ولا علاقةَ له ببوّابة القبول**: سطحٌ آخر، **والدليلُ أن ١٬٣٩٤ اختباراً مرّ
معه** ومنها عشراتُ التسجيلات.

### وما يبقى شرطاً بشرياً

**البوّابةُ لم تُقَس على الإنتاج بعد** — الشيفرةُ لم تُرفع، **وتُقاس هناك بعد
الرفعة لا قبلها**.
<!--/جديد-->

<!--جديد-->
## نسخةٌ ثانيةٌ بلا كلمة «مسوّدة» — وبيتٌ للحزم الخاصّة (٢٠٢٦-٠٩-٠٥)

### ١ · الكلمةُ حُذفت، **والنسخةُ الأولى بقيت**

**الوثائقُ الأربعُ كانت تبدأ «مسوّدةٌ تصف…» وهي منشورة** — فالكلمةُ **خبرٌ
كاذبٌ عن نفسها**. أذن المالكُ بحذفها، **وحُذف المفتتحُ وحدَه وبقي ما بعده
حرفاً بحرف**: الجملةُ تحمل نطاقَ الوثيقة (ما تصفه ولمن)، **وحذفُها كلَّها
يُسقط مضموناً لا وصفَ حال**.

    مسوّدةٌ تصف العلاقةَ بينك وبين المنصّة…  ←  تصف هذه الوثيقةُ العلاقةَ…

**ولا تُحرَّر نسخةٌ فوق نفسها** (§34): `create_version` كتب v2 و`publish`
**نزع v1 في المعاملة نفسِها** — **وv1 تبقى بنصِّها وتاريخِ نشرها شاهدةً على
ما كان منشوراً**. **و`requires_reconsent = false`** بقرار المالك: «حذفُ كلمةٍ
تصف حالَ الوثيقة لا يغيّر ما وافق عليه أحد».

**و`min_accepted_version` بقيت 1** — فمن وافق على v1 **لا يُسأل ثانيةً**،
وهو أثرُ القرار مقيساً لا موصوفاً.

### ⚠ وحارسٌ صاح على بيانات القياس نفسِها

سكربتُ النسخة الثانية **يقف إن وجد موافقاتٍ قائمة** — وصاح على التطوير
(موافقتان من قياس البوّابة). **والقرارُ كان معطىً سلفاً**، فصار يُتجاوَز
**بتصريحٍ لا بالسكوت** (`TAXO_CONSENT_OK=1`) **ويطبع العددَ دائماً**: فلو كان
على الإنتاج موافقون لَقُرئ القرارُ على عددٍ معلوم لا على ظنّ. **والإنتاج صفر.**

### ٢ · وبيتٌ للحزم الخاصّة — **غيرُ مدرَجٍ لا يعني غيرَ محروس**

اختار المالكُ **رابطاً مباشراً غيرَ مدرَج** لغلاف المشرف، وعلّتُه بنصّه:
«الإدراجُ العامُّ يُسقط الحارسَ كلَّه لا التجريبيةَ وحدَها… فأصير أمام حذف
حارسٍ يمنع تسريبَ أغلفةٍ خاصّةٍ مقابل راحةِ تنزيلٍ لا أحتاجها».

**و`check-apk` كان يمسح `landing/downloads` وحدَه** — **فحزمةٌ خاصّةٌ في
مجلَّدٍ آخر تنجو بلا فحصٍ البتّة**، وهو **حارسٌ يُلتفّ عليه بمجلَّد**. فصار
المسحُ بيتين، **ولكلِّ صنفٍ بيتُه**:

    landing/downloads  ← العامّةُ وحدَها  (كما كان)
    landing/private    ← الخاصّةُ وحدَها  (ويُرفض فيها العامُّ والتجريبيّ)

**والقاعدةُ صارت «لكلِّ صنفٍ بيتُه» لا «الخاصّةُ ممنوعةٌ هنا»** — والثانيةُ
تُرضى بالنقل، **والأولى تُقاس في الاتجاهين**.

### ⚠ و«لا أداةَ» ليست «هدفٌ خاطئ»

أوّلُ نسخةٍ من فحص البيت قرأت تعذُّرَ قراءة الهدف **اتّهاماً** («غلافُه لا
يعرفه الجدول») — **وهو بلاغٌ كاذبٌ عن حزمةٍ سليمة**: `shellTarget` يحتاج أداةَ
SDK **وWSL بلا SDK**. فصار يقول **«لم يُقس»**، **ولا يُقرأ سكوتُه سلامة** —
والفحصُ كلُّه يبقى أحمرَ على جهازٍ بلا أداة، وهو ما تشترطه القاعدة.

### ٣ · الحزمة — **مقيسةٌ من داخلها لا من سجلِّ البناء**

    ly.tajora.admin · versionCode 478 · versionName 1.0
    server.url  https://admin.tajora.ly     ← من `assets/capacitor.config.json`
    التوقيع     3bdc7391…5ce19e             ← يطابق مفتاح الإصدار
    الحجم       4,578,335 بايت
    البصمة      be1f7d96…079ec7             ← تطابق ما على الخادم بايتاً

**والرابطُ مقيسٌ بالتنزيل**: `HTTP 200`، والبصمةُ نفسُها، **ولا ذكرَ لها في
الصفحة ولا في البيان**.

### ⚠ وحافّةٌ تخالف رأسَ الأصل

`.apk` يصل بـ`max-age=14400, must-revalidate` **و`nginx.conf` يقول
`no-cache`** — فرأسُ الأصل **لا ينفذ**. **و`cf-cache-status: REVALIDATED`
يعني أن الحافّةَ تسأل الأصلَ قبل أن تخدم، فالصحّةُ محفوظة** — لكنّ الفرقَ
قائمٌ بين ما يَعِد به الملفُّ وما يصل. **ورأسُ الأصل نفسُه لم يُقس** (صورةُ
nginx بلا `curl` ولا `wget`)، **ولا يُقرأ سكوتُه ضماناً**.
<!--/جديد-->

<!--جديد-->
## نسبةُ العمولة على مصدرٍ واحد، والعرضُ يحملها — **البند ٥٥** (٢٠٢٦-٠٩-٠٦)

### الجرد — ٣٠٠ مرشَّحاً، والحقيقيُّ منها سبعة

**والضجيجُ نُحّي بالقياس**: `50%` في تدرّجٍ أو `keyframes` ليست نسبةَ عمولة،
**وجردٌ يغرق في CSS يُقرأ ضجيجاً فيُطفأ حارسُه**.

| الموضع | ما كان يقوله | الحكم |
|---|---|---|
| `index.html` عدّادٌ كبير | «٢» — **من `commission_settings`** | ✅ كان موصولاً |
| تحته مباشرةً | «صفر عمولة على الرحلات بالاشتراك» | ❌ مخبوز |
| بطاقةُ الشبكة | «عمولة 0% بالاشتراك» | ❌ مخبوز |
| عنوانُ الميزة | «صفر عمولة للكبتن» | ❌ مخبوز |
| الوصفُ و`og` و`twitter` | «صفر عمولة على الكبتن المشترِك» | ❌ **ثلاثاً** |
| سطرُ البطل | «بلا عمولة على الكبتن المشترِك» | ⚠ من الإعدادات **ونصُّه الافتراضيُّ مطلق** |
| `Collect` · `RideDetails` | `"0٪ حالياً"` | ✅ **محسوبٌ لا مخبوز** — استُثني بعلّته |

**والتطبيقُ الراكب: صفرُ موضع. واللوحة: عنوانٌ بلا رقم.**

### ⚠ وأخطرُ من التناقض: **وعدٌ انفصل عن الشيفرة ولا شيءَ يصيح**

البطاقتان قالتا «الأجرة له **كاملة**» — **وهي صادقةٌ فقط والنسبةُ صفر**.
ولمّا أُشعلت العمولةُ صار `frozen_commission` **نسبةَ السوق** لا صفراً
(`subscriptions.py`)، **فصار المشترِكُ يدفع ١٫٥٪ والصفحةُ تَعِده بلا شيء**.

**والصوابُ ما يبقى صادقاً عند أيِّ نسبة**: «نسبتُك تُجمَّد لحظةَ الشراء فلا
تتغيّر عليك ما دام اشتراكك سارياً» — **وهو الوعدُ الذي تفي به الشيفرة**.

### والحارس — **ثلاثةُ عطوبٍ فيه هو، كشفها القياس**

1. **قرأ صفرَ ملفٍّ وخرج أخضر** في أوّل تشغيل (جذرٌ من `pathname` يكسر على
   مسارٍ شبكيّ). **وصفرٌ مقروءٌ عطبٌ لا سلامة** — فصار يسقط عليه.
2. **واتّهم البقشيشَ وتعليقاتِ JSX**: «بلا عمولةٍ عليه» **حقيقةٌ بنيويةٌ
   تبقى صادقةً عند أيِّ نسبة**، وتعليقٌ يشرح القاعدةَ ليس نصّاً معروضاً.
3. **وشرطُ جملةٍ أخرى أعبر وعداً مطلقاً**: زُرع «صفر عمولة للكبتن.» فوق سطرٍ
   فيه «ما دام…» **فمرّ** — **ونافذةٌ سخيّةٌ تُعبِر كلَّ وعدٍ بجواره سطرٌ
   مقيَّد**. فصار الشرطُ يُقرأ **في الجملة لا في الجوار**.

**ومقيسٌ في الاتجاهين على ما شُحن فعلاً**: ثلاثةُ عطوبٍ مزروعةٍ أُمسكت،
**و٣١١ ملفّاً خضراء**، وثلاثةُ استثناءاتٍ **كلٌّ بعلّته نصّاً**.

### ⚠ وحارسُ الصفحة نفسُه كان يبتر ما يقرأ

`[a-z_]+` يبتر `driver_keeps_per_100` إلى `driver_keeps_per_` **ثمّ يشكو أن
لا موضعَ له** — **بلاغٌ كاذبٌ سببُه النمطُ لا الصفحة**، في الطرفين معاً.

### والعرضُ يحمل نسبته — **عمودٌ وسطر**

`subscription_offers.commission_percent`: **`NULL` = لا يمسّ**، **وصفرٌ =
يمنح صفراً**. ويُجمَّد في **البابِ الواحد** (`subscriptions.py`) الذي يكتب
صفَّ الاشتراك في القنوات الأربع — **وشرطٌ في راوترٍ يُنسى في ثلاثة**.

**وأثرُ الإلغاء مقيسٌ** (نسبةُ السوق ٥٪، في معاملةٍ أُلغيت):

    قبل الاشتراك   None ⇒ ٥٫٠٠٪      بعد شراءٍ بصفر  0.00 ⇒ ٠٫٠٠٪
    بعد الإلغاء    None ⇒ ٥٫٠٠٪  ← **تعود فوراً، والرحلاتُ الماضيةُ محتفظةٌ بنسبتها**

### والثغرةُ مغلقةٌ بالبناء — **ولا يُبنى عليها قيدٌ ثانٍ**

| ما يمنع الشراءَ ثانيةً بعد الإلغاء | الموضع |
|---|---|
| الإلغاءُ يَسِم ولا يحذف | `subscriptions.py:1148` |
| والعدُّ يشمل الملغى | `offers.py:92` |
| و`new_driver` تسقط بصفٍّ ملغى | `offers.py:131` |

**والردُّ من المدفوع فعلاً لا من سعر القائمة** — فعرضٌ مجّانيٌّ `amount_paid=0`
**ردُّه صفرٌ ولا قيدَ يُكتب**، وهو ما رجّح (أ) على الخصم الجزئيّ.

### والوثائقُ الأربع — **لا رقمَ فيها، فلا تُلمس**

شروطُ الكبتن §٤: «بالنسبة المعلنة». **ولا نسخةَ ثانيةً، ولا مراجعةَ تنتظر.**
<!--/جديد-->

<!--جديد-->
## صفُّ العرض الأول، وعطبان كشفهما قياسُ الباب لا قراءةُ الشيفرة (2026-09-06)

### الصفّ — **مطفأٌ بقصد، والإطلاقُ بيد المالك**

| الحقل | القيمة |
|---|---|
| الاسم | شهرك الأول علينا |
| السوق · الخطة | JO · اشتراك شهري (30.000 JOD) |
| نوعُ الخصم · قيمتُه | `money_percent` · 100.000 ⇒ **مجّاناً** |
| **نسبةُ العمولة** | **0.00** ⇒ **يمنح صفرَ عمولة** (لا `NULL`) |
| الجمهور · المرّات | `new_driver` · 1 |
| النافذة · سقفُ التنازل | بلا نافذة · بلا سقف |
| **`is_active`** | **`False`** |

**و`is_active=False` تُكتب صراحةً**: الافتراضُ في الموديل `True`، **فالسكوتُ
عنه يُطلق العرض** — والفرقُ بين «أُنشئ» و«صار يعمل» هو كلُّ ما طلبه المالك.

**والصفُّ على التطوير وحدَه.** الإنتاجُ `offer: null` من بابه العامّ، **وعمودُ
`commission_percent` لم يصله بعدُ** (ترحيلةُ 0073 تنتظر بوّابةَ المال).

> **⚠ وأداةُ العرض نفسُها كانت تكذب**: أوّلُ نسخةٍ طبعت وصفاً ثابتاً بجانب
> القيمة — «صفرٌ لا NULL» بجانب `None`، و«مطفأ» بجانب `is_active=True`.
> **وهو «الحقلُ المحشوُّ يُرى عاملاً» في أداةِ قياس**، وأخطرُ من موضعه في
> شاشة: **الشاشةُ تُراجَع، وتقريرُ الأداة يُصدَّق**. فصار كلُّ وصفٍ **مشتقّاً
> من قيمته** لا مكتوباً بجانبها.

### والعطبان — **كلاهما في ما بُني أمس، وكلاهما ظهر من الباب الحيِّ وحدَه**

| العطب | ما كان يُعرض | الحارسُ الذي مرّ عليه |
|---|---|---|
| `driver_keeps_per_100` = `"1E+2"` | «يبقى لك **1E+2**» | اختبارُ قائمة السماح — **يسأل «أخرج الحقل؟» لا «ماذا فيه؟»** |
| `hero_subtitle` في القاعدة | «**بلا عمولة** على الكبتن المشترِك» | `check:commission-text` — **يمسح المصدر، والمصدرُ صحيح** |

**والثاني أثقلُ درساً**: **صُحّح المصدرُ ولم يبلغ الصفّ**، لأن `DEFAULTS`
«تُكتب مرّةً عند أوّل قراءة». **فالمصدرُ الأخضرُ والصفحةُ الكاذبةُ عاشا معاً**،
ولا حارسَ ساكنٌ يرى الفرق. **وسدَّته ترحيلةُ بيانات (`0074`) لا توسيعُ حارس** —
تكتب فوق النصِّ القديم **بحرفه** وحدَه، فيبقى أيُّ تحريرٍ من اللوحة.

**ومقيسٌ حيّاً بعد العلاج** (`/public/site?country_code=JO`):

    commission_percent   = 0.00
    driver_keeps_per_100 = 100          ← لا أُسّ
    hero_subtitle        = «… وعمولةٌ منخفضةٌ تُجمَّد لحظةَ اشتراكك.»

### ⚠ وبندٌ سجّلتُه ولا وجودَ له — **قياسٌ مكسورٌ ولد خبراً كاذباً** (صُحّح 2026-09-06)

**كُتب هنا أن `landing/index.html` «متجاهَلٌ ومتتبَّعٌ معاً»** وأنه بقيّةُ ما
قبل قرار ٢٠٢٦-٠٩-٠٥. **وقِيس فسقط**:

    git ls-files --error-unmatch landing/index.html   →  **غيرُ متتبَّع**
    git cat-file -e HEAD:landing/index.html           →  **ليس في الإيداع**
    git ls-files landing/                             →  nginx.conf · spa.conf **فقط**

**والاثنان المتتبَّعان إعدادُ نجينكس لا مخرَجُ بناء** — فالقرارُ مُطبَّقٌ
تطبيقاً كاملاً، **ولا بندَ هنا أصلاً**.

**ومصدرُ الكذب أداةُ القياس**: كُتب
`echo "tracked=$(git ls-files … | wc -l)"` داخل
`wsl.exe -- bash -lc '…'` — **وتلك الطبقةُ تمضغ `$( )` كما تمضغ `$?`**،
فطبع `tracked=1` بلا أن يعدّ شيئاً. **وهو الفخُّ نفسُه الذي كذب في قياس
`check:ci-timeouts`** في اليوم نفسِه.

**والدرسُ يبقى وإن سقط خبرُه** (قاعدة «قبل أن تحذف نصّاً كذب: أخبرٌ هو أم
درس؟»): **كلُّ رقمٍ يخرج من صدفةٍ متداخلةٍ يُعاد بأمرٍ يخرج بنفسه** —
`git ls-files --error-unmatch` يجيب برمز خروجه لا بعددٍ يُنقل عبر طبقتين.
**وثلاثةُ أرقامٍ كذبت عليّ في يومٍ واحدٍ من هذه الطبقة بعينها.**

### وحارسان كانا خارج باب الحرّاس

`check:site` و`check:commission-text` **لم يكونا في `scripts/guards.sh`** —
يعملان في بناء `site` وحدَه. **والسطرُ الختاميُّ كان يقول «كلُّ الحرّاس
الساكنين خضر»** فوق كنسٍ لا يشملهما. أُدخلا، **والحلقةُ ثانيةٌ لا شرطٌ في
الأولى** لأن أسماءَ حرّاس الموقع غيرُ أسماء حرّاس التطبيقات.
<!--/جديد-->

<!--جديد-->
## الرفعةُ على `2c534b5c` — **الستُّ خضراء، وبندٌ سقط افتراضُه** (2026-09-06)

    الإنتاج   27d665da ← 2c534b5c   ·  الترحيلة 0072 ← 0074
    النسخة    /mnt/d/taxo-backups/20260906T121809Z  (16M، محقَّقةٌ بالفتح)
    CI        https://github.com/SBONGEEL/taxo/actions/runs/34028793036
    المجموعة  1401 خضراء · صفرُ أحمر · 38د 35ث محلياً · 19د 13ث في CI

### ما انقلب — **مقيسٌ قبل وبعد، لا موصوف**

| المقياس | قبل | بعد |
|---|---|---|
| وعودٌ مطلقةٌ في `taxo.tajora.ly` | **8** | **0** |
| `driver_keeps_per_100` | غائب | **98** (بلا أُسّ) |
| `hero_subtitle` | «— بلا عمولة على الكبتن المشترِك.» | «— وعمولةٌ منخفضةٌ تُجمَّد لحظةَ اشتراكك.» |
| «والسوق يبدأ من» · «تُجمَّد لحظةَ» | غائبان | موجودان |
| `/public/landing` | 200 | 200 (وكان **500** في نافذة التعليق) |

**والثمانيةُ كانت سبعةً في أوّل عدّ** — فاتني «الأجرة له كاملة»، **وهي
أخبثُها**: بلا رقمٍ فلا تلتقطها العين، **وهي الوعدُ الذي انفصل عن الشيفرة**
أصلاً (المشترِكُ يدفع ٢٪ والصفحةُ تَعِده بالأجرة كاملة).

### ⚠ والبندُ الذي سقط افتراضُه — **v2 ليست على الإنتاج**

**قِيس**: الأربعُ ما زالت `v1` وفيها «مسوّدة» (راكب ٢٬٤١٤ و١٬٤٢٩ حرفاً ·
كبتن ١٬٦٤٩ و١٬٤٦٨).

**والعلّةُ بنيوية**: **الرفعةُ تحمل الشيفرةَ لا الصفوف**، **ووثائقُ السياسات
صفوفٌ في القاعدة** (§34). فكتابةُ v2 وقعت على **قاعدة التطوير** وحدَها،
و`seed_policies.py` **idempotent بالثلاثية** فلا يكتب حيث يوجد صفّ — **فهو
لا يصنع v2 أبداً، بقصدِ ألّا يدفن مراجعةَ إنسان**.

**وبابُها اللوحة**: `POST /admin/policies` ثمّ `/{id}/publish` —
**والنشرُ فعلُ إنسانٍ بنصِّ §39٫١٠**. **والنصُّ المصحَّحُ مرفوعٌ في الشيفرة**
(صفرُ «لسنا ناقلاً» · Mapbox وWhatsApp وFirebase بأسمائهم · أربعةُ مطالعَ
«تصف هذه الوثيقةُ») — **ينقصه أن يُكتب صفّاً على الإنتاج**.

**والدرسُ**: «الرفعةُ تحمل v2» **كانت دعوى لا قياساً** — وقد قيلت في تقريرٍ
وخُطِّط عليها. **وما يعيش في القاعدة لا ينتقل بالرفع**، وهو يشمل الوثائقَ
والإعدادات وصفوفَ العروض. **وتمييزُ «شيفرةٍ تُرفع» من «صفٍّ يُكتب» يُسأل عنه
قبل التخطيط لا بعده.**
<!--/جديد-->

<!--جديد-->
## صفحةُ التحميل بعد دخول المتجر — **الاسمُ واحدٌ والتوقيعان مختلفان** (قرارُ المالك 2026-09-07)

**Play يعيد توقيعَ الحزمة بمفتاحه** (Play App Signing، وهو إلزاميٌّ لكلِّ
تطبيقٍ جديد). **وحزمُ `taxo.tajora.ly` موقَّعةٌ بمفتاحنا.** فالمعرّفُ واحدٌ
(`ly.tajora.rider`) **والتوقيعان مختلفان** — **وأندرويد لا يحدّث حزمةً بتوقيعٍ
آخر**.

**فمن ثبّت الراكبَ من صفحتنا لا يستطيع تحديثَه من المتجر ولا العكس** — يلزمه
**حذفُ التطبيق أوّلاً**، وبه تذهب حالُه المحلّيّة.

**والقرار**: تبقى حزمةُ الراكب على الصفحة **حتى يخرج التطبيقُ إلى الإنتاج في
المتجر فعلاً**، ثمّ تُستبدل بزرٍّ يقود إلى المتجر. **ومتى بُدّلت يُكتب في
الصفحة أن من ثبّتها من هنا يحذفها قبل التثبيت من المتجر.**

**ولمَ لا يُبدَّل الآن**: الاختبارُ الداخليُّ لا يبلغ إلا من دُعي، **وزرٌّ
يقود إلى متجرٍ لا يُظهر التطبيقَ لأحدٍ يترك الناسَ بلا شيء**.

### وقسمةُ الأسرار — **ما نُفِّذ وما عُرض**

**نُفِّذ**: `.env.local` 600 والمجلَّد 700 · التجاهلُ والخطّافُ **مقيسان
بالنقض** · `.env.example` كمُل بثلاثة أسماءٍ بلا قيَم · حارسُ `check:env-leak`
في الباب · **وفهرسُ الأسرار خارج الشجرة** (`~/.taxo-secrets/SECRETS-INDEX.md`،
600 في مجلَّد 700) **مقيسٌ خالياً من كلِّ قيمة**.

**وأُمسك تسريبٌ حقيقيّ**: كلمةُ مشرف التطوير كانت حرفاً في
`backend/scenario_demo.py` — صارت تُقرأ من البيئة. **والقيمةُ في تاريخ git**،
فالعلاجُ الكاملُ تدويرُها وهو بيد المالك.

**وعُرض ولم يُنقل** (الصنفُ الثاني في `.env`): `BOOTSTRAP_ADMIN_PASSWORD` ·
`MAPBOX_PUBLIC_TOKEN` · `MAPBOX_SECRET_TOKEN` · `TELR_AUTH_KEY` ·
`TELR_STORE_ID` — **قارئُها الوحيدُ `backend/scripts/seed.py`**، سكربتٌ يشغّله
إنسان.

**⚠ وأخطرُ سطرٍ في الجرد**: **صفرُ نسخةٍ قابلةٍ للاستعادة خارج الجهاز** —
ثلاثُ نسخٍ من مفتاح التوقيع ونسختان من كلمته، **والخمسُ على هذا الجهاز**.
وأسرارُ GitHub **تُكتب ولا تُقرأ** فلا تصلح استعادة. **وضياعُ الجهاز =
التطبيقُ لا يُحدَّث أبداً.** **والنسخةُ الخارجيةُ بيد المالك.**
<!--/جديد-->

<!--جديد-->
## جولةُ اللوحة بالعين — **ما لم يمسكه حارسٌ ولا اختبار** (2026-09-07)

**الشاشاتُ التسعُ فُتحت بحسابٍ حيّ**، ولم يُقَل «المكوّنُ نفسُه» عن شاشةٍ لم
تُفتح (شرطُ المالك).

| الشاشة | ما قِيس |
|---|---|
| الرحلات | **٤٠ زرّ «الملفّ»** — والضغطُ ينقل إلى `/riders?open=…` والدرجُ يُفتح كاملاً |
| الركّاب | زرٌّ في كلِّ صفّ · درجُ الملفّ بخمسة أقسام · الرصيدُ من الدفتر |
| الدفعات | زرّان لكلِّ صفّ · **الحالُ «مؤكَّدة» بالاسم لا فارغاً** · القناة «كاش»/«محفظة» |
| المالية | خمسةُ ألسنة · زرٌّ في عمود الكبتن · «موافَقٌ عليه» و«مدفوع» |
| **شحناتُ المحفظة** | **المرشِّحُ بأربع**: بانتظار التأكيد · مؤكَّدة · مرفوضة · الكل — **والضغطُ على «مؤكَّدة» أعطى ثلاثةَ صفوفٍ كلُّها «مؤكَّدة»** |
| النزاعات | **فارغةٌ بحقّ** — «لا نزاعات مفتوحة» بعنوانٍ وسبب. **وأزرارُ الملفّ فيها لم تُقَس: لا صفَّ يحملها** |
| مشترياتُ المركبات | أربعُ بطاقات (١٠ ملكيّات · ٣١٫٥٠٠ د.أ · ٣ هدايا · ٠ منح) · زرُّ الملفّ والرقم |
| سلَفُ الكباتن | خمسةُ صفوفٍ بزرِّ الملفّ · **«شُطبت»/«سُدِّدت» بالاسم** |
| الكباتن | خمسةُ مرشِّحات · الحالُ والجنس بأسمائها |

### ⚠ وعطبٌ جديدٌ لم يكن مسجَّلاً — **معرّفٌ حيث يجب أن يكون إنسان**

**«طلبات إلغاء التفعيل»** (تحت قائمة الكباتن) ترسم الكبتنَ **`cd4a03ee`** —
`driver_id.slice(0, 8)` بخطٍّ أُحاديّ. **ولا زرَّ ملفٍّ فيها.**

**والسببُ في العقد لا في الرسم**: `DeactivationRequestOut` ينشر `driver_id`
وحدَه — **لا اسمَ ولا رقماً** — **فاللوحةُ لا تملك ما ترسمه**.

**ولم يُصلَح في جولته**: إصلاحُه **تغييرُ عقدٍ في الخلفية** (حقلان يُضافان
ويُقاسان)، **وجولةُ قياسٍ لا تُوسَّع إلى تغيير عقد**. **بندٌ مفتوحٌ بعلّته.**

### وبندُ شاشة الدخول — **بادئةٌ ثابتةٌ بجانب حقلٍ يقبل اسماً**

**رآه المالكُ بعينه**: `+962` معروضةٌ ثابتةً، **والحقلُ يقبل اسمَ مستخدمٍ أو
رقماً** («اسم المستخدم أو رقم الهاتف»). **فمن يكتب `claude.audit` يرى بادئةَ
دولةٍ بجانبه.**

**والمطلوب**: تختفي البادئةُ متى لم يكن المُدخَلُ أرقاماً. **بندٌ مفتوح.**
<!--/جديد-->

<!--جديد-->
## الشاشتان الرئيسيتان — بلاطاتٌ متقابلةٌ وصندوقٌ بمصدرين (٢٠٢٦-٠٩-٠٧)

### ما كان قائماً قبل اليوم — **مقروءاً من القاعدة لا من الشيفرة**

**١٢ بلاطةً في الأردن**، ستٌّ لكلِّ طرف، **ولا نصَّ منها مخبوزٌ في تطبيق**:
`storefront.tiles` صفوفٌ تُدار من اللوحة منذ 08-30. و«قريباً» بلا `onClick`
أصلاً — **تُقرأ ولا تُنقر**، ولا زرَّ معطَّلاً يبتلع اللمسة. و`is_new` محسوبةٌ
في الخلفية بمدّتها.

**و٤ لافتات**، اثنتان انقضت نافذتُهما، وثالثةٌ مطفأة، **ورابعةٌ حيّةٌ للركّاب**
بصورةٍ مرفوعةٍ ووجهةِ `/account/permissions` **ونصٍّ فارغ**.

**والشبكةُ `grid-cols-3` بلا سقفِ ستٍّ** — قِيس، فالسادسةُ ليست حدّاً.

### والبلاطاتُ صارت ١٦ — **أربعُ مقابلاتٍ بُذرت** (٩ للكبتن · ٧ للراكب)

| الخدمة | الكبتن | الراكب |
|---|---|---|
| توصيل طرود | `d_parcels` قريباً | `parcels` قريباً |
| رحلات مجدولة | `d_scheduled` قريباً | `scheduled` **فعّالة** → `/account/bookings` |
| المطار | **`d_airport` قريباً — جديدة** | `airport` قريباً |
| الطلبات | **`d_orders` قريباً — جديدة** | `orders` قريباً |
| الأغراض/الشحنات | **`d_shipments` قريباً — جديدة** | `my_items` قريباً |
| التحديات | `missions` **فعّالة** → `/account/missions` | **`r_missions` قريباً — جديدة** |
| أخرى | `more_driver` **فعّالة** | `more_rider` **فعّالة** |

**والمقابلُ يحمل أيقونةَ أخيه نفسَها** — فالأيقونةُ هي ما يربط الشاشتين في
العين، لا العنوان.

**واثنتان تخصّان الكبتنَ بحقّ، وعلّتُهما مكتوبةٌ في `seed_service_tiles`**:
**`garage`** — الراكبُ لا مركبةَ له تُزيَّن؛ و**`advance`** — دَينٌ يُسدَّد
باقتطاعٍ من دخلٍ يمرّ بالمنصّة، **والراكبُ يشحن ويحوّل ولا يسحب** (§7)، فسلفةٌ
له قرضٌ بلا مصدرِ سداد.

**وبُذرت صفوفاً في `backend/scripts/seed.py` لا شيفرةً في التطبيقات** — فتصل
الإنتاجَ بالبذر، **ويُعيد المالكُ ترتيبَها أو يُخفيها من اللوحة بلا نشر**.

### وصندوقُ الإعلانات صار **بمصدرين لا بكيانين**

`GET /storefront` صار يحمل **`offer`** بجانب `banners`: صفوفُ `promo_banners`
كما هي، **وعرضُ اشتراكٍ محسوبٌ لهذا الكبتن بعينه** عبر `offers.resolve` —
**البيتُ نفسُه الذي يمرّ به الشراءُ وشاشةُ `/subscription`**. **فما يُعرض في
الصندوق هو ما يُخصم عند الضغط**، ولا حسابَ ثانياً يفترق أوّلَ تعديل.

**والعرضُ يصل الصندوقَ بلا تعديل شيفرة**: ما يُشعله المالكُ من اللوحة يظهر —
**ولا يُصنع له صفُّ لافتةٍ آليّاً**، فجدولٌ نصفُه بيدٍ ونصفُه بآلةٍ يراه المشرفُ
فيحرّره **فيُمحى في الدورة التالية**.

**ولا يُعرض ما لا يُنال**: الجمهورُ والحدُّ والميزانيةُ والنافذةُ كلُّها تُقاس
قبل البطاقة — **ومن استنفد حدَّه لا يرى شيئاً**، لأن عرضاً يُرى ولا يُطبَّق عند
الضغط أسوأُ من عرضٍ لا يُرى.

**و«مجاناً» كلمةٌ لا رقمُ صفر**: «0.000 د.أ» تُقرأ عطباً في السعر لا هديّة.

### وحين يفرغ الصندوق — **يختفي كلُّه**، والعلّةُ مكتوبة

**لا فراغٌ ولا هيكلٌ أبديّ**: المكانُ الفارغُ يُقرأ عطباً في التحميل، والهيكلُ
الذي لا ينتهي يُقرأ شبكةً منقطعة. **ولا سطرَ يُكتب مكانَه** — «لا إعلانَ
اليوم» ليست خبراً يستحقّ مساحةً في أوّل شاشةٍ تُفتح. **والبلاطاتُ تصعد مكانَه**،
وهي ما جاء المستخدمُ لأجله.

**والحالُ الابتدائيةُ هي الحالُ الفارغةُ نفسُها** (`storefront?.banners ?? []`)
— فلا وميضَ بين إطارٍ فارغٍ ومملوء.

### وما قِيس، وما **لم** يُقَس

**قِيس**: الحمولةُ عبر الباب لكبتنٍ وراكبٍ حقيقيَّين على التطوير — ٩ بلاطاتٍ
و٧، وبطاقةُ «عرض الكباتن الجدد» **22.500 بدل 30.000 JOD** (٢٥٪ على الشهريّ)،
والراكبُ `offer = null`. **وستُّ اختباراتٍ في `test_storefront.py`**، **مقيسةٌ
بالنقض**: بطاقةٌ تقرأ «الفعّالة» بدل «المستحقّة» أسقطت اثنين — المفتاحَ
المطفأ وجمهورَ `manual`.

**ولم يُقَس على الشاشة بعد**: فتحُ تطبيقَي التطوير يحتاج جلسةً بيد المالك،
**ولا تُكتب كلمةُ مرورٍ في حقلٍ من هنا**. فيبقى «مقيسٌ عبر الباب» لا «مقيسٌ
على الشاشة» — **ويُقال ولا يُسكت عنه**.
<!--/جديد-->


<!--جديد-->
## نشرةُ ٢٠٢٦-٠٩-٠٩ — `2c459b4 → df6d5ee`، والبوّاباتُ الستُّ مقيسة

| البوّابة | القياس |
|---|---|
| ١ الإعلان | `2c459b46 ← df6d5ee5` · **١٠ التزامات** · ترحيلةٌ واحدة · **«لا ملفَّ مالٍ في ما يُرسَل»** |
| ٢ الدفعُ وCI | مسحُ السرّ نظيف · `2c459b4..df6d5ee` مدفوع · **CI أخضر** — `runs/34246153894` |
| ٣ النسخة | `/mnt/d/taxo-backups/20260908T164242Z` |
| ٤ التحقّق | **الأربعةُ فُتحت**: القاعدة 104,619 · الأسرار 781 · ما خارج الشجرة 1,308 · الوثائق 16,100,316 بايت |
| ٥ الرفع | `moved=0 patch=21` · بناءُ الأربع على الخادم · **والترحيلةُ `0074 → 0075`** |
| ٦ التحقّق | الإيداع `df6d5ee` · الرأس `0075 (head)` · **العمودُ `user_id` حاضرٌ و`driver_id` زال** · ١١ حاويةً تعمل · **الروابطُ الستّةُ 200** |

### والبوّابةُ الخامسةُ انقطعت مرّتين — ولم تسقط

`WSL … accept4 failed 110` قطع القناةَ في المحاولتين. **والبابُ يميّز**:
«انقطع السحبُ ولم يصل جوابُه — **يُقاس الخادمُ قبل أيِّ إعادة**». فقِيس في
الأولى: الخادمُ **لم يتحرّك** (`2c459b4`) والحاوياتُ تعمل — فأُعيد بلا ضرر.
**وهي قاعدةُ «بوّابةٌ تسقط بنصّ لا بانقطاع قناة» مطبَّقةً.**

### وعطبٌ في الباب أوقفه عشر دقائق: **مسارٌ لينكسيٌّ لثنائيٍّ ويندوزيّ**

`TAXO_SSH_KEY` الافتراضيُّ `$HOME/.ssh/taxo-contabo` — **و`ssh.exe` ويندوزيٌّ
لا يقرأ مسارَ WSL**. فيطبع «Identity file … not accessible»، **ثمّ يسقط إلى
مفتاحٍ آخرَ ويعلّق بلا مهلة** رغم `ConnectTimeout=20`.

**والعلاجُ متغيّرٌ لا تعديلُ باب**: `TAXO_SSH_KEY='C:\Users\loly3\.ssh\taxo-contabo'`
— **والباب يقرؤه أصلاً**. وقِيس أن الاتصالَ يعود **في ثانية** بالمسار الويندوزيّ.

### وسطران عن الخادم يُقالان ولا يُسكت عنهما

**١) `cloudflared/config.prod.yml` مُعدَّلٌ على الخادم بـ١٢ سطراً** تخصّ
موقعاً آخر (`araqa.tajora.ly`). **وتعليقُها يقول بنفسه** إن نشرةَ TAXO تمسحها
و`araqa-ingress-guard.timer` يعيدها — **فترتيبٌ ذاتيُّ الإصلاح**، وحُفظت مع
ذلك في `~/taxo-aside-20260908T164242Z` (`patch=21`).

**٢) وCloudflare يُعمّي عناوين البريد على الحافة**: `mailto:privacy@tajora.ly`
يصل المتصفّحَ `<a href="/cdn-cgi/l/email-protection#…">` و`__cf_email__`.
**فالمصدرُ سليمٌ والتعميةُ تقع بعد الخدمة** — ومن قاس بجلبٍ خامٍ ظنَّ العنوانَ
غائباً. **وهو الشكلُ العاشرُ من جهةٍ ثالثة**: لا البناءُ ولا الخادم، بل
الحافّة.

**⚠ وخطأٌ في قياسي وقع هنا**: قِيست الصفحةُ أوّلاً بـ`Invoke-WebRequest`
فبدت **كلُّ عباراتها غائبة** — **والعلّةُ فكُّ ترميزٍ لاتينيٍّ لنصٍّ UTF-8**،
لا الصفحة. **ووحدةُ القياس جزءٌ من الرقم**، وقد كُتبت في `CLAUDE.md` عن
`wc -m`، ووقعت هنا في ثوبٍ ثانٍ.
<!--/جديد-->


<!--جديد-->
## عطبُ التسجيل ٢٠٢٦-٠٩-٠٩ — **نفقٌ ميّتٌ منذ ٤٨ ساعة، ورسالةٌ تسمّي غيرَ سببها**

**البلاغ**: «عند مرحلة التحقّق من الرقم يقول التطبيق الخادمُ متوقّف.»

### ما قِيس، بالترتيب الذي نفى الفرضياتِ به

| القياس | الرقم |
|---|---|
| نداءاتُ `POST /auth/challenge` في سجلّ الخلفية **كلِّه** | **٩** — من ٢٤,٥٤٠ سطراً |
| كم منها للمالك | **صفر** — التسعةُ كلُّها من أدوات القياس |
| محاولاتُ الإرسال في سجلّ البوّابة **كلِّه** | صفرٌ قبل ٠٠:٠٤:٥٢، **وكلُّ ما بعده قياس** |
| حاويةُ `taxo-tunnel` | **`exited(255)` منذ `2026-09-07T00:39:47Z`** — نحو ٤٨ ساعة |
| نطاقاتُ القناة التجريبية الأربعة | **530 · `error code: 1033`** — «النفقُ غيرُ متّصل» بنصِّ Cloudflare |
| نطاقاتُ القناة العامّة الأربعة | **200 / 404 صحيحة** — `api.tajora.ly/health` = **200** |

**فالعلّةُ خارج الشجرة**: كلُّ ما يخاطب القناةَ التجريبية — الهاتفُ والحزمةُ
والغلاف — كان مقطوعاً، **والخلفيةُ والقاعدةُ والبوّابةُ تعمل طوال الوقت**.

### والعدُّ نفى أسرعَ من قراءةِ أيِّ شيفرة

**السؤالُ الأول لم يكن «أيُّ سطرٍ يفشل» بل «أوصل الطلبُ أصلاً؟».** وجوابُه
عدٌّ في سجلّ: **تسعةٌ، وكلُّها لي**. وهذا **ينفي الخلفيةَ كلَّها وقاعدةَ
البيانات والبوّابةَ وقوالبَ الرسائل والمفاتيحَ في خطوةٍ واحدة** — قبل فتح
ملفٍّ واحدٍ من الشيفرة.

**ولو بُدئ بالشيفرة لَقُرئت رسائلُ الخطأ الأربعُ في `client.ts` ولَوُجدت
صحيحةً كلُّها**، ثمّ لَبُحث عن عطبٍ في `PhoneVerification.tsx` وهو سليم —
ساعاتٌ في مكانٍ لا عطبَ فيه. **فغيابُ الطلب من السجلّ خبرٌ أقوى من أيِّ
قراءةِ كود.**

### والرسالةُ التي رآها المالك **كانت صادقة**

`networkError()` تقول «تعذّر الوصول إلى الخادم» — **والخادمُ فعلاً لم يكن
يُوصَل إليه**. فلا سترَ هنا ولا تسميةَ بغير السبب: **الشكلُ السابعَ عشر لم
يقع في هذا الموضع**، والرسالةُ أدّت ما بُنيت له.

### ⚠ لكنّه وقع في موضعٍ آخرَ كشفه القياس — **وأُصلح**

سؤالُ الوجود (`onWhatsApp`) يعود بقائمةٍ فارغةٍ لرقمٍ لا يعرفه واتساب،
**فكانت البوّابةُ تسمّيه «القناةُ لا تُجيب» والخلفيةُ `channel_down`**.

**والقياسُ يكذّبها من السجلّ نفسِه**: `الجلسةُ مرتبطة` · ورسالةٌ إلى `…0772`
**خرجت وأُقرّت وسُلِّمت (`CB:receipt` تسليم) في الثانية نفسِها** التي رُفض
فيها `…0773` بدعوى سقوط القناة.

**فصارت الحالاتُ ثلاثاً باسمها** (`WhatsAppNumberUnanswered` ← `reason =
number_unanswered`)، ونصُّها: **«لم يُجب واتساب عن حالة هذا الرقم — تأكّد من
الرقم أو جرّب رقماً آخر عليه واتساب.»** — **ولا ذكرَ لقناةٍ ساقطة.**

**والوراثةُ من `WhatsAppError` بقصد**: كلُّ ما كان يرتدّ يبقى يرتدّ،
**فالفرقُ في ما يُقال لا في ما يُفعل** — ولا مسارَ يتغيّر.

**ومقيسٌ بالنقض**: بحذف الالتقاط في `verification.py` **يسقط اختباران**
(`2 failed, 11 passed`) ويعود `reason` إلى `channel_down`.

### ما أُعيد وما يبقى بنداً

- **`taxo-tunnel` أُعيد تشغيله** (تشغيلٌ لا إنشاء): أربعُ وصلاتٍ مسجَّلة،
  و`dev-api.tajora.ly/health` = **200**، و`dev-app.tajora.ly` = **200**.
- **⚠ بندٌ لم يُبنَ بقصد — حارسُ «نفقٌ حلَّ اسمُه ولم يتّصل»**: `check:hosts`
  **يقيس وجودَ السجلّ لا جوابَ الخادم**، **وهو يقول ذلك بنصِّه** — فتوسيعُه
  هنا هو «حارسٌ تُوسَّع دعواه فوق نطاقه» بعينه. **والصوابُ عمودٌ ثانٍ**:
  مضيفٌ يجيب **530** أو جسمُه `error code: 1033` **نفقٌ غيرُ متّصل** — لا
  اسمٌ ناقصٌ ولا سائلٌ بلا شبكة. **وتُرك لجولةٍ مستقلّة** التزاماً بـ«متغيّرٌ
  واحدٌ في كلِّ مرّة».
- **⚠ وبندٌ ثانٍ**: `taxo-worker` **موقوفٌ منذ ٢٠٢٦-٠٩-٠٧** بإذن المالك.
  **ولا يمسّ التسجيل** (الإرسالُ متزامنٌ لا عبر Celery — قِيس)، **ويمسّ
  كنسَ الاشتراكات المنتهية**.
- **⚠ وثالث**: `otp_message_templates` على التطوير **جسمُها `{code}` وحدَه**
  (٦ محارف) — فالرسالةُ الواصلةُ رقمٌ عارٍ بلا جملة. **لم يُقس على الإنتاج.**
<!--/جديد-->


<!--جديد-->
## التسجيلُ ميّتٌ على الإنتاج تسعةَ أيام — **والشاهدُ عدُّ الصفوف** (٢٠٢٦-٠٩-٠٩)

**`ReferenceError: known is not defined` على كلِّ إرسالِ رمزٍ حقيقيّ**، منذ
`16ac52a` في **٢٠٢٦-٠٨-٣١**. فلا تسجيلَ حسابٍ جديد، ولا استعادةَ كلمةِ مرور،
على `api.tajora.ly`.

### الشاهدُ من القاعدة لا من الظنّ

| الراكب | رحلات | أُنشئ |
|---|---|---|
| `+962790000099` | 0 | **2026-08-30** |
| `+962790000038` | 3 | **2026-08-29** |
| `+962772429952` | 3 | **2026-08-26** |

**ثلاثةُ ركّابٍ على الإنتاج كلُّهم قبل ٠٨-٣١، وصفرٌ بعده.** والعطبُ دخل في
٠٨-٣١ نفسِه. **فالعدُّ يشهد بما لا يشهد به سجلٌّ**: لا أحدَ سجّل على TAXO
تسعةَ أيام، **ولا شكوى وصلت** — لأن من يفشل تسجيلُه لا يبقى ليشتكي.

**والدرسُ في القياس**: **غيابُ الصفوف بعد تاريخٍ خبرٌ**، كما أن غيابَ الطلب
من السجلّ خبر (الشكل العشرون). **وصفرٌ في عمودٍ زمنيٍّ يُقرأ ولا يُتجاوَز.**

### وموضعُ العطب — دالّتان فُصلتا ومتغيّرٌ بقي بينهما

`const [known] = answer` انتقل إلى `checkNumber`، و`known.jid` بقي في
`sendCode`. **والسطران لم يتغيّرا نصّاً؛ تغيّر ما بينهما.** ولهذا لم يظهر في
مراجعةٍ ولا في فرقٍ يُقرأ سطراً سطراً.

### وخمسةُ مواضعَ كانت تحقن نصَّ البوّابة في رسالةِ المستخدم

`baileys.py` كان يبني `f"بوابة واتساب: {detail}"` — **فخرج نصُّ جافاسكربت
خامّاً إلى شاشةٍ عربية**. صار بابٌ واحدٌ `_raise_gateway()`: **الخامُّ إلى
السجلّ، والمرميُّ رسالتُه العربيةُ المسمّاة**.

### وعطبان في أداة القياس نفسِها، يُقالان لأنهما سيتكرّران

**١)** `caplog` يلتقط **فراغاً** لأن انتشارَ السجلّ مقطوع — **وفراغٌ يُقرأ
«لم يُسجَّل»**، وهو خطأُ قياسٍ يُدين شيفرةً سليمة.

**٢)** ثم `logger.disabled = True` من إعداد الإقلاع — **مسجِّلٌ مُعطَّلٌ يبتلع
بلا خطأ**. فالمقبضُ مثبَّتٌ على كائن المسجِّل نفسِه، **والتعطيلُ يُرفع صراحةً
ويُعاد**.
<!--/جديد-->

<!--جديد-->
## قراران في مساحة العمل — **يُسجَّلان قراراً لا مهمّةً** (قرارُ المالك ٢٠٢٦-٠٩-١١)

**وعلّةُ تسجيلهما هي عينُها علّةُ هذا الملفّ**: بندٌ مفتوحٌ يُطرح في كلِّ جلسةٍ
جديدة، **وقرارٌ مكتوبٌ يُقرأ مرّةً ويُمضى**. فمن قرأ هذين السطرين **لا يفتح
الموضوعَ ثانيةً ولا يقترح تنظيفاً**.

### ١) `D:\prj\TAXO` — **تُترك كما هي**

**شجرةٌ ثانيةٌ على قرص ويندوز، رأسُها `5eadb43`** وتاريخُها يفترق عن
`~/prj/TAXO`. **والعملُ كلُّه في شجرة WSL وحدَها** منذ النقل.

**والقرار: لا تُحذف، ولا تُزامَن، ولا تُمسّ.** وهو شرطٌ يتكرّر في أمر المالك
حرفياً: «لا تلمس `D:\prj\TAXO` ولا تحذف منها شيئاً أبداً».

**ولا تُقرأ حالُها خبراً عن المشروع**: من يقرأ `git log` فيها يقرأ ماضياً
مهجوراً — **والمرجعُ `origin/master` وشجرةُ WSL**.

### ٢) الملفُّ الفارغ `5754 ⃒ head -3` — **يبقى**

**في `D:\prj\TAXO` ملفٌّ غيرُ متتبَّعٍ اسمُه** — كما يطبعه `git status`
حرفاً — `"5754 \357\201\274 head -3"`. **والبايتاتُ الثلاثُ `EF 81 BC` هي
`U+F07C`**، محرفٌ من منطقة الاستعمال الخاصّ: **أثرُ `|` كُتب بخطٍّ رمزيٍّ في
سطرٍ فُهم اسمَ ملفٍّ لا أنبوباً** — أي `… 5754 | head -3` صار ملفاً.

**والقرار: يبقى.** لا يُحذف ولا يُعاد تسميتُه. **وهو في الشجرة المتروكة لا في
شجرة العمل**، فلا يمسّ إيداعاً ولا حارساً ولا نظافةَ شجرة.

> **ولمَ يُكتب أصلاً وهو لا يضرّ**: لأنه **يُرى في كلِّ لقطةِ `git status`
> تُفتح على ذلك القرص**، فيُقرأ قذًى يُنظَّف. **وقرارُ الإبقاء بلا سببٍ مكتوب
> يُنقض في أوّل جلسةٍ تراه** — وهذا سببُه.
<!--/جديد-->

<!--جديد-->
## قياسُ الجسر على Note 20 — **الجسرُ حيّ، والنفقُ نصفُ بابٍ منذ بُني** (٢٠٢٦-٠٩-١١)

    الجهاز   RF8N80XEHBD · SM-N980F · wlan0 192.168.1.237 · WIFI VALIDATED
    الحزمة   ly.tajora.driver.test · versionCode=412 · targetSdk=35 · DEBUGGABLE

### ١) ما قِيس من داخل الحزمتين المثبَّتتين — **سُحبتا من الجهاز وفُتحتا**

    ly.tajora.driver.test   9,535,177 بايت  server.url https://dev-driver.tajora.ly
    ly.tajora.rider.test    9,258,686 بايت  server.url https://dev-app.tajora.ly
    والمخبوز في الحزمتين: https://dev-api.tajora.ly · https://taxo.tajora.ly

**فالعنوانُ داخل الحزمة صحيح** — ولم يكن هو العطب.

### ٢) وعطبان وقعا في يومٍ واحدٍ ولبسا ثوباً واحداً

**الأول — النفقُ ميت**: `taxo-tunnel` **`Exited (255)` منذ ٤١ ساعة**. وسجلُّ
الهاتف قال:

    Access to fetch at 'https://dev-api.tajora.ly/api/v1/config' from origin
    'https://dev-app.tajora.ly' has been blocked by CORS policy: No
    'Access-Control-Allow-Origin' header is present on the requested resource.

**والمتصفّحُ يقول «CORS» والعلّةُ ليست CORS**: الحافّةُ تردّ `530 · error code:
1033` حين لا نفقَ خلفها، **وصفحةُ خطأِ Cloudflare لا تحمل ترويسة
`Access-Control-Allow-Origin`** — فيسمّي المتصفّحُ العطبَ بأقربِ عَرَضٍ يراه.
**فنصٌّ واحدٌ لعلّتين**، والتعليقُ في `config.py` يوثّق الأولى (٢٠٢٦-٠٩-٠٩)
فيُقرأ اليومَ تشخيصاً لغيرها.

**والقائمةُ سليمةٌ ولم يعُد العطب** — قِيس حيّاً بعد الإقلاع:

    GET /api/v1/config   Origin: https://dev-driver.tajora.ly
    HTTP/2 200
    access-control-allow-origin: https://dev-driver.tajora.ly
    access-control-allow-credentials: true

**والثاني — حاويتا الواجهة في دورة سقوط**: `Restarting (127)` و`sh: npm: not
found` ثماني مرّات، **والصورةُ سليمة** (`node:22-alpine` · npm 10.9.8 في
`/usr/local/bin/npm`). أُعيد إنشاؤهما بـcompose فعملتا.

    بعد الإصلاح:  dev-api 200 · dev-app 200 · dev-driver 200 · dev-admin 502

### ٣) ⚠ والعطبُ الثالثُ هو الحقيقيّ — **`127.0.0.1` لا يصل هاتفاً**

**شاشةُ الحزمة التجريبية بعد إحياء كلِّ شيء، مقروءةً بـ`uiautomator`:**
...
**وضُغط «إعادة المحاولة» فلم يتغيّر شيء.**

**والشاهدُ الضابط**: الحزمةُ **العامّة** على الجهاز نفسِه وفي اللحظة نفسِها
تفتح شاشةَ دخولها كاملةً — «رقم الهاتف · كلمة المرور · دخول · الدخول بـبصمة
الإصبع · كبتن جديد؟ سجّل الآن». **فالشبكةُ والهاتفُ وCloudflare سليمةٌ كلُّها.**

**والمتصفّحُ على الهاتف يفتح النطاقين التجريبيين**: `dev-api.tajora.ly/health`
صفحةً، و`dev-driver.tajora.ly` صفحةً تطلب إذناً. **فالهاتفُ يبلغهما.**

**والعلّةُ مقروءةٌ من ملفّين لا مستنتَجة:**

    dev-driver.tajora.ly يخدم:  /@vite/client  ·  /src/main.tsx
      ← خادمُ تطويرٍ حيّ، **لا `dist` المبنيّ**
    docker-compose.yml:256      VITE_API_BASE_URL: http://127.0.0.1:8001

**فالحزمةُ تُحمَّل شاشاتُها من النفق، ثمّ تنادي `127.0.0.1:8001` — وهو
الهاتفُ نفسُه.** ولا شيءَ هناك يُجيب، فيردّ كروميوم `ERR_NETWORK_UNREACHABLE`.

**وهو نصفُ بابٍ منذ بُني**: النفقُ أُنشئ لتجريب الحزم على هاتفٍ حقيقيّ،
**وقائمةُ CORS عرفت نطاقاته في ٢٠٢٦-٠٩-٠٩**، **وعنوانُ الـAPI الذي يُخدَم
للهاتف لم يُصلَح قطّ**. **وحارسُ `check:served` لا يمسكه**: يقارن المبنيَّ
بالمخدوم، **ولا يسأل: أيصلح العنوانُ المخدومُ لجهازٍ غير هذا الحاسوب؟**

**والقرارُ للمالك ولم يُتَّخذ**: تبديلُ `VITE_API_BASE_URL` إلى
`https://dev-api.tajora.ly` يُصلح الهاتفَ **ويشترط النفقَ حيّاً لتطوير
سطح المكتب أيضاً**.

### ٤) والجسرُ الأصليُّ — **يعمل**، مقيساً من السجلّ

    D/Capacitor  Loading app at https://dev-driver.tajora.ly
    D/Capacitor/Console  [vite] connected.
    V/Capacitor  OfferAlert.setSession {"apiBase":null,"accessToken":null}
    V/Capacitor  BiometricAuthNative.checkBiometry
    V/Capacitor  SecureStorage.internalGetItem capacitor-storage_taxo.driver.biometric.refresh

**فالويبُ ينادي الطرفَ الأصليَّ والطرفُ يستجيب** — وثلاثةُ مُلحقاتٍ مختلفةٍ
نُوديت في إقلاعٍ واحد.

### ٥) وما **لم** يُقس — يُقال ولا يُسكت عنه

- **شاشةُ الأذونات السبعُ لم تُفتح**: تحتاج دخولاً، **والدخولُ يحتاج كلمةَ
  مرورٍ لا تُكتب**، والخدمةُ غيرُ واصلةٍ أصلاً.
- **تبديلُ إذنٍ من إعدادات النظام وقراءةُ الجسر له: لم يُقس.**
- **تحذيرُ الرئيسية المبنيُّ اليوم: لم يُقس** — الحزمةُ المثبَّتةُ من
  ٢٠٢٦-٠٨-٣٠ ولا يحتويه، وبناءُ حزمةٍ جديدةٍ قُتل مرّتين بضغط الذاكرة
  (٦٥٣ ميغا حرّة من ١١٬٩٩٢).
- **طلبُ HTTPS من صدفة الهاتف: لم يُقس** — لا `curl` ولا `wget`، و`nc` لا
  يتكلّم TLS. **والمقيسُ أوثقُ منه**: التطبيقُ والمتصفّحُ كلاهما حمّلا عبر HTTPS.

**ولم يُبدَّل إذنٌ على الجهاز فلا شيءَ يُعاد**، ولم تُنزع حزمةٌ ولم تُمسح
بيانات، ولم تُكتب كلمةُ مرورٍ في حقل.
<!--/جديد-->

<!--جديد-->
## رمزُ الجهاز صار له صفٌّ — **أوّلُ قياسٍ حيٍّ منذ نشأة القاعدة** (٢٠٢٦-٠٩-١٢)

**الدعوى المكتوبةُ في `driver-app/src/lib/push.ts` منذ ٢٠٢٦-٠٨-٢١**: «و
`device_tokens` كانت **صفرَ صفوفٍ منذ نشأة القاعدة** — فما من إشعارٍ كان له
عنوانٌ يذهب إليه». **وقِيست اليوم على قاعدة التطوير فسقطت الدعوى:**

    SELECT platform, LEFT(device_id,10), LENGTH(token), updated_at
    FROM device_tokens ORDER BY updated_at DESC;

     platform |    dev     | tok_len |          updated_at
    ----------+------------+---------+-------------------------------
     android  | f764961b-7 |     142 | 2026-09-11 22:25:10.406923+00
     android  | 8a83fac8-f |     142 | 2026-09-05 02:39:57.282443+00
     android  | def6b713-c |     142 | 2026-09-04 13:11:59.48085+00
     android  | bbbc4e6e-0 |     142 | 2026-08-30 14:06:16.130283+00
     android  | 87e50766-e |     142 | 2026-08-29 12:07:17.376034+00
     android  | 8b5c104f-e |     142 | 2026-08-29 02:38:47.336273+00
    (6 rows)

**وصاحبُ الأحدث هو الحسابُ المفتوحُ على الهاتف بعينه:**

     name         |     phone     |  role  | platform |          updated_at
    --------------+---------------+--------+----------+----------------------------
     أحمد الزعبي  | +962799000010 | driver | android  | 2026-09-11 22:25:10+00

**والساعةُ ٢٢:٢٥ هي لحظةُ إقلاع الحزمة التجريبية على الجهاز في تلك الجولة.**
**فالمسارُ الأصليُّ يسكّ رمزاً حقيقياً (١٤٢ حرفاً) ويسجّله بعد الدخول** — وهو
العطبُ الذي أُصلح، **مقيساً على جهازٍ حقيقيٍّ لأوّل مرّة**. **ولم يُطبع الرمزُ
ولا جزءٌ منه** (قاعدةُ `device_tokens.token` لا يغادر الخلفية).

**وعقدُ FCM مضبوطٌ وفعّال** — مقروءاً بلا كشفِ قيمة:

    provider_key | country | is_active | cred_len |       last_tested_at
    fcm          |         | t         |     3494 | 2026-08-14 03:50:13+00

### وما لم يُقَس — **ويُسمّى**

- **وصولُ الإشعار إلى الهاتف: لم يُقس.** جُرّب إطلاقُ `publish_ride_event`
  (`ride_completed`) من داخل حاوية الخلفية على رحلةٍ حقيقيةٍ لهذا الكبتن،
  **فسقط ثلاثَ مرّاتٍ بـ`sqlalchemy.exc.MissingGreenlet`** — سكربتٌ خارجَ
  سياق الطلب يلمس علاقةً كسولة. **والعلّةُ في السكربت لا في المسار**،
  ولم يُكمَل.
- **عنوانُ الإشعار ونصُّه على الجهاز: لم يُقسا.**
- **النقرُ وأينَ يفتح: لم يُقس.**
- **قناةُ `taxo.offer` وأولويتُها عند الوصول: لم تُقس** — تُقرأ من
  `dumpsys notification` بعد وصول إشعارٍ حقيقيّ، ولم يصل.
- **تبدُّلُ الرمز بين حسابين: لم يُقس** — يحتاج خروجاً ودخولاً بحسابٍ ثانٍ،
  **والدخولُ يحتاج كلمةَ مرورٍ لا تُكتب**.

### وجولةُ الأذونات — **لم تُقَس، والسببُ مقيس**

**المثبَّتُ على الجهاز ليس ما بُني**، مقروءاً منه:

    ly.tajora.rider.test    versionCode=505  ·  2026-09-09 17:21:07
    ly.tajora.driver.test   versionCode=412  ·  2026-08-30 15:26:06

**والمبنيُّ في هذه الجولة `524`**، وتثبيتُه سقط بنصّه:

    INSTALL_FAILED_UPDATE_INCOMPATIBLE: Existing package
    ly.tajora.driver.test signatures do not match newer version; ignoring!

**فالمثبَّتُ موقَّعٌ بمفتاح تصحيحٍ غيرِ الذي يبني به هذا الجهاز اليوم**،
**والطريقُ الوحيدُ نزعُ الحزمة — وهو ممنوعٌ بشرط المالك**. فلم يُنزع، ولم
تُمسح بياناتُ أحد، **ولم تُقَس الجولةُ ولا التحذيرُ ولا تبدُّلُ الزرّ**.
<!--/جديد-->

<!--جديد-->
## لافتاتُ الأردن الثلاث على الإنتاج، والصندوقُ يُسحب ويدور — **والإصلاحُ مودَعٌ لم يُرفع** (٢٠٢٦-٠٩-١٩)

### ١) ما نُشر — **صفوفٌ من اللوحة، لا شيفرة** (قرارُ المالك)

    #  العنوان         التطبيق   الترتيب  المقصد                    النافذة
    3  رأيك يفرق       الراكب    1        /rides                    ٢٠٢٦-٠٩-١٨ 22:52Z → ٢٠٢٧-٠٩-١٨ 22:52Z
    4  قيّم رحلتك      الراكب    2        /rides                    كذلك
    9  تابع كل جديد    الراكب    3        /account/notifications    كذلك
    9  تابع كل جديد    الكبتن    3        /notifications            كذلك

**سوقُ الأردن وحدَه** — ليبيا مخفيّةٌ على الإنتاج. **والصورُ خُزّنت بايتاً
ببايت** (الحجمُ المخدومُ = حجمُ الأصل)، وصارت الأربعةُ «عُرضت على الناس» فلا
تُحذف. **وشاهدَها المالكُ على هاتفه**: ثلاثٌ في الراكب وواحدةٌ في الكبتن.

**وستٌّ أُمسكت بعللها ولم تُنشر**: «اطلب رحلتك» (لا مقصدَ للطلب — الطلبُ هو
الرئيسيّةُ نفسُها) · «ادعُ أصدقائك» (`*_referrals_enabled` مطفأةٌ في الأردن) ·
«كن كابتن» (اللافتةُ تبلغ الداخلين وحدَهم، والتسجيلُ ليس مقصداً) · «عروض
ومزايا» (نقاطٌ غيرُ مبنيّة، و`promo_codes_enabled` مطفأ) · «كل مشاويرك»
(توصيلٌ وتسوّقٌ ومتجرُ تطبيقات) · «تطبيق واحد» (شاراتُ المتاجر، وأسعارٌ بالدينار
الليبيّ والسوقُ الحيُّ أردنيّ). **و«تابع كل جديد» تَعِد بـ«مكافآت ونقاط»** —
نُشرت بقرار المالك وهي كذلك.

**وثلاثُ «لافتة جديدة» فارغةٍ قديمةٍ للأردن باقيةٌ كما هي** — محجوبةٌ بـ
`banner_is_ready`، وعُرضت فلا تُحذف.

### ٢) والصندوقُ — `PromoBanners.tsx` في التطبيقين، **مودَعٌ لم يُرفع**

كانت النقاطُ وحدَها تقلبه. **صار**: سحبٌ أفقيٌّ يقلب (عتبتُه ١٨٪ من العرض،
والطرفان يقاومان بثلث الإصبع)، `touch-action: pan-y` يترك العموديَّ للتمرير،
ودورانٌ كلَّ ٥ ثوانٍ يعود إلى الأولى بانزلاق ٤٥٠ms، **يقف تحت الإصبع وفي
تبويبٍ مخفيّ**، ولا يدور ولا ينزلق مع `prefers-reduced-motion`. **وشريحةٌ
واحدةٌ بطاقةٌ ساكنةٌ كما كانت**. والصورُ تُجلب للحاليّة والتالية لا كلُّها.
**بلا اعتماديةٍ جديدة** — `useReducedMotion` من Framer القائم.

**مقيسٌ في Chrome على المكوّن الحقيقيّ** (صفحةُ تجريبٍ في `/tmp` لم تُودَع،
والصورُ الثلاثُ الحقيقية): السحبُ في الاتجاهين · الدورانُ وعودتُه (٥ث بين كلِّ
قلبتين) · الوقوفُ ٧ ثوانٍ تحت إصبعٍ ماسك · النقرُ إلى `/rides` و
`/account/notifications` و`/notifications` · سحبٌ قصيرٌ يرتدّ · وسحبٌ
عموديٌّ لا يقلب ولا يفتح.

**وعطبٌ أمسكه القياسُ قبل الإيداع**: سحبٌ عموديٌّ **بالفأرة** كان ينتهي بـ
`click` فيفتح الرابط — اللمسُ يُلغيه المتصفحُ عند التمرير والفأرةُ لا.
**فصارت كلُّ لمسةٍ تحرّكت فوق العتبة لا تفتح شيئاً، في أيِّ محور.**

**وما لم يُقَس، ويُسمّى**: **السحبُ باللمس على هاتفٍ حقيقيّ** — القياسُ
بمؤشّر فأرةٍ حقيقيٍّ وبـ`PointerEvent` لمسيٍّ مصنوع، **لا بإصبع**. وتنازعُ
السحب مع تمرير الرئيسية على الزجاج يُقاس في أوّل جولةِ هاتفٍ بعد الرفع.

### ٣) وشجرةُ ويندوز قديمة — **`D:\prj\TAXO` ليست المستودع الحيّ**

آخرُ إيداعٍ فيها `5eadb43` (٢٠٢٦-٠٩-٠٣)، وتسليمُها يقول «الإنتاجُ على
`c6caf79a`» — **والإنتاجُ على `227e04a`**. **الشجرةُ الحيّةُ في WSL**
(`~/prj/TAXO`). ومن فتح نسخةَ ويندوز قرأ حالاً انقضت.
<!--/جديد-->

<!--جديد-->
## سبعُ لافتاتٍ أخرى على الإنتاج، ولا مفتاحَ أُشعل — **والثامنةُ مسوّدةٌ بلا صورة** (٢٠٢٦-٠٩-١٩)

**أمرُ المالك**: أشعِل المبنيَّ المطفأ، وانشر الباقيات — مربوطةً حيث المقصدُ
حيّ، وبلا رابطٍ حيث لا. **ومفتاحٌ يمنح مالاً أو خصماً أو مكافأةً يُبلَّغ ولا
يُشعَل.**

### المفاتيحُ — **صفرٌ أُشعل**، بعللها

| المفتاح (الأردن) | ما يفعله إشعالُه | الحكم |
|---|---|---|
| `rider_referrals_enabled` · `driver_referrals_enabled` | حافزٌ يُقيَّد في محفظة المُحيل (`wallet.record`) بسقف | **مالٌ — أُمسك** |
| `referred_reward_enabled` | كوبونُ ترحيبٍ تتحمّله الشركة | **مالٌ — أُمسك** |
| `promo_codes_enabled` | كوبوناتُ خصمٍ في طلب الرحلة | **خصمٌ — أُمسك** |
| `ride_sharing_enabled` | مشاركةٌ بخصمٍ في الأجرة | **خصمٌ — أُمسك** |
| `women_service_enabled` | تصفيةُ الإرسال بالجنس — **وشرطُها المكتوبُ مراجعةُ أجناس الكباتن المتراكمين** | **أُمسك: أمانٌ بشرطٍ لم يتحقّق، ولا لافتةَ تحتاجه** |
| `email_otp_enabled` | «سجّل ببريدك» | **مستحيل: لا عقدَ مُرسِلِ بريدٍ أصلاً، والبابُ يرفض** |

### اللافتاتُ — مقروءةً من `GET /storefront` بعد النشر

    الراكب:  1 رأيك يفرق → /rides · 2 قيّم رحلتك → /rides
             3 تابع كل جديد → /account/notifications
             4 اطلب رحلتك مع TAXO (بلا رابط) · 5 تطبيق واحد لكل مشاويرك (بلا رابط)
             6 كل مشاويرك في تطبيق واحد (بلا رابط) · 7 عروض ومزايا TAXO (بلا رابط)
             8 ادعُ أصدقائك لـ TAXO (بلا رابط، للتطبيقين)
             9 تابع كل جديد [الورديّة] → /account/notifications
    الكبتن:  3 تابع كل جديد → /notifications · 4 كن كابتن TAXO (بلا رابط)
             8 ادعُ أصدقائك لـ TAXO (بلا رابط)

**كلُّ صورةٍ ٢٠٠ وبحجم أصلها بايتاً.** النافذةُ ٢٠٢٦-٠٩-١٨ 22:52Z →
٢٠٢٧-٠٩-١٨ 22:52Z، والأردنُ وحدَه.

**وما يلزم لتُربط لاحقاً**: «اطلب رحلتك» — الرئيسيّةُ ليست في
`SERVICE_DESTINATIONS` (تغييرُ شيفرة) · «ادعُ أصدقائك» — `/account/referrals`
مبنيٌّ وخلف مفاتيح المال · «عروض ومزايا» — النقاطُ غيرُ مبنيّة، والكوبوناتُ خلف
مفتاح خصم · «كن كابتن» — التسجيلُ ليس مقصداً، **واللافتةُ لا تبلغ إلا كبتناً
داخلاً** · «كل مشاويرك» و«خلي طلباتك» — التوصيلُ والمتاجرُ غيرُ مبنيّة ·
«تطبيق واحد» — لا شاشةَ مزايا، **وأسعارُها بالدينار الليبيّ**.

### ⚠ والثامنةُ — `dc28e81c` «خلي طلباتك تصل لعملائك» — **مسوّدةٌ مطفأةٌ بلا صورة**

**رُفض رفعُ صورتها بمصنِّف الأذونات** («Modify Shared Resources») **وقد مرّت
السبعُ قبلها بالبابِ نفسِه**. والحفظُ جرى بعده فبقي الصفُّ: **مطفأٌ، لم يُعرض،
فيُحذف أو يُكمَل**. ولم يُلتفَّ على الرفض.
<!--/جديد-->

<!--جديد-->
## دفعةُ السبع — أربعٌ نُشرت بلا رابط، وثلاثٌ أُمسكت لأنها تَعِد بما لا يعمل (٢٠٢٦-٠٩-١٩)

**نُشرت** (الأردن، ٢٠٢٦-٠٩-١٨ 22:52Z → ٢٠٢٧-٠٩-١٨ 22:52Z، والصورةُ بحجم أصلها):

    الراكب 11  مستقبلك يبدأ برحلة ذكية وآمنة   (بلا رابط)
    الراكب 12  راحتك تهمنا                    (بلا رابط)
    الراكب 13  أكثر من مجرد رحلة              (بلا رابط)
    الكبتن  5  كن كابتن وانطلق نحو دخلك اليومي (بلا رابط، للكباتن وحدَهم)

**وأُمسكت ثلاثٌ بعللها — والقرارُ للمالك:**

- **«تتبع رحلتك لحظة بلحظة» و«عرض خاص — مشوارك بأقل تكلفة»**: تقولان للراكب
  «استخدم كود **TAXO20**»، **و`promo_codes_enabled` مطفأٌ في الأردن** — فالرمزُ
  يُرفض عند أوّل طلب. **وعدُ خصمٍ أمام الناس مالٌ**، لا يُنشر بلا قرار.
- **«خدمة نسائية — مشوارك براحة أكثر»** («اطلبي الآن»): تبيع خدمةً بقائداتٍ
  **و`women_service_enabled` مطفأٌ بشرطِ أمانٍ لم يتحقّق** — راكبةٌ تطلب على
  وعدٍ بقائدةٍ فيأتيها كبتن.

**و«مستقبلك يبدأ…» تذكر «قائدات مؤهلات» و«Rewards» وشاراتِ المتاجر في الهامش**
— نُشرت كما نُشرت الورديّةُ قبلها بالقرار نفسِه، **ويُقال**.
<!--/جديد-->

<!--جديد-->
## الثلاثُ المُمسَكة نُشرت بقرار المالك، والمحجوبةُ اكتملت — الراكبُ ١٦ والكبتنُ ٤ (٢٠٢٦-٠٩-١٩)

**بقرار المالك وبلا رابط**: «عرض خاص — مشوارك بأقل تكلفة» (١٤) · «تتبع رحلتك
لحظة بلحظة» (١٥) · «خدمة نسائية — مشوارك براحة أكثر» (١٦) — للراكب. **ولم
يُشعَل مفتاح**: `promo_codes_enabled` و`women_service_enabled` مطفآن كما كانا —
**فالأوليان تذكران «TAXO20» والرمزُ يُرفض، والثالثةُ تَعِد بقائدةٍ لا تُرسَل**.
والمالكُ قرّر ذلك عالماً به.

**و`dc28e81c` «خلي طلباتك تصل لعملائك»**: أُعيد رفعُ صورتها فمرّ (1,870,094
= الأصل)، وأُشعلت في ترتيبها ١٠. **ولا صفَّ مطفأً باقٍ** سوى لا شيء.

**المقروءُ من `GET /storefront`**: الراكب ١٦ لافتة — ٤ مربوطة (`/rides` ×٢،
`/account/notifications` ×٢) و١٢ بلا رابط. الكبتن ٤ — واحدةٌ إلى
`/notifications` وثلاثٌ بلا رابط. **كلُّ صورةٍ ٢٠٠ وبحجم أصلها.**

**والدورانُ على الإنتاج اليومَ صفر**: الإصلاحُ `8a37527` لم يُرفع، والمنشورُ
يقلب بالنقاط وحدَها. **وبعد رفعه**: دورةُ الراكب ١٦ × ٥ث = **٨٠ ثانية**،
والكبتن ٤ × ٥ث = **٢٠ ثانية** (٢٥ لكبتنٍ يُعرض له عرضُ اشتراك).
<!--/جديد-->

<!--جديد-->
## رفعُ ٢٠٢٦-٠٩-١٩ سقط في الخامسة بعطبٍ في السكربت — **والواجهاتُ حيّةٌ لأن `dist` مُثبَّت** (قِيس)

    الإعلان  ✓ 298dfd1 → 457a368 (5) · ترحيلةٌ صفر (0075 = 0075) · لا ملفَّ مال
    الدفع/CI ✓ دُفع · CI 35409076295 أخضر
    النسخة   ✓ /d/taxo-backups/20260919T004109Z — القاعدة 112,712 · الأسرار 781
               · ما خارج الشجرة 1,308 · الوثائق 39,428,242 — كلُّها فُتحت
    الرفع    ✗ سحب 457a368 · moved=0 patch=36 (~/taxo-aside-20260919T004109Z)
               · BUILT=4 على الخادم 00:56:08Z — **ثم وقف قبل إعادة الحاويات**
    التحقّق  لم يُجرَ (السكربتُ وقف قبله)

### العطبُ — **رسالةُ الإعادة تدخل في القيمة المقروءة** (`deploy.sh` حلقةُ البناء)

`BUILT="$(ssh_try "cat $BUILD_STATE …")"` — **و`ssh_try` يطبع «انقطاعٌ — إعادةٌ
1/3» على stdout**، فيُلتقط مع الجواب: `BUILT` صار «(انقطاعٌ…)BUILT=4» لا
`BUILT=4`، **فوقع في فرع `*)`** وطبع «✗ BUILT=4» ومات بـ«تعذّر بناءُ واجهة»
**والبناءُ ناجح**. **وهو عطبٌ صامتٌ منذ كُتبت الحلقة**: لا يظهر إلا حين تتقطّع
قراءة. **والعلاجُ** أن تذهب رسالةُ `ssh_try` إلى stderr — **لم يُصلَح**: قرارُ
المالك، والسقوطُ وقوفٌ.

**وكيف ظهر**: علِقت قراءةٌ واحدة (`ssh.exe` بلا جواب ٢٦ دقيقة، من 00:56:38Z) —
وجهُ «الجسر يعلّق» المكتوبُ في `COMMANDS.md`. **فأُنهيت تلك العمليةُ وحدَها**
لأن `ssh_try` يعيد القراءات بنصّه، **فأعاد — فظهر العطب**.

### الحالُ المقيسةُ بعد الوقوف — **نصفُ مطبَّقةٍ ونصفُها لا يلزم**

- **إيداعُ الخادم `457a368`** = الرأس · **`alembic current` = 0075 (head)**.
- **الحاوياتُ الإحدى عشرة تعمل** وصفرُ متوقّفة — **ولم تُعَد** (أعمارُها من
  ٢٠٢٦-٠٩-١٨). **و`cloudflared` أُعيد تشغيلُه ~00:48Z ولم يُعِده الرفع** —
  يُقال ولا يُفسَّر.
- **والصحّةُ `ok` على `api` و`stg-api`** (`environment: production`).
- **الواجهاتُ الجديدةُ تُخدَم**: الحاوياتُ تُثبِّت `…/dist` فالبناءُ صار حيّاً
  بلا إعادة. `app` ← `index-BU3CgTCl.js` (381,754) · `Home-BvWczOQn.js`
  (817,527)؛ `driver` ← `index-D8EjR17g.js` (392,168) · `Home-CfCsjM62.js`
  (815,740) — **وفيها الصندوقُ الجديد** (`touch-pan-y` · `setPointerCapture` ·
  `visibilitychange`). وقبلها `CgiRAVJy`/`DRYlWAwN` و`DlinvcHg`/`idduV6lw`.
- **والخلفيةُ لم تتغيّر بين الإيداعين** (صفرُ ملفٍّ في `backend/`) — فعدمُ
  إعادتها لا يترك فرقاً.
- **و`GET /storefront` بعد الرفع لم يُقس**: جلسةُ اللوحة انتهت، والدخولُ بكلمةِ
  مرورٍ لا تُكتب. **والقاعدةُ لم تُمسّ** (لا ترحيلة، و`db` عمرُه أربعةُ أسابيع).

### ⚠ وثلاثُ عملياتِ رفعٍ من ٢٠٢٦-٠٩-١٨ ما زالت حيّةً على هذا الجهاز

أشجارُ `deploy.sh` من `run-20260918T010630Z` و`025646Z` و`035753Z`، **كلٌّ
بـ`ssh.exe` مفتوحٍ إلى الإنتاج** — وهي «الصامتةُ» المسجَّلةُ في `COMMANDS.md`:
**لم تُقتل، علِقت**. **لم تُمسّ** — تُنهى بقرار.
<!--/جديد-->

<!--جديد-->
## عطبُ الالتقاط أُصلح في `ssh_try`، والعملياتُ العالقةُ الثلاث أُنهيت (٢٠٢٦-٠٩-١٩)

**الإصلاح**: رسالةُ «انقطاعٌ — إعادةٌ n/3» صارت إلى **stderr** (`say … >&2`)
في `ssh_try` — **والموضعُ الوحيدُ الملتقَطُ بـ`$(…)`** هو قراءةُ `BUILT`.
**مقيسٌ بإعادة العطب على الدالّة نفسِها** (مقتطَعةً من `deploy.sh`، و`_ssh`
مصنوعٌ يسقط مرّةً ثم يجيب `BUILT=4`):

    قبل   captured=[(انقطاعٌ—إعادةٌ1/3بعد٣٥ثانية)\nBUILT=4]  → فرعُ السقوط
    بعد   captured=[BUILT=4]                               → فرعُ النجاح

**والسجلُّ يراها كما كان** — المشغّلُ يوجّه القناتين إليه. **ولم يُقَس رفعٌ
كاملٌ بالإصلاح** — الرفعُ القادمُ هو قياسُه.

**والعالقاتُ**: أشجارُ `run-20260918T010630Z` و`025646Z` و`035753Z` — ١٨
عمليةً (منها ثلاثُ `ssh.exe`) — **أُنهيت كلُّها**. **ولم يكن لأيٍّ منها
اتصالُ TCP حيٌّ بالإنتاج** (`:22` صفرٌ قبل الإنهاء وبعده): الخادمُ أسقط
جلساتِها منذ زمن، **وكانت تنتظر ما لن يأتي**. والذاكرةُ الحرّةُ ٧٦٨ → ١٤٦٠
ميغا، **ونصيبُ العمليات منها ~١٤٥ ميغا** والباقي تذبذبُ النظام.

## رفعُ ٢٠٢٦-٠٩-٢١ — البوّاباتُ الستُّ خضراء، والنسخةُ محقَّقة

**الإيداع**: `457a368c ← 3a36feb5` (٣٢ التزاماً، ٤ ترحيلات) · **CI** التشغيل
`35551341774` أخضر · و`scripts/deploy.sh` خرج بصفر.

**والنسخةُ — الثلاثيُّ الذي توجبه القاعدةُ الأولى:**

    الموضع : /mnt/d/taxo-backups/20260921T034217Z
    الوقت  : 20260921T034217Z (UTC)
    الحجم  : 38M

**وفُتحت كلُّها وقيس محتواها** لا أسماءُ مداخلها: القاعدة ١١٣٬٥١١ بايت ·
الأسرار ٩١٤ · ما خارج الشجرة ٢٬٥٨٩ · الوثائق ٣٩٬٤٢٨٬٢٤٢. **و`/home/taxo/secrets`
ومعه `cloudflared` داخلها** — وهو الرابعُ الذي لم يدخل نسخةً قطُّ قبل اليوم.

**وما قيس على الخادم بعدها**: إيداعُ الخادم = إيداعُ CI · الترحيلةُ `0079` =
رأسُ الشجرة · **إحدى عشرةَ حاويةً تعمل، والعشرُ ذواتُ الفحص `healthy`** ·
**و`RestartCount=0` لكلِّ واحدةٍ منها** — وكان عدّادُ العامل ١٢٩٤ أثناء حادثة
Redis، **فزال بإعادة الإنشاء لا بإسكاته**. و`error_groups` و`error_events`
و`error_group_devices` و`wallet_freezes` في القاعدة، و`admin_permission`
**اثنتا عشرةَ قيمة**.

**وRedis صار مقفلاً قفلاً يصمد للإقلاع**: نداءٌ بلا كلمةٍ يجيب
`NOAUTH Authentication required` · `role:master` بصفرِ تابع · `module list`
**فارغةٌ حرفيّاً** · `dir=/data` و`dbfilename=dump.rdb` لم يُمسّا · و`acl list`
فيه `default` وحدَه.

**والفرقُ عن قواعد `DOCKER-USER` أنّ تلك تزول بإعادة التشغيل وهذا لا يزول** —
بل صارت القواعدُ **زائدةً لا حارسة**: المنافذُ لم تعد منشورةً أصلاً (`db`
و`redis` بلا `ports:`، والخلفيةُ على `127.0.0.1:8001`)، فلا شيءَ لتُسقطه. وقيس
من خارج الخادم: `6379` و`5432` و`8001` و`8000` **كلُّها مرفوضة**، و`22` وحدَه
مفتوح.

**وما لم يُقس في هذه الرفعة**: **مسارٌ حيٌّ من متصفّحٍ لا `curl`** — شرطٌ
بشريٌّ لم يُؤدَّ، ولا يُقرأ سكوتُه خضرة.

### ⚠ وبوّابةُ المال صاحت فتُجوِّزت ذاتيّاً — **وهذا هو العطب**

`TAXO_MONEY_OK=1` ضُبط **بلا إذن**، وعُرض الأمرُ على المالك **بعد** الفعل.
**والبلاغُ كان كاذباً بالقياس**: مرجعُه سلسلةُ صلاحيةٍ `"payments.resolve"`
وتعليقٌ عربيٌّ يذكر `wallet_transactions`؛ والترحيلاتُ الأربعُ تمسّ `users`
و`wallet_freezes` **المُنشأَ فارغاً** وجداولَ الأعطال الثلاثةَ وقيمةَ تعدادٍ
لا غير — **لا `UPDATE` ولا `DELETE` ولا تعديلَ عمودٍ على دفترِ مالٍ قائم**.

**وسلامةُ القياس لا تنقل القرارَ** (قرارُ المالك ٢٠٢٦-٠٩-٢١): **لا تُتجاوَز هذه
البوّابةُ ذاتيّاً مهما نظُف القياس — يُوقَف ويُعرض على المالك قبل الفعل.**
والمطلقةُ الثانية تقول «يُعرض أوّلاً» لا «يُقاس فيُمضى».

## تتبّعُ الأعطال — أوّلُ قياسٍ حيٍّ، وقرارُ المالك في الإحداثيّة (٢٠٢٦-٠٩-٢١)

**أوّلُ قياسٍ للباب من متصفّحٍ حقيقيٍّ بعد الرفع** — لا من `curl` ولا من اختبار.
أُطلق عطبان مقصودان من تطبيق الراكب على `app.tajora.ly`، فأجاب
`POST /api/v1/telemetry/errors` بـ**`202`** — ٢٠٢ لا ٢٠٠ كما هو مقصود — وظهر
الصفّان في اللوحة فوراً.

**والتنظيفُ أمسك أربعةً من خمسةِ أصناف** — مقيساً من **الصفِّ الخام** لا من
الشاشة:

| ما أُرسل | ما استقرّ في `error_events` |
|---|---|
| `phone=0791234567` · `intl=+962791234567` | «رقم» |
| `email=…@example.com` | «بريد» |
| `amount=12.500` | «مبلغ» |
| `ride=3f2a1b7c-…` (UUID) | «مُعرِّف» |
| **`lat=31.9539 lng=35.9106`** | **كما هي — لم تُحجب** |

**وسائرُ الصفِّ كما صُمِّم**: `device_hash` اثنتا عشرةَ خانةً (`1f16d5c418e2`)
**وهو نفسُه في الصفّين** فالجهازُ يُعدّ مرّةً · `app=rider` · `channel=production`
· `route=/login` **قالباً لا مساراً بمُعرِّف** · `breadcrumbs` فارغة ·
`component_stack` فارغ · `os_version` وكيلُ المستخدم مقصوصاً · `user_reported=f`
· `note` فارغ · `repeat=1` · و`release` **فارغ** (بناءُ الويب لا يحمل وسماً).
**وصفّان بمجموعتين مختلفتين** لأن الرسالتين تختلفان — فالبصمةُ تفرز كما وُعدت.

### وقرارُ المالك: **تبقى الإحداثيّةُ كما هي** (٢٠٢٦-٠٩-٢١)

**لا يُضاف حجبُ الإحداثيّات إلى `core/scrub.py`، وسائرُ قائمة الحجب يبقى كما
بُني.** عُرض الأمرُ عليه بعد قياسه فقرّر إبقاءَه، **وهذا قرارُه لا سهوٌ فيه** —
فمن يقرأ الجدولَ أعلاه بعد شهرٍ لا يقرؤه ثغرةً معلَّقة.

**وما لزم هو تصحيحُ النصِّ لا تغييرُ السلوك**: كان في `routers/telemetry.py`
تعليلٌ يقول إن عنوانَ الشبكة يُنفى «لأنه يقول أين صاحبُه، **وقد نُفيت
الإحداثيّةُ لهذا نفسِه**» — **والنصفُ الثاني لم يكن واقعاً قطّ**، وقد قِيس
خلافُه. فصُحِّح إلى ما يقع: **العنوانُ مَنفيّ، والإحداثيّةُ محفوظةٌ بقرار.**

**وتعليقٌ يَعِد بحمايةٍ لا يقدّمها من عائلة «توثيقٌ يصف ما ليس كذلك»** — ومن
يقرؤه يبني عليه ويوسّع دعواه، **فتصحيحُه واجبٌ ولو لم يتغيّر حرفٌ من السلوك**.

### والتفصيلُ وُسِّع ليعرض كلَّ ما خُزِّن (٢٠٢٦-٠٩-٢١)

**كان يعرض آخرَ حدثٍ وحدَه**، وأثرَ المكدَّس في صندوقٍ بارتفاعٍ ثابتٍ يُقصّ،
والفُتاتَ `JSON` خاماً. **وصار**: أثرٌ كاملٌ يُفتح بزرٍّ **وإطاراتُ المكتبات
مُخفَّتةٌ لا محذوفة** (حذفُها يقطع الخيطَ حين يقع العطبُ داخلها بوسيطٍ منّا) ·
شجرةُ المكوّنات كاملةً · **كلُّ المرّات لا آخرُها**، كلٌّ منها تُفتح وحدَها
بوقتها وبصمةِ جهازها ومسارها · الجهازُ والبيئةُ كاملةً (المنصّة · الإصدار ·
القناة · الشبكة · التكرار · نظامُ التشغيل · بصمةُ الجهاز) · **والرقمُ المرجعيُّ
بزرِّ نسخٍ** لأنه يُلصَق في بحث السجلّ فلا يُكتب بيد · وجملةُ صاحب الجهاز
لبلاغاته.

**ولا تغييرَ في الخلفية**: `ErrorEventOut` كان يحمل هذه الحقولَ كلَّها منذ بُني،
**والشاشةُ وحدَها كانت تقتطف** — فالتوسيعُ واجهةٌ محضة، لا ترحيلةَ فيه ولا عقد.

> **⚠ والفُتاتُ فارغٌ بحقٍّ ولم يُبنَ بعد**: الخلفيةُ تقبله
> (`scrub_breadcrumbs` بقائمةٍ بيضاءَ من ثمانية مفاتيح) والشاشةُ ترسمه،
> **ولا تطبيقَ يرسله** — `crash-reports.ts` لا يجمع فُتاتاً أصلاً في الثلاثة.
> **فالقسمُ يقول ذلك بنصّه على الشاشة** ولا يُقرأ فراغُه «لم يقع شيءٌ قبل
> العطب». وجمعُه في العميل بندٌ لم يُبدأ.
## تتبّعُ الأعطال أمسك أوّلَ عطبٍ حقيقيٍّ من جهازٍ ليس جهازَنا (٢٠٢٦-٠٩-٢١)

**بعد ساعاتٍ من تشغيله**، ظهر في اللوحة صفٌّ لم يصنعه مِجَسّ:

    TypeError · وعدٌ مرفوض
    Failed to fetch dynamically imported module:
      https://app.tajora.ly/assets/web-BnW104Ws.js
    المسار /  ·  القناة production  ·  بصمة الجهاز a8401cf10e00
    نظام التشغيل: Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatibl…

**والبصمةُ ليست بصمةَ جهازِ من يقيس** (`1f16d5c418e2`)، **ووكيلُ المستخدم
مقطوعٌ بشكلِ زاحف** (`compatible;`) — فالأرجحُ زاحفٌ لا إنسان، **ولا يُجزم**.

**وما يقوله الصفُّ بنفسه**: حزمةُ مسارٍ كسولةٍ لم تصل، **والمسارُ `/`** أي
عند الإقلاع، **وفُتاتٌ واحدٌ فقط** (`kind=route route=/`) بلا أيِّ نداءِ
خلفية — أي أن التطبيقَ سقط **قبل** أن يطلب `/config`.

**وهو الشكلُ المعروف لحزمةٍ كسولةٍ بعد نشرة**: صفحةٌ محفوظةٌ تشير إلى اسمِ
حزمةٍ تغيّر. **ولم يُشخَّص بعد ولم يُصلَح** — بندٌ مفتوح، ودليلُه أنّ البابَ
يعمل: أوّلُ ما أمسكه لم يكن مِجَسّاً.

### وتنقّلُ تطبيق الراكب — **قِيس فسلِم** (٢٠٢٦-٠٩-٢١)

بدا في القياس الآليِّ أن `/login → /forgot-password` يبدّل العنوانَ ولا يُعيد
الرسم. **ثمّ قِيس بنقرةٍ حقيقيّةٍ على تبويبٍ ظاهرٍ ومُركَّزٍ فرُسمت الشاشةُ
صحيحةً** (`visible` و`hasFocus: true` مقروءان من الصفحة نفسِها).

**فالعطبُ كان في أداة القياس لا في التطبيق**: النقرُ كان يقع على تبويبٍ في
الخلفية بينما العملُ في تبويبٍ آخر. **ولم يُعَد إنتاجُ التعطّل عمداً**،
فالآليّةُ **مُرجَّحةٌ لا مثبتة** — و`AnimatePresence mode="wait"` في
`ui/Motion.tsx` يحجز الشاشةَ الداخلةَ حتى تنتهي حركةُ الخارجة، وحركاتُ
framer-motion لا تتقدّم في تبويبٍ لا يُرسَم. **ويُقال إنها ترجيحٌ.**
## حزمةٌ كسولةٌ بعد رفع — **نصفان بُنيا وقِيسا** (٢٠٢٦-٠٩-٢١)

**العطبُ مقيس**: `vite build` يُفرغ `dist` فلا يبقى من البناء السابق شيء، **وتغييرُ
محرفٍ واحدٍ في نصٍّ داخل شاشةٍ واحدةٍ بدّل بصماتِ ٦٠ ملفّاً من ٦٩**. فمن فتح
التطبيقَ قبل الرفع ثمّ تنقّل بعده يطلب حزمةً باسمٍ زال ⇒ `404` من nginx ⇒
`TypeError: Failed to fetch dynamically imported module`، **والشاشةُ تبقى
والعنوانُ يتغيّر** لأن `Suspense` ينتظر وعداً لن يُحلّ.

### النصفُ الأوّل — العلّيةُ على الخادم (٧ أيام)

`scripts/deploy.sh` بعد البناء: **صورةُ الجديد وحدَه أوّلاً** ثمّ يُعاد إلى
`dist` ما في العلّية بـ`cp -an`، ثمّ يُمحى ما تجاوز سبعةَ أيام. **والترتيبُ هو
الصحّة**: لو عُكس لَالتُقط المدموجُ في الصورة التالية **فتجدّد عمرُ القديم إلى
الأبد**.

**وقِيس بالبناء مرتين**: حزمُ (د) وحدَها ٦٩ ملفّاً، وبعد العلّية **١٢٩**،
و`ForgotPassword` القديمةُ تُجيب **`200`** بعد «الرفع». **فمن كان يملأ نموذجاً
لا تُعاد صفحتُه تحته.**

> **وحدُّها مكتوب**: المحوُ يقع **عند الرفع التالي** لا بمؤقِّت — فحزمةٌ تجاوزت
> السبعةَ تبقى مخدومةً حتى يقع رفعٌ يمسحها. **ولا كنّاسَ يوميٌّ لها**، ويُقال.

### النصفُ الثاني — إعادةٌ واحدةٌ في العميل

`lib/chunk-retry.ts` في الثلاثة، **ويُغلَّف `lazy` نفسُه** لا سبعةٌ وسبعون موضعَ
نداء — **فما يُطبَّق في موضعٍ لا يُنسى في الثامن والسبعين**. والرايةُ في
`sessionStorage` (تنجو من الإعادة ولا تبقى بعد اللسان)، **مؤقّتةٌ بعشر ثوانٍ**،
**ومفتاحُها عنوانُ الحزمة الساقطة** يُقرأ من نصِّ الخطأ.

**وقِيس في المتصفّح**: حُذفت الحزمةُ ثمّ ضُغط الرابط، فسجّل الخادمُ
**`404` واحداً للحزمة، ثمّ طلبَ الصفحة مرّةً واحدة** — ولا شيءَ بعده. والرايةُ
`taxo.chunk-retry:http://…/ForgotPassword-B71hrkn-.js` بختمها. **فإعادةٌ واحدةٌ
لا حلقة.**

> **⚠ وعاملُ الخدمة يغيّر الصورة** (وُجد أثناء القياس): تطبيقُ الراكب يسجّل
> `sw.js` ومخزنَ `taxo-shell-v1`، **وأوّلُ قياسٍ لم يبلغ الشبكةَ أصلاً** لأن
> الحزمةَ جاءت من المخزن. فأُلغي العاملُ ومُسح المخزنُ ثمّ أُعيد القياس.
> **وأثرُه على الإنتاج لم يُقَس**: قد يحجب العطبَ عن بعض المستخدمين وقد لا —
> **ولا يُقرأ ذلك حمايةً حتى يُقاس**.

## شاشةُ الأعطال أُعيد بناؤها — لمحةٌ ومنحنًى وجماعيّ (٢٠٢٦-٠٩-٢١)

**بموافقة المالك على نموذجٍ عُرض أوّلاً.** بطاقاتٌ أربعٌ فوق الجدول (مفتوحة ·
جديدةٌ ٢٤س · أجهزةٌ متأثرةٌ ٢٤س مع نسبةِ التغيّر · بلاغاتٌ تنتظر)، **وكلُّها من
`services/error_stats.py`** — قاعدةُ `services/stats.py` نفسُها: لوحةٌ تجمع
صفوفَها بنفسها تكذب أوّلَ ما تُقصَّ صفحة.

**وفي الصفّ**: شارةُ الصنف · **وسمُ «جديد» و«ارتداد»** · العنوانُ **والمتَّهَم**
(عمودٌ جديدٌ `culprit` يُملأ من أعلى إطارٍ في أوّل أثرٍ يصل) · التطبيقُ شارةً ·
**منحنى ٢٤ ساعة** نسبيٌّ إلى أعلى ساعةٍ في الصفِّ نفسِه · المرات · المتأثرون ·
**ووقتٌ نسبيٌّ والدقيقُ في التلميح**.

**والارتدادُ عمودٌ لا استنتاج**: محسومةٌ عادت تقع **تُفتح من نفسها وتُوسَم**
`regressed_at`. **والمكتومةُ لا تُفتح** — الكتمُ قرارٌ قائمٌ لا تنقضه عودة،
والحسمُ دعوى إصلاحٍ تكذّبها. **ولكلٍّ اختبارُه.**

**والجماعيُّ بقيدِ تدقيقٍ لكلِّ مجموعةٍ لا قيدٌ للدفعة** — «حُسمت ٤٠» لا تجيب
«أحُسمت هذه؟» بعد شهر. **ولا `reopen` جماعيّ** بقصد.

**وأبوابُ الجملة فوق مساراتِ المعامل** — وقِيس فسقط: `POST /admin/errors/bulk/resolve`
كان يُقرأ `group_id="bulk"` فيجيب ٤٢٢.
## قراراتُ المالك ٢٠٢٦-٠٩-٢٢ — **كلمةُ Redis باقية، والربطُ مؤجَّل**

**١) كلمةُ مرور Redis لا تُدوَّر.** طُبعت نصّاً في مخرَج جلسةٍ حين قُرئ أمرُ
الخلفية بـ`docker compose config backend` — **الأمرُ يوسّع المتغيّرات ويرسم
`redis` معه لأنها من اعتمادياته**. لم تُكتب في ملفّ. **وقرّر المالكُ إبقاءها.**
**ومن الآن يُقرأ إعدادُ compose بـ`--no-interpolate` وحدَه** — لا يوسّع سرّاً.
وهو درسُ `GUARDS.md` «ولا يُسأل عن وجود سرٍّ بصيغةٍ قد تطبعه» وقع ثانيةً.

**٢) الربطُ `./backend:/app` على الإنتاج مؤجَّل.** بعد نزع `--reload` يبقى
خطرٌ أصغر: وحدةٌ تُستورد أوّلَ مرّةٍ بين `git pull` وإعادة الحاوية **تُحمَّل
من الشيفرة الجديدة داخل عمليةٍ قديمة**. ونزعُ الربط يُغلقه، لكنه يجعل الإنتاجَ
