...
**تُقرأ الثلاثة** (`HANDOFF.md` · `SPEC.md` · `CLAUDE.md`) **قبل أيِّ شيء**، ثم
يُقال للمالك: **أين نقف، وما أولُ خطوة.**
...
>
> **١٠) ولا تكتب أمرَ فحصٍ بيدك حيث للمشروع بابُه — فقد تصنع حارساً كاذباً.**
> **ووقع مقيساً 2026-08-24**: قِيس أثرُ تعديلٍ بـ`npx tsc --noEmit` فخرج
> **بصفرٍ على عطبِ نوعٍ حقيقيّ**، وأمسكه `tsc -b` الذي يشغّله البناء
> (`TS2741`). **والعلّةُ بنيويّةٌ لا عارضة**: `tsconfig.json` في التطبيقات
> الثلاثة `{"files": [], "references": [...]}` — **فالأمرُ المجرَّدُ يفحص صفرَ
> ملفّ** ولا يتبع المراجع، ووضعُ البناء `-b` وحدَه يتبعها. وهو «صفرٌ مقروءٌ
> يُقرأ سلامة» في ثوب أمرٍ يدويّ.
>
> **وقِيست أبوابُ المشروع كلُّها فوُجدت سليمة** (2026-08-24): الثلاثةُ
> تستعمل `tsc -b` في `build` و`tsc -b --noEmit` في `lint`، **ولا حارسَ فيها
> بالأمر الخاطئ**. **فالكاذبُ كان أمراً كُتب باليد، لا أداةً ناقصةً في
> المشروع** — ومنه القاعدة: **يُنادى بابُ المشروع (`npm run lint` /
> `npm run build`) ولا يُخترع أمرٌ يشبهه.**
>
