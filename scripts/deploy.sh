...
ssh_try() {
  ...
  until _ssh "$@"; do
    ...
    # **ورسالةُ الإعادة إلى stderr لا stdout** (قِيس ٢٠٢٦-٠٩-١٩): من يلتقط
    # جوابَ `ssh_try` بـ`$(…)` كان يلتقطها معه — `BUILT="(انقطاعٌ…)BUILT=4"`
    # **فقُرئ بناءٌ ناجحٌ سقوطاً** ووقف الرفعُ قبل إعادة الحاويات. والسجلُّ
    # يراها كما كان: المشغّلُ يوجّه القناتين إليه.
    say "    (انقطاعٌ — إعادةٌ $n/3 بعد ٣٥ ثانية)" >&2; sleep 35
  done
}
...
# ═══════════ العلّية: حزمُ البناء السابق تبقى سبعةَ أيام ═══════════
#
# **العطبُ مقيسٌ لا مفترَض** (٢٠٢٦-٠٩-٢١): `vite build` يُفرغ `dist`، **فلا
# يبقى من البناء السابق شيء** — قِيس أنّ كلَّ ملفّات `assets/` تحمل ختماً
# زمنيّاً واحداً هو لحظةُ الرفع. **وتغييرُ محرفٍ واحدٍ في شاشةٍ واحدةٍ يُدوِّر
# بصماتِ ٦٠ ملفّاً من ٦٩** لأن تغيُّرَ بصمةٍ يُدوِّر بصماتِ من يستوردها.
#
# **فمن فتح التطبيقَ قبل الرفع ثمّ تنقّل بعده** يطلب حزمةً باسمٍ زال، فيجيب
# nginx ٤٠٤، **ويقف التنقّلُ بلا رسالةٍ ولا بياض**: `Suspense` ينتظر وعداً لن
# يُحلّ، فالعنوانُ يتغيّر والشاشةُ لا.
#
# **وفي العميل إعادةٌ واحدة** (`lib/chunk-retry.ts`) — **وهي الشبكةُ الأخيرة
# لا الأولى**: الإعادةُ تُصلح التنقّلَ **وتُضيّع ما بيد صاحبها**، فمن يملأ
# نموذجاً أو في رحلةٍ قائمةٍ تُعاد صفحتُه تحته. **وهذه العلّيةُ تجعلها لا تقع
# أصلاً** لمن كان بين البناءين.
#
# **والترتيبُ هو الصحّة**: تُلتقط صورةُ البناء **الجديد وحدَه أوّلاً**، ثمّ
# يُعاد إلى `dist` ما في العلّية. **ولو عُكس** لَالتُقط المدموجُ في الصورة
# التالية **فتجدّد عمرُ القديم إلى الأبد** ولم تنقضِ سبعةُ أيامٍ على شيء.
#
# **و`cp -an` لا يطمس**: الجديدُ يفوز عند تطابق الاسم، والقديمُ يُضاف فقط حيث
# لا بديلَ له. **و`-a` يحفظ الختمَ الزمنيّ** فالعمرُ يُقاس بالحقيقة.
say "  العلّية : حزمُ البناء السابق تبقى ٧ أيام…"
ATTIC_REPORT="$(_ssh "cd $REMOTE && bash -s" <<'ATTIC_EOF' 2>&1 || true
set -u
KEEP_DAYS=7
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
for a in customer-app driver-app admin-panel site; do
  d="./$a/dist/assets"
  [ -d "$d" ] || continue
  attic="./.attic/$a"
  mkdir -p "$attic/$STAMP"
  # ١) صورةُ الجديد وحدَه — **قبل الدمج**
  cp -a "$d/." "$attic/$STAMP/" 2>/dev/null || true
  fresh="$(ls -1 "$d" 2>/dev/null | wc -l)"
  # ٢) يُعاد المحفوظُ حيث لا بديلَ له — الأحدثُ أوّلاً فلا يطمسه أقدمُ منه
  for old in $(ls -1d "$attic"/*/ 2>/dev/null | sort -r); do
    [ "$old" = "$attic/$STAMP/" ] && continue
    cp -an "$old". "$d/" 2>/dev/null || true
  done
  total="$(ls -1 "$d" 2>/dev/null | wc -l)"
  # ٣) وما تجاوز المدّةَ يُمحى — **والقياسُ على ختم الصورة لا على الدمج**
  gone="$(find "$attic" -mindepth 1 -maxdepth 1 -type d -mtime "+$KEEP_DAYS" 2>/dev/null | wc -l)"
  find "$attic" -mindepth 1 -maxdepth 1 -type d -mtime "+$KEEP_DAYS" -exec rm -rf {} + 2>/dev/null || true
  kept="$(ls -1d "$attic"/*/ 2>/dev/null | wc -l)"
  echo "    · $a: $fresh جديدة + $((total - fresh)) محفوظة = $total · صورٌ $kept · مُسح $gone"
done
echo "    · حجمُ العلّية: $(du -sh ./.attic 2>/dev/null | cut -f1)"
ATTIC_EOF
)"
[ -n "$ATTIC_REPORT" ] && say "$ATTIC_REPORT"
# **ولا تُسقط العلّيةُ رفعاً**: هي تحسينُ نجاةٍ لا شرطُ صحّة — ورفعٌ يقف لأن
# نسخةً احتياطيةً من حزمٍ قديمةٍ تعذّرت يمنع إصلاحاً هو الأهمّ.
...
# **وكلُّ حاويةٍ تربط ملفَّ إعدادٍ من القرص تُعاد صراحةً** — لا النفقُ وحدَه.
#
# **العلّةُ عُرفت في 2026-08-25 وطُبّقت على واحدةٍ فقط**: ملفُّ التوجيه
# `cloudflared/config.prod.yml` **مربوطٌ من القرص**، و`up -d` لا يعيد حاويةً لم
# يتغيّر **تعريفُها** — فيبقى القديمُ حيّاً في الذاكرة والجديدُ على القرص،
# **ولا يشكو شيء**. لا رسالةَ خطأ، ولا فرقَ يُرى إلا حين يُطلب شيءٌ جديد.
#
# **وربطُ ملفٍّ مفردٍ يمسك عقدتَه لا مسارَه**: `git pull` يكتب ملفّاً جديداً
# بعقدةٍ جديدة، **والحاويةُ تبقى على العقدة التي وُلدت عليها** — فلا يبلغها
# التغييرُ ولو أُعيد تشغيلُ nginx من داخلها.
#
# **ووقع مقيساً على الإنتاج ٢٠٢٦-٠٩-٠٥**: `landing/nginx.conf` كُتب فيه وكيلُ
# `/api/` وصفحةُ ٤٠٤، **والحاويةُ لم ترَ منهما شيئاً** — صفرُ `error_page`
# وصفرُ `location /api/` داخلها بعد رفعةٍ خضراءَ في بوّاباتها الستّ.
# **فالموقعُ كان يُخدَم بقيَمه الافتراضية**: لا إعداداتِ موقعٍ ولا سطرَ عرض،
# **ولا شيءَ يقول ذلك** — وهو الشكلُ العاشر بعمودٍ خامس، عند الحافّة.
#
# **والدرسُ أن الدرسَ نفسَه لم يُعمَّم**: عُولج `cloudflared` وحدَه، ثمّ جاء
# الموقعُ بملفٍّ ثانٍ من الشكل نفسِه — **والتطبيقاتُ الثلاثةُ تربط `spa.conf`
# منذ البداية**. فالقائمةُ تُذكر كلُّها، **لا التي تذكّرناها**.
#
# **وثمنُه ثوانٍ من انقطاعٍ في كلِّ رفعة** — أرخصُ من إعدادٍ بائتٍ لا يعرف
# أحدٌ أنه بائت. و`|| true` لأن الخدماتِ غيرُ معرَّفةٍ في التشغيل المحلّيّ،
# **وغيابُها ليس عطباً هناك**.
...
RESULT="$(_ssh "cd $REMOTE && \
  ...
  docker compose $COMPOSE_FILES up -d --build >/dev/null 2>&1 &&   { docker compose $COMPOSE_FILES restart $CONF_MOUNTED >/dev/null 2>&1 || true; } && \
...
# ═══ والحافّةُ تُسأل عمّا وعدت به، لا عن حياتها ═══
#
# **العلّةُ مقيسةٌ على الإنتاج ٢٠٢٦-٠٩-٠٥**: خضّرت البوّاباتُ الستُّ كلُّها —
# الإيداعُ والترحيلةُ والحاوياتُ والصحّةُ والسجلّ — **والموقعُ يردّ ٤٠٤ على
# `/privacy` ويُخدَم بلا وكيل `/api/`**. **فكلُّ ما قِيس كان صحيحاً، وما لم
# يُقَس هو ما سقط.**
#
# **وسؤالان لا يُجيبهما «أتعمل الحاوية؟»:**
#  · **أيصل المسارُ إلى ملفّه؟** — `/privacy` ملفٌّ مبنيٌّ لا مسارُ تطبيق.
#  · **أيعمل وكيلُ `/api/`؟** — بدونه تُخدَم الصفحةُ بقيَمها المخبوزة
#    **وتبدو سليمة**: لا إعداداتِ موقعٍ ولا سطرَ عرض، **ولا رسالةَ خطأ**.
#
...
printf '%s' "$_API" | grep -q '"offer"\|"commission_percent"\|"apps"' \
  || die "وكيلُ /api/ لا يجيب من $EDGE — **الصفحةُ تُخدَم بقيَمها المخبوزة ولا تشكو**. الردّ: ${_API:0:120}"
say "  ✓ الحافّة: وكيلُ /api/ يجيب من الأصل نفسِه"
