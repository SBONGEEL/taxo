...
# ── ٤) **ملفُّ النتيجة يُمسح قبل التشغيل** ────────────────────────────────
# **الحكمُ يُقرأ من هذا الملفّ** (`grep -c '^EXIT=0$'`) — **ولم يكن يُمسح**،
# فتشغيلٌ يسقط **قبل أن يكتب حرفاً** يقرأ حكمَ التشغيل الذي قبله ويعلن
# «خضراء». **وهو بابٌ يشهد لما لم يقع.**
#
# **ووقع مقيساً 2026-08-24**: كُسر أمرُ الحاوية بتعليقٍ في غير موضعه، فلم
# يعمل شيء — **وأعلن البابُ خضرةً محلياً** لأن الملفَّ يحمل `EXIT=0` من
# تشغيلٍ سابق، **وسقط في CI** حيث الاستنساخُ نظيفٌ ولا ملفَّ فيه.
# **فالخضرةُ الكاذبةُ كانت على جهاز المطوّر وحدَه** — وهو أخطرُ موضعٍ لها،
# لأنه الموضعُ الذي يُقرَّر فيه الدفع.
: > "$OUT"

# **`python -u` لا `pytest` مباشرةً**: المخرَجُ المُعادُ توجيهُه إلى ملفٍّ
# يُخزَّن كتلاً، **فمهلةٌ تنتهي تترك ملفاً فارغاً** ولا يُعرف أين وقف — وقع
# مقيساً في التشغيل ٤٨: ثلاثون دقيقةً ثم قطعٌ بلا سطرٍ واحد.
#
