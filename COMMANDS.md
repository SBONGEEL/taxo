...
<!--جديد-->
## أداتان تعملان على ويندوز والشجرةُ في WSL (٢٠٢٦-٠٩-٠٥)

**Chrome مركَّبٌ على ويندوز والشجرةُ في WSL**، **وPlaywright يقود المركَّب لا
متصفّحاً يُنزَّل** — فالأداتان تعيشان في الشجرة **وتُشغَّلان من جزيرةٍ فيها
`node_modules`**، ومعهما `TAXO_ROOT` يقول أين الشجرة. **ولا تُخمَّن**:

    TAXO_ROOT='\\wsl.localhost\Ubuntu-24.04\home\<you>\prj\TAXO'
    node tools/capture-screens/capture.mjs  <مجلَّد المخرَج>
    node tools/check-mobile/check-mobile.mjs [--negate]

| الأداة | تقيس | ولا تقيس — **وتقول ذلك** |
|---|---|---|
| `capture-screens` | **الحزمةُ المخدومةُ = `dist`** · **وعلامةٌ تخصّ الشاشة** · **وزوالُ الدوّارة** | **صحّةَ الشاشة بصرياً** — تقول إنها **التي طُلبت وقد اكتملت** لا إنها سليمة |
| `check-mobile` | ٤ مقاسات × ١٠ مسارات: تمريرٌ أفقيّ · عنصرٌ خارجَ الإطار · لمسٌ دون ٤٤ · تقاطعُ اثنين · محتوىً تحت شريطٍ ثابت · وأثرُ لوحة المفاتيح | **خلايا الجداول التي تُمرَّر أفقياً** — تُعدّ وتُقال **ولا تُسقط**، ورفعُها إلى ٤٤ إعادةُ تصميمٍ لا إصلاحُ تخطيط |

**وكلاهما يحتاج البيئةَ قائمةً**: التطبيقاتُ الثلاثةُ على `5176`/`5174`/`5175`
**من `docker port` لا من العُرف** — و`5173` منفذُ الحاوية **من داخلها**، ومن
قصده من المضيف أصاب خادماً أجنبيّاً.

**ولذلك ليسا في `npm run` ولا في CI**: حارسٌ يُشغَّل حيث لا يملك ما يقيسه
**يعلن «لم يُقس» لا «سليم»** — وCI بلا بيئةٍ حيّةٍ ولا Chrome.

**و`--negate` يحقن العطبَ المقيس** (ترويسةٌ لا تلتفّ وأزرارٌ ٣٤) **ويسقط إن
لم يصح الحارس** — فبابٌ يصمت دائماً ليس باباً.
<!--/جديد-->

<!--جديد-->
## `scripts/deploy.sh` على هذا الجهاز — **يُشغَّل من Git Bash لا من WSL** (قِيس ٢٠٢٦-٠٩-١٠)

**والسببُ مقيسٌ لا مُقدَّر**: الشجرةُ في WSL، **و`ssh.exe` الخاصُّ بويندوز هو
العميلُ الوحيدُ الذي يملك المفتاح** (المفتاحان `taxo-contabo` و`taxo-deploy`
**كلاهما بعبارة مرور**، ووكيلُ WSL فارغ). **وجسرُ WSL→`ssh.exe` يموت في منتصف
تدفّقٍ ثنائيٍّ كبير** — أي في نقل `bundle.tar` بالبوّابة الثالثة.

**وثلاثةُ وجوهٍ لسقوطٍ واحد، وقعت كلُّها في جولةٍ واحدة:**

| ما ظهر | ما هو |
|---|---|
| تعلُّقٌ سبعَ دقائقَ على `SELECT version_num` بلا مهلةٍ تقطعه | `ssh.exe` بلا طرفيّةٍ (`setsid`) ينتظر وحدةَ تحكُّمٍ لا وجودَ لها |
| `bundle.tar` **بزيادةِ ١٫٨٤ ميغا** ثم «Skipping to next header» | حشوٌ في التدفّق — **لا بترة**، ولا قلبَ نهاياتِ أسطر (CR = 0.37% = المعدّلُ الطبيعيّ) |
| توقّفٌ عند 1,912,832 بايتاً **ولا عمليةَ `ssh` قائمة** | الجسرُ مات وترك `bash` ينتظر ابناً لن يعود |

**وعُزل المتغيّر**: وُجِّهت النسخةُ إلى ext4 داخل WSL (`TAXO_BACKUP_DEST`)
فسقطت كذلك — **فليست 9p على `/mnt/d`، بل الجسر**. ومن Git Bash تمَّ النقلُ
نفسُه **في دقيقتين**.

**وشرطان لا واحد** لتشغيله من Git Bash:

1. **`core.fileMode` يُعطَّل بالبيئة لا بتعديل إعداد** — Git Bash يقرأ شجرةَ
   WSL **«٥١ تغييراً» وهي نظيفة** (بتُّ التنفيذ وحدَه)، **والبوّابةُ الأولى
   تسقط بقراءةٍ كاذبة**. و`git -c` لا ينفع لأن السكربت ينادي `git` مباشرةً:

       export GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=core.fileMode GIT_CONFIG_VALUE_0=false

2. **ويُطلق عمليةَ ويندوزَ مستقلّةً** خارج متتبِّع المهامّ — **وإلا قتله ضغطُ
   الذاكرة في منتصف البوّابة الخامسة** (وقع: متاحٌ ٥٩٧ ميغا من ١١,٩٩٢، والحالُ
   بعده **نصفُ مطبَّقة**: الخادمُ سحب، والحاوياتُ لم تُعَد).

       Start-Process bash.exe <ملفُّ مشغِّل>   # **ملفٌّ لا سطرٌ مقتبَس**

   **ووسائطُ `Start-Process` تُعاد اقتباسُها فتكسر السطر** — جُرِّب سطرٌ فيه
   `&&` و`>` فمات صامتاً بلا سجلّ.

**⚠ ولا تُمرَّر مسارات WSL من Git Bash إلى `wsl.exe`**: MSYS يحوّل
`/home/loly3/...` إلى `C:/Program Files/Git/home/loly3/...` — **فأنشأ مجلَّد
`C:` داخل الشجرة** وأوسخها. يُصرَّح المتغيّرُ **داخل** `bash -lc` لا خارجه.

**وهذه قواعدُ كانت تُتذكَّر — والتي تُتذكَّر تُنسى. فصارت شرطاً يُطبَّق**
(2026-09-11): **`scripts/deploy-here.sh`** يضبط الاثنين ويرفض ما عداهما، ثم
ينادي `deploy.sh` نفسَه.

    bash scripts/deploy-here.sh        # من Git Bash وحدَها

**ويرفض بنصِّه**: من WSL يخرج `1` بعلّة الجسر؛ وبشجرةٍ ليست نظيفة يخرج `1`
ويسرد الأسطر. **وأثرُ ضبطِه مقيس**: `git status --porcelain` من Git Bash على
شجرة WSL **بلا ضبطٍ ٥٣ سطراً، ومعه سطران**.

**⚠ ولم يُقَس رفعٌ كاملٌ خلاله بعد** — قِيست مساراتُ رفضِه وحدَها، والحدُّ
مكتوبٌ في رأس الملفّ.
<!--/جديد-->

<!--جديد-->
## ⚠ والعطبُ الثالثُ يقتل الرفعَ في **الوضعين معاً** — **تصحيحٌ يعلو ما فوقه** (قِيس ٢٠٢٦-٠٩-١٨)

**السطرُ الذي فوق هذا يقول**: «يُطلق عمليةَ ويندوزَ مستقلّةً خارج متتبِّع
المهامّ — **وإلا** قتله ضغطُ الذاكرة». **والقياسُ أسقط شِقَّ «وإلا»**:
**العمليةُ المستقلّةُ قُتلت ثلاثَ مرّاتٍ من أربع.** فالاستقلالُ **لا يحمي من
القتل**، وإنما يُخفي سببَه.

### الأربعُ، بأوقاتها ووضعِ إطلاقها

| السجلّ | الإطلاق | ما وقع |
|---|---|---|
| `deploy-20260918T010630Z` | مستقلّة | صمت بعد `01:09:23Z` — والبناءُ تمّ على الخادم `01:16Z` |
| `deploy-20260918T022746Z` | مستقلّة | **نجا، وخضّرت الستُّ كلُّها** |
| `deploy-20260918T025646Z` | مستقلّة | صمت بعد `03:19:16Z` — و`BUILT=4` على الخادم |
| `deploy-20260918T035753Z` | مستقلّة | صمت بعد `04:00:41Z` |
| مهمّةٌ يتتبّعها الهيكل | **متتبَّعة** | **قُتلت، وقيل السببُ بنصّه** |

**والموضعُ واحدٌ في الأربع**: بعد إطلاق بناء الواجهات، **في حلقة الاستطلاع**،
**والبوّاباتُ ١-٥ قد خضّرت** — فتسقط السادسةُ وحدَها.

### والفرقُ الحقيقيُّ بين الوضعين: **مَن يقول السبب**

**المستقلّةُ تموت صامتة**: السجلُّ يتوقّف عند آخر سطرٍ ولا رسالةَ ولا رمزَ
خروج، **فيُقرأ الصمتُ «ما زال يبني»** — وهو أخطرُ من سقوطٍ معلَن.
**والمتتبَّعةُ تموت ناطقة**، وبنصّها:

    Background command … was stopped because the system is running low on memory

**فالمتتبَّعةُ ليست الخطأ — هي التي كشفت الخطأ.** وثلاثُ جولاتٍ صامتةٍ سبقتها
ظُنَّ في كلٍّ منها ظنٌّ آخر.

### العلاجُ الحقيقيُّ: الذاكرةُ قبل الإطلاق

**المتاحُ لحظةَ القتل: ٦٦٧ ميغا من ١١,٩٩٢** — وفي الجولة المسجَّلة قبلها ٥٩٧.
**فالرقمُ الذي يُقاس قبل كلِّ رفع**، ويُفرَّغ ما يشغله:

    powershell -NoProfile -Command "(Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory/1KB"

**ولا يُقرأ الإطلاقُ المستقلُّ ضماناً**: هو شرطٌ لأسبابٍ أخرى مكتوبةٍ فوق
(اقتباسُ الوسائط وبيئةُ `git`)، **وليس درعاً أمام قاتل الذاكرة**.

### وحين تقع: **تُقاس السادسةُ بيدٍ، وتُقال «لم يطبعها السكربت» لا «خضراء»**

**والفرقُ ليس لفظاً**: البوّابةُ حكمٌ يُصدره البابُ، **وما قاسه إنسانٌ بيده
مقياسٌ لا حكم** — فيُسمّى باسمه (قاعدةُ «حارسٌ يُشغَّل حيث لا يملك ما يقيسه
يعلن *لم يُقس* لا *سليم*»). وما يُقاس، سبعةٌ:

1. **إيداعُ الخادم = الرأس** — `git -C ~/taxo rev-parse HEAD` مقابلَ المحلّيّ.
2. **الحاويات** — عددُها، **وصفرُ متوقّفة**.
3. **الصحّة على النطاقين** — `stg-api` و`api`، **ويُقرأ `"environment"` لا
   الرمزُ وحدَه**: الخادمُ يعلن `production` وجهازُ التطوير `development`.
4. **`alembic current`** = رأسُ الشجرة.
5. **صفحاتُ السياسة الأربع** — الطولُ فوق ٤٠٠٠ بايت، **ونسختُها المخبوزةُ
   مقابلَ المنشورة** (`data-policy-version` مقابل `/api/policy`).
6. **وكيلُ `/api/` يجيب من الأصل** — `‎/api/landing?country_code=JO`.
7. **مسحُ السجلّ** — `docker compose logs --since <مدّة>` على الخدمات السبع.

**وما لا يُقاس بهذا كلِّه فيبقى مقولاً**: **المسارُ الحيُّ من متصفّح**
و**الأعمدةُ الأربعةُ للثلاثة** — وهما مكتوبان في ذيل البوّابة نفسِها.
<!--/جديد-->

<!--جديد-->
## `scripts/dev-up.sh` — **يُشغَّل بعد كلِّ إعادة تشغيل** (قرارُ المالك ٢٠٢٦-٠٩-١٢)

    bash scripts/dev-up.sh

**هذا أوّلُ أمرٍ بعد إقلاع الجهاز أو إقلاع Docker Desktop** — قبل أيِّ بناءٍ
وقبل أيِّ قياسٍ على هاتف. **ولا يمسّ ما يعمل**: يُقيم الناقصَ ولا يُعيد القائم.

### ولمَ بابٌ لا أمرُ `up -d` عارياً

**سقطت الحزمةُ التجريبيةُ على الهاتف ثلاثَ مرّاتٍ في يومٍ واحد، بثلاثة وجوه
ولسببين عارضين وواحدٍ دائم** (قِيس ٢٠٢٦-٠٩-١١/١٢):

| ما ظهر | ما هو | أيعود وحدَه؟ |
|---|---|---|
| `dev-*` تردّ `530 · error code: 1033` | `taxo-tunnel` خرج `(255)` منذ ٤١ ساعة | **عارض** |
| `dev-*` تردّ `502` | حاويتا الواجهة `Restarting (127)` · `sh: npm: not found` بعد إقلاع Docker | **عارض** |
| «تعذّر الوصول إلى الخدمة · `ERR network_unreachable`» | `VITE_API_BASE_URL: http://127.0.0.1:8001` — **وهو الهاتفُ نفسُه** | **دائم — أُصلح في compose** |

**والثلاثةُ تُقرأ عَرَضاً واحداً على الشاشة**، فيُصلَح العارضُ ويُظنّ الأمرُ
انتهى — **ثمّ يعود المشهدُ نفسُه لأن الدائمَ لم يُمسّ**. وهذا بعينه ما جعله
يبدو «يتكرّر كلَّ مرّة».

> **ورسالةُ CORS التي ظهرت أوّلاً لم تكن CORS**: حافّةُ Cloudflare تردّ `530`
> **بلا ترويسة `Access-Control-Allow-Origin`** حين لا نفقَ خلفها، فيسمّي
> المتصفّحُ العطبَ بأقربِ عَرَضٍ يراه. **نصٌّ واحدٌ لعلّتين** — والتعليقُ في
> `backend/app/core/config.py` يوثّق العلّةَ الحقيقيةَ الأولى (٢٠٢٦-٠٩-٠٩)
> فيُقرأ تشخيصاً لغيرها.

### وما يقيسه — **الجوابُ لا السجلّ**

1. **حالُ كلِّ حاوية بعد ١٢ ثانية** لا لحظةَ الأمر — **ودورةُ السقوط لا تُقرأ
   في الثانية الأولى**.
2. **جوابُ النطاقات الأربعة** — `530` يُسمّى «النفقُ غيرُ متّصل» و`502` «النفقُ
   حيٌّ وما خلفه ساقط»، **فلا يُخلط العَرَضان**.
3. **وما يُخدَم للهاتف**: `VITE_API_BASE_URL` مقروءاً من الحاوية — **ويسقط
   الفحصُ إن كان `127.0.0.1`** ولو كان كلُّ شيءٍ آخرَ أخضر.

**ولا يُقرأ خروجُ `docker compose up -d` بصفرٍ حياةً**: يخرج بصفرٍ وحاويةٌ تدخل
دورةَ سقوطٍ بعده بثوانٍ. **فالدليلُ جوابُ النطاقات وحدَه.**

### ومُبدَّلٌ واحدٌ لمن يطوّر بلا نفق

    TAXO_DEV_API_BASE=http://127.0.0.1:8001 docker compose --env-file .env.local up -d

**ولا يُحرَّر `docker-compose.yml`** — تحريرُه يركب إيداعاً لا شأنَ له به.
<!--/جديد-->
