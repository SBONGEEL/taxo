...
**والإصلاحُ المقترح — لا منفَّذ**:
1. **في الشيفرة لا في الاختبارات، وهو الأصحّ**: `leeway` صغيرٌ مكتوبٌ في `decode_token` (ثوانٍ لا دقائق). لأن الإنتاجَ نفسَه يسكّ ويتحقّق بساعةٍ قد يصحّحها NTP إلى الوراء — **فالعطبُ ليس في الاختبارات بل يظهر فيها أوّلاً**. وكلفتُه: سطرٌ واحد، وتوسيعٌ لنافذة الرمز المنتهي بالقدر نفسه.
2. **في الاختبارات**: مُثبِّتٌ يثبّت الزمنَ الذي يقرؤه PyJWT (`freezegun` أو ما يعادله) — **يُخفي الشكلَ ولا يعالجه**، ويُسقط اختباراتٍ تقيس الزمنَ فعلاً (المهل، والانتهاء).
3. **لا يُقترح**: إعادةُ سكّ الرمز عند الفشل داخل مُثبِّت العميل — **يُخفي كلَّ `invalid_token` حقيقيٍّ بعده**، وهو ما رفضه تعليقُ `admin_headers` نفسُه («ولا يُعاد سكُّ رمزٍ عند الفشل»).
<!--/جديد-->
