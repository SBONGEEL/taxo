...
### ⚠ وما قِيس بعد الرفع — **القاعدةُ تعمل عند الحافّة، ولا تبلغ رأسَ المتصفّح** (2026-09-22)

**يُكتب لأن السطورَ فوقه توحي بأن القاعدةَ حسمت الأمر، ولم تحسمه كلَّه.**

    الأصل (قِيس محلياً بـnginx)       Cache-Control: no-cache · no-cache, must-revalidate
    عبر الحافّة، طلبٌ مكسورُ الكاش     Cache-Control: max-age=14400, must-revalidate · MISS
    index.html للمقارنة              no-cache · no-cache, must-revalidate · DYNAMIC

**فCloudflare تُعيد كتابةَ رأسِ `.js` للمتصفّح** إلى أربع ساعات، **وتترك `.html`**
— لأن `.js` من امتداداتها المخزَّنة افتراضاً و`.html` ليس منها. **وإعدادُ
المنطقة ليس لنا** بقرار المالك.

**لكنّ الحافّةَ نفسَها صارت تسأل الأصلَ كلَّ مرّة**: `driver.tajora.ly/sw.js`
بعد أن خُزِّن تحت القاعدة الجديدة — ثلاثةُ طلباتٍ متتالية **`REVALIDATED`**
كلُّها. **و`app.tajora.ly` ما زال يخدم نسخةً خُزِّنت قبل القاعدة** (`HIT`، عمرٌ
3036 من 14400) — **وتلك النسخةُ هي العاملُ الجديدُ أصلاً**، فلا يتأخّر إلا
التعديلُ **التالي** حتى تنتهي.

**وما لم يُقس**: أن المتصفّحَ يتجاهل `max-age=14400` حين يفحص تحديثَ عامله.
المواصفةُ تقول إن فحصَ السكربت الرئيس يتجاوز كاشَ HTTP افتراضاً
(`updateViaCache: 'imports'`) **ويقصّ أيَّ عمرٍ عند 24 ساعة** — **وهذا نصٌّ لا
قياس**. ولا يُقرأ سكوتُه ضماناً حتى يُقاس على جهاز.

**فالدرسُ الثاني**: **الأصلُ يقول، والحافّةُ تُعيد الصياغة** — ورأسٌ قِيس عند
الأصل ليس رأساً قِيس عند المتصفّح. **يُقاس كلٌّ في موضعه**، وهي الطبقاتُ الأربعُ
في الشكل الثلاثين بعينها.
<!--/جديد-->
