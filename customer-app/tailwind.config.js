/** نظام التصميم — نسخةٌ حرفية من `design/DESIGN.md` §6 (المرحلة 12-أ).
 *
 * هذا الملف كان مختلفاً: بُني التطبيق في المرحلة 9 **قبل** أن يوجد
 * `DESIGN.md`، فبقي على سلالم Tailwind الافتراضية (`text-sm`، `p-4`) وعلى
 * لوحةٍ من ثلاثيّات rgb. والترحيلُ **بصريٌّ بحت**: لا سطرَ منطقٍ تغيّر.
 *
 * ويبقى فرقٌ واحدٌ عن نسخة الكبتن واللوحة، وهو سلوكٌ قائمٌ لا انحراف:
 * **النهاري هو الافتراضي هنا** (`:root` نهاري و`.dark` يبدّله) لأن هذا
 * التطبيق يتبع تفضيل النظام منذ المرحلة 9، بينما تطبيق الكبتن ليليٌّ ثابت.
 */
...
export default {
  ...
  theme: {
    ...
    spacing: {
      ...
      menu: "240px", // أقصى ارتفاعٍ لقائمة اقتراحاتٍ منسدلة — أربعةُ صفوفٍ ثمّ تمرير
      list: "320px", // أقصى ارتفاعٍ لجدولٍ تابعٍ تحت جدول الصفحة — سبعةُ صفوفٍ ثمّ تمرير
    },
    transitionDuration: {
    ...
    extend: {
      ...
      keyframes: {
        // حلقةُ البحث عن كبتن — حركةٌ يملكها هذا التطبيق وحده (القسم 11.4)
        pulseRing: {
