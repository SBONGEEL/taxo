...
const UI_UNIONS = new Set([
  ...
  "State",
  // اختيارُ المظهر: حالةُ عرضٍ في `localStorage` لا عمودَ في القاعدة
  "ThemeChoice",
  // قنواتُ شاشة الشحن: `manual` **تسميةُ شاشة** لا قيمةَ مزود — تصف
  // «حوّل ثم أدخل المرجع»، وما يُرسل للخلفية `method: "cliq"` نفسه.
  // والقناتان تختلفان في الخطوات لا في القيمة المخزَّنة
  "Channel",
  // نغماتُ الهوية الصوتية (§9): أسماءُ **لحظاتٍ في الواجهة** لا قيمَ خلفية —
  // «قبولُ كبتن» لحظةٌ يسمعها الراكب، و`accepted` في `RideStatus` حالُ صفٍّ في
  // القاعدة. تشابهُ بعض الأسماء مصادفةٌ لا مرآة
  "Cue",
  // مآلُ ضغطِ الصورة قبل الرفع (`lib/shrink.ts`): أربعُ حالاتٍ **تُعرض في
  // سطرٍ واحدٍ لصاحبها** ولا يخرج منها شيءٌ على السلك — لا صفَّ في القاعدة
  // ولا حقلَ في أيِّ حمولة.
  // **ويُعلَن هنا لأنه اتحادٌ مختلط**: `failed` قيمةٌ تعرفها الخلفيةُ في
  // تعدادٍ آخر، والثلاثةُ الباقيةُ لا — وهو بعينه الشكلُ الذي بُني الحارسُ له
  // (درسُ `SettlementState.none`). فالإعلانُ تصريحٌ بأنه واجهةٌ لا مرآة.
  // **والنصُّ هو نصُّ تطبيق الكبتن حرفاً** لأن الاتحادَ هو هو (نفسُ العقد)
  "ShrinkOutcome",
]);
