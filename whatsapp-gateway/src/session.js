...
class Session {
  ...
  async checkNumber(to) {
    ...
    // **الوجهةُ تُعاد مع الجواب لا تُترك في نطاقٍ آخر** (عطبٌ مقيسٌ
    // ٢٠٢٦-٠٩-٠٩): حين فُصل هذا الفحصُ عن `sendCode` (2026-08-31) بقي
    // `known.jid` في `sendCode` **و`known` انتقل إلى هنا** — فصار
    // `ReferenceError: known is not defined` على **كلِّ إرسالٍ حقيقيّ**.
    //
    // **ولم يظهر تسعةَ أيام** لأن الحاويةَ كانت تخدم صورةً أقدمَ من الشجرة،
    // **فأوّلُ بناءٍ شحن العطبَ الساكن**. وهو الشكلُ العاشرُ مقلوباً: لا
    // «الشجرةُ أحدثُ فالخادمُ متأخّر»، بل **«الخادمُ متأخّرٌ فالعطبُ مستور»**.
    //
    // **وواتساب يعيد الوجهةَ أدقَّ ممّا نبنيه** (رقمٌ مُوجَّهٌ إلى `@lid` أو
    // إلى حسابٍ تجاريّ)، فتُفضَّل وتُبقى على المبنيّ عند غيابها.
    return { digits, jid: known.jid || jid };
  }
  ...
  async sendCode(to, code, ttlMinutes, text = null) {
    ...
    try {
      await this._sock.sendMessage(
        // **الوجهةُ من `checkNumber`** — ولا متغيّرَ من نطاقٍ آخر
        jid,
