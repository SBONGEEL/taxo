...
_ACCEPTED: dict[str, str] = {
    ...
    # **وموضوعُه قاعدةُ العدّ لا مسارُ الشراء** (البند ٥٥، §55٫٤): المقيسُ
    # **أن صفّاً ملغىً يبقى محسوباً** فلا يُشترى العرضُ ثانيةً — وهو حكمٌ
    # على `_uses_by_driver` لا على بابِ الشراء. **والمرورُ بالمسار الحقيقيّ
    # يحتاج شراءً ثمّ إلغاءً بمشرفٍ ومحفظة**، فيقيس ثلاثةَ أبوابٍ ليشهد
    # على عدٍّ واحد — **واختبارٌ يقيس ما لا يقصده يسقط لأسبابٍ ليست موضوعَه**.
    "test_offer_commission.py:173": "صفٌّ ملغىً لقياس العدّ — والمقيسُ قاعدةُ العدّ لا الشراء",
