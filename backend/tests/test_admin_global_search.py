...
async def test_the_global_search_never_outreaches_the_lists_it_summarises(
    ...
    **والقياسُ اليومَ يقول إن الأربعَ `StaffUser` بلا صلاحيةٍ مسمّاة** — فهذا
    البابُ يعطي ما تعطيه هي حرفاً. **ومن حرس إحداها غداً يسقط هنا** بنصٍّ يقول
    ماذا يُفعل، فلا يبقى البابُ يعرض ما صارت هي تمنعه.
    """
    await rider_session(client)
