...
async def _has_qualifying_subscription(
    ...
    **ولا تُقرأ الحالةُ لتقرير «أانقضى؟»**: كنسُ الاشتراكات يعلّم المنتهيَ كل
    ...
    **لكنّ الملغى يُستثنى** (قرارُ المالك 2026-09-02، §38): وهو **ليس نقضاً
    للسطر فوقه بل تطبيقٌ لنصِّ هذه الدالّة نفسِها** — «**المقيسُ مدّةُ عملٍ
    اكتملت**»، **والملغى لم يكتمل**: أوقفناه نحن ورددنا مالَه، **فمن رددنا له
    مالَه لم يشترِ دورةَ عمل**. والساعةُ وحدَها لا تفرّق الاثنين: صفٌّ أُلغي
    اليومَ يمرّ تاريخُ انتهائه الأصليِّ بعد أسبوع، **فيُقرأ دورةً اكتملت وهو
    مالٌ عاد إلى صاحبه**.

    **والاستثناءُ هنا لا في مستدعٍ** (شرطُ المالك): شرطٌ ثانٍ بجانب هذه
    الدالّة يُنسى في ثاني مستدعٍ يُضاف، **وهذه هي البيت**.
    ...
    return (
        await session.scalar(
            ...
            .where(
                ...
                DriverSubscription.status != SubscriptionStatus.CANCELLED,
            )
            .limit(1)
        )
    ) is not None
