    ...
    *,
    ...
    await _safe_notify(
        ...
        message=PushMessage(
            ...
            high_priority=True,
            # **قناةُ الطلبات لا القناةُ الافتراضية** (قرارُ المالك 2026-08-30):
            # صوتُها على **مجرى المنبّه** فتصيح ولو كان الهاتفُ صامتاً، واهتزازُها
            # نبضٌ مزدوجٌ يُميَّز بلا نظرٍ إلى الشاشة. **والاسمُ يطابق
            # `OfferAlert.CHANNEL_ID` حرفاً** — واختلافُ حرفٍ يُسقط الإشعارَ
            # إلى قناةٍ افتراضيةٍ صامتةٍ بلا خطأٍ يظهر.
            android_channel_id="taxo.offer",
        ),
    )
...
async def publish_admin_message(
    session: AsyncSession,
    redis: Redis,
    *,
    user_id: uuid.UUID,
    ...
    """رسالةٌ يكتبها مشرفٌ إلى حسابٍ بعينه — **البند ١١ (§39٫١١، §46)**.

    **وتمرّ من هذا الباب لا من مسارٍ جديد** (شرطُ المالك بنصِّه: «يمرّ بمسار
    FCM القائم لا بمسارٍ جديد»): **صندوقُ الوارد ثمّ Push لمن كان تطبيقُه
    مغلقاً**، وتعطيلُ الرموز الميتة، وابتلاعُ عطبِ المزوّد — كلُّها من
    `_safe_notify` نفسِها. **ومسارٌ ثانٍ كان سيعيد كتابة الأربعة** ويفترق عنها
    أوّلَ تعديل.

    **وهي والحملةُ استثناءُ قاعدةِ «لا جملةَ مؤلَّفةٌ في `data`»**: الجسدُ هنا
    **هو المحتوى** الذي كتبه إنسان، لا رقمٌ يُنسَّق في الشاشة. **و`data` تبقى
    خاليةً إلا من النوع** — فما يمرّ بخوادم المزوّد لا يحمل شيئاً عن الحساب.

    **ولا تقرأ `marketing_push_enabled`**: هذه رسالةٌ تشغيليةٌ إلى شخصٍ بعينه
    كإشعار الوثيقة والدفعة، **لا حملةً تسويقية** — ومن أطفأ الإعلانَ لم يطفئ
    أن يُبلَّغ بما يخصّ حسابَه. **وبابُ الحملات هو من يقرأ ذلك المفتاح**.
    """
    await _safe_notify(
        session,
        redis,
