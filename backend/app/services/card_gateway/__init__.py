...
def return_url_for(cart_id: str, *, opened_from: str) -> str:
    ...
    **ولكل تطبيقٍ عنوانه**، ويُختار من **دور الدافع** لا من ترويسةٍ يرسلها
    العميل: `RiderUser` و`CurrentDriver` يفرضان الدورَ على كل مسار، فالدورُ
    يقول أيّ تطبيقٍ فتح الصفحة بيقين، وترويسةٌ من العميل بيانٌ يُصدَّق بلا داعٍ.

    **وذلك اليومُ جاء** (نموذجُ الأدوار، 2026-08-19): هذا السطرُ كان يَعِد بأن
    «يوم يستعمل دورٌ واحد تطبيقين يصير التصريح واجباً على العميل». وقد صار
    الحسابُ يحمل دورين، فالدورُ كفّ عن تعيين التطبيق يقيناً — **ولم يُستبدل
    بتخمين**: من يحمل الدورين يرفع `AmbiguousRole` باسمه، والتصريحُ من العميل
    قرارٌ يُتخذ مع زرِّ التبديل لا يُخترع هنا.
    """
    base = (
