...
class WhatsAppCloudProvider:
    ...
    async def check_number(self, to: str) -> None:
        """**Cloud API لا يبيع هذا السؤال** — ولا مسارَ فيه يقول «أهذا الرقم
        على واتساب؟» قبل الإرسال.

        **فيمرّ صامتاً ولا يدّعي**: `None` هنا تعني «لم أستطع أن أسأل» لا
        «الرقمُ عليه» — **والإرسالُ بعده يحكم**. وهو صادقٌ لأن البديلَ أن
        نخترع جواباً، **وحارسٌ يجيب ما لا يعرف أسوأُ من غيابه**.
        """
        return None
