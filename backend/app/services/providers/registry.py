...
PROVIDERS: dict[ProviderKey, ProviderSpec] = {
    ...
    ProviderKey.WHATSAPP: ProviderSpec(
        ...
        #
        # **وقناتان خلف عقدٍ واحد** (قرارُ المالك 2026-08-16): الرسميةُ من
        # ميتا (`cloud`)، والذاتيةُ عبر بوابة Baileys الداخلية (`baileys`).
        # ويحكمهما حقلُ `transport` وحدَه — **لا عقدان مفعّلان** يفتحان سؤالَ
        # «أيُّهما أولاً»، وهو سؤالٌ يُجاب بحالةٍ ثانيةٍ تخالف العقود يوماً.
        #
        # **وثمنُ الذاتية مكتوبٌ لا مُنكَر**: أتمتةُ حسابٍ عبر مكتبةٍ غير رسمية
        # مخالفةٌ لشروط واتساب وعقوبتُها حظرُ الرقم. اختارها المالكُ قناةً
        # دائمةً بعد أن وقف التسجيلُ على عقدٍ رسميٍّ لم يصل، وبحرّاسٍ مكتوبة:
        # رقمٌ مخصّصٌ لا يُستعمل شخصياً، وسقفان أدناه، وتباعدٌ عشوائيٌّ ونصٌّ
        # واحدٌ بلا روابط في البوابة، وارتدادٌ فوريٌّ إلى الرسائل القصيرة.
        # **وخطةُ الطوارئ في `CLAUDE.md`** — ما يُفعل إن حُظر الرقم وكم يستغرق.
        fields=(
            # **أوّلُ الحقول لأنه يقرّر ما بعده**: الرسميةُ تطلب حقولَ ميتا،
            # والذاتيةُ تطلب البوابةَ ومفتاحَها. وغيابُه يُقرأ `cloud` فلا
            # يتبدّل سلوكُ عقدٍ قائمٍ بترقية
            ProviderField(
                key="transport",
                label="القناة: cloud (ميتا الرسمية) أو baileys (البوابة الذاتية)",
                secret=False,
                required=False,
            ),
            # --- البوابةُ الذاتية (baileys) ---
            # عنوانٌ **داخليٌّ** لا يُنشر منفذُه: `http://whatsapp-gateway:8080`
            ProviderField(
                key="gateway_url",
                label="عنوان البوابة الداخلي",
                secret=False,
                required=False,
            ),
            # سرٌّ حقيقي: من يملكه يُرسل من رقمنا إلى أي رقم
            ProviderField(
                key="gateway_key", label="مفتاح البوابة المشترك", required=False
            ),
            # **السقفان حارسا الحظر**، وفارغُهما يعني الافتراضَ لا صفراً —
