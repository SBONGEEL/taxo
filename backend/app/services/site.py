...
async def public_payload(session: AsyncSession) -> dict[str, Any]:
    ...
    **ويُقرأ معه ما لا يسكن هنا**: نسبةُ العمولة من إعداد العمولة، ومفاتيحُ
    ...
    #
    # **⚠ و`str()` على `normalize()` يطبع أُسّاً** (عطبٌ مقيسٌ ٢٠٢٦-٠٩-٠٦):
    # النسبةُ صفرٌ ⇒ `Decimal("100.00").normalize()` = `Decimal("1E+2")`،
    # **و`str()` عليه `"1E+2"`** — فالصفحةُ كانت تقول «يبقى لك 1E+2».
    # **وهو الشكلُ السابع بعينه** (مبلغٌ يُسلسَل «0» لا «0.000») في ثوبٍ
    # مقلوب: `normalize` يُقصَد به حذفُ الأصفار التابعة، **وهو على مضاعفات
    # المئة يحذف الخاناتِ نفسَها إلى الأُسّ**. **و`format(…, "f")` يمنع
    # الأُسَّ في كلِّ الحالات** ويُبقي المقصود: ٩٨٫٠٠ ⇒ «98» · ٩٧٫٥٠ ⇒
    # «97.5» · ١٠٠٫٠٠ ⇒ «100».
    #
    # **ولم يمسكه اختبارُ الحقول**: كان يسأل «أخرج الحقل؟» لا «ماذا فيه؟».
    payload["driver_keeps_per_100"] = format(
        (Decimal("100") - commission).quantize(Decimal("0.01")).normalize(), "f"
    )
