...
class AdminPromoBannerOut(BaseModel):
    ...
    **وحقلٌ يقول «لها صورة» بيتٌ ثانٍ للحقيقة** يفترق عن الملفّ أوّلَ رفعٍ
    ...
    first_shown_at: datetime | None = None
    #: **سببُ حجبها عن الناس، أو `null` إن كانت تبلغهم** (٢٠٢٦-٠٩-٠٩).
    #:
    #: **ولا يكفي أن تُحجب صامتةً**: مشرفٌ يرى `is_active = true` ويظنّها
    #: تعمل **يقضي يوماً يبحث عن سببٍ ليس هناك**. فالسببُ يُقال في الصفّ
    #: نفسِه، **ونصُّه عربيٌّ لأنه يُعرض كما هو**.
    #:
    #: **وهو محسوبٌ لا مخزون** — بيتُ الحقيقة `title`/`body`، وحقلٌ ثانٍ
    #: يفترق عنهما أوّلَ تحرير.
