...
class Ride(UUIDMixin, TimestampMixin, Base):
    ...
    # ---------------------------------------------- شكلُ المسار (البند ٨)
    #
    # إحداثياتُ المسار كما قالها Mapbox، `[[lng, lat], …]` بصيغة JSON —
    # **تُطلب مرةً عند القبول وتُحفظ**، فلا نداءَ لكل فتحةِ شاشةٍ ولا لكل طرف.
    #
    # **ولا تُعاد قراءتُها بعد الكتابة**: المسارُ المعروضُ للطرفين واحدٌ، وطلبٌ
    # ثانٍ لاحقاً قد يعطي مساراً آخرَ (زحمةٌ تبدّلت) فيرى الراكبُ خطاً والكبتنُ
    # غيرَه على رحلةٍ واحدة. وهي قاعدةُ التجميد نفسُها التي تحكم العمولة
    # والانتظار: ما عُرض لا يتبدّل تحت عين صاحبه.
    #
    ...
    # **تعليماتُ الملاحة بهندستها** (البند ٧) — تُجمَّد مع الخط ومنه، فلا
    # يفترقان: خطٌّ من نداءٍ وتعليماتٌ من آخر يعني سهماً يشير إلى غير ما يُرسم.
    #
    # **وعمودٌ مستقلٌّ لا حقلٌ داخل الخط**: الخطُّ يُقرأ في **كل** إطارٍ يرسمه
    # التطبيقان، والتعليماتُ يقرؤها الكبتنُ وحدَه وخلف مفتاح — فدمجُهما يحمّل
    # شاشةَ الراكب حمولةً لا يرسمها (٢٧٧ رأساً مقابل ٢١ — مقيس).
    #
    # **وفارغةٌ حالٌ مقبولة** كالخط: مفتاحٌ مطفأٌ، أو نداءٌ لم يُطلب فيه، أو
    # مسارٌ بلا خطواتٍ صالحة — والشريطُ لا يُرسم، ولا شيءَ يفشل.
    route_steps: Mapped[str | None] = mapped_column(Text, nullable=True)
