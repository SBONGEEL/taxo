...
<!--جديد-->
## وحارسٌ مرجعُه نسخةٌ محلّيةٌ مُهمَلة يصيح على إنتاجٍ سليم (قِيس ٢٠٢٦-٠٩-١١)

**`tools/check-apk.mjs` سأل سؤالين وخلطهما في مرجعٍ واحد**: «أالصفحةُ متّسقةٌ
مع نفسِها؟» و«أهذا الجهازُ هو من نشرها؟». **ومرجعُه في الاثنين كان
`landing/downloads/manifest.json` المحلّيّ.**

**والمجلَّدُ مُهمَلٌ في git** — وهو البندُ المكتوب سلفاً «النسخةُ الرابعةُ
عمياءُ عن المُهمَل». **فنسخةُ المطوّر تبلى وحدَها بلا أن يتغيّر سطرٌ في
المستودع**:

    بيانُ هذا الجهاز     v0.2.0 · إيداع 41656f2e · 2026-08-28 · رمز 392
    بيانُ الصفحة         v0.2.4 · إيداع 346efdbb · 2026-09-10 · رمز 514

**فصاح الحارسُ «✗ ما يُحمَّل ليس ما بُني» على إنتاجٍ سليمٍ تماماً.** والسلامةُ
مقيسةٌ بايتاً لا مُفترضة: حُمِّلت الحزمتان من الحافّة فكانتا **7,641,837**
و**7,864,877** بايتاً، وبصمتاهما `dff07a68ee…` و`061e7e4656…` — **وهما بعينهما
ما يصرّح به بيانُ الصفحة**.

**وأخطرُ من الحمرة الكاذبة ما تحتها**: لو تصادف أن نسخةَ الجهاز تطابق الصفحة،
**لَأخضرَّ الحارسُ بلا أن يقرأ بيانَ الصفحة أصلاً** — فخضرتُه صدفةٌ لا قياس،
وهو «حارسٌ يُشغَّل حيث لا يملك ما يقيسه» في أوضح صوره.

**والعلاجُ فصلُ المرجعين لا تليينُ الحكم:**

| السؤال | مرجعُه بعد الإصلاح |
|---|---|
| أما تخدمه الصفحةُ = ما تصرّح به؟ | **بيانُ الصفحة** — يُجلب مرّةً قبل أيِّ مقارنة |
| أالبيانُ المحلّيُّ = ما على قرص هذا الجهاز؟ | البيانُ المحلّيّ (كما كان — طرفاه محلّيّان) |
| أهذا الجهازُ نشر الصفحة؟ | **مقارنةُ الإيداعين، وتُطبع خبراً لا سقوطاً** |

**ولا يُلين الحكمُ حيث يجب أن يشتدّ**: جُرِّب بحافّةٍ مزيَّفةٍ تخدم بايتاتٍ
صحيحةً وبيانُها يحمل بصمةً واحدةً مبدَّلة، **فسقط الحارسُ بنصّه**: «✗ ما تخدمه
… يخالف بيانَ الصفحة نفسِه: `dff07a68ee` ≠ `0000000000`». **فالإصلاحُ أزال
البلاغَ الكاذبَ وأبقى الصادق** — وهو الشرطُ الذي يفرّق الإصلاحَ من الإطفاء.

**وغيابُ بيان الصفحة يُعدّ «لم يُقس» لا «سليم»**، ويُطبع في سطرِه.

## وبابٌ واحدٌ لتشغيل الرفع على جهاز المطوّر — **قاعدةٌ صارت شرطاً** (٢٠٢٦-٠٩-١١)

**ثلاثُ قواعدَ كانت مكتوبةً في `COMMANDS.md` يقرؤها من يتذكّر أن يقرأها**، وقد
سقط بها رفعٌ واحدٌ ثلاثَ مرّات. **و«القاعدةُ التي تُتذكَّر تُنسى» هي بعينها
العلّةُ التي أنشأت `scripts/deploy.sh` و`scripts/suite.sh`** — فأُغلقت هنا
بالشيء نفسِه: **`scripts/deploy-here.sh`**.

**وأربعُ بوّاباتٍ فيه، وكلُّها ترفض ولا تُصلح في صمت:**

1. **WSL تُرفض بعلّتها** — ولينكسُ الحقيقيُّ يمرّ. **والرفضُ العريض كان سيمنع
   الخادمَ نفسَه**، فالتمييزُ بـ`/proc/version`.
2. **`core.fileMode` يُضبط بالبيئة** — لا بتعديل `.git/config` الذي يبقى بعد
   الجولة فيخفي تغييراً حقيقياً في بتِّ التنفيذ غداً.
3. **الشجرةُ تُقاس بعد الضبط لا قبله** — وإلا سقطت البوّابةُ الأولى بقراءةٍ
   كاذبة.
4. **الإطلاقُ عمليةٌ مستقلّةٌ بملفِّ مشغِّل** — لا سطرٌ مقتبَس: وسائطُ
   `Start-Process` تُعاد اقتباسُها، وسطرٌ فيه `&&` و`>` مات صامتاً بلا سجلّ.

**وأثرُ البوّابة الثانية مقيسٌ في موضعه** (٢٠٢٦-٠٩-١١، من Git Bash على شجرة
WSL): **`git status --porcelain` بلا ضبطٍ = ٥٣ سطراً، ومعه = سطران** — وهما
التغييران الحقيقيّان وحدَهما. **فإحدى وخمسون قراءةً كاذبةً كانت ستُسقط
البوّابةَ الأولى.**

**وحدُّه مكتوبٌ فيه**: **قِيست مساراتُ رفضِه وحدَها** — الرفضُ من WSL
(خروجٌ `1` بنصِّه)، والرفضُ لشجرةٍ ليست نظيفة (خروجٌ `1` وسردُ الأسطر).
**ولم يُقَس رفعٌ كاملٌ خلاله بعد**، وهو مكتوبٌ في رأس الملفّ **كي لا يُقرأ
سكوتُه ضماناً**.
<!--/جديد-->

<!--جديد-->
## والعمودُ الرابعُ في `check:served` — **سؤالٌ يختلف في نوعه لا في درجته** (٢٠٢٦-٠٩-١٢)

**ثلاثةُ أعمدةٍ كانت تسأل سؤالاً واحداً**: «أالمخدومُ هو المبنيّ؟». **وأجابت
بنعم وهي صادقةٌ تماماً** — والحزمةُ المخدومةُ هي بعينها المبنيّة — **وسقطت
الحزمةُ على الهاتف ثلاثَ مرّاتٍ في يومٍ واحد**.

**والعلّةُ أن العنوانَ الذي تناديه الحزمةُ المخدومةُ كان `http://127.0.0.1:8001`**
— **وهو الهاتفُ نفسُه**. فيردّ كروميوم `ERR_NETWORK_UNREACHABLE`، ويعرض
التطبيقُ «تعذّر الوصول إلى الخدمة».

    الأعمدةُ الثلاثة   أالمخدومُ هو المبنيّ؟        ← نعم، صادقةً
    العمودُ الرابع     أيصلح العنوانُ لجهازٍ آخر؟   ← **لا**

**فالسؤالُ الرابعُ مختلفٌ في نوعه**: الأولُ عن **عمر** الحزمة، والثاني عن
**صلاحية عنوانها لمن ليس على هذا الحاسوب**. وحارسٌ يجيب الأولَ وحدَه **يخضرّ
ثلاث مرّاتٍ على عطبٍ قائم**.

### وحدُّه فيه: **لا يُسأل إلا عمّا يصل هاتفاً**

**من يخدم على `127.0.0.1` لمتصفّحِ هذا الحاسوب صائبٌ تماماً** — والحكمُ يقع
**حين يُخدَم النفقُ** وحدَه، وهو الطريقُ الذي لا يسلكه إلا جهازٌ آخر. **فلا
يصيح على سليم**، وحارسٌ يصيح على سليمٍ يُطفأ فيسقط معه ما يمسكه حقاً.

**ويقرأ العنوانَ من الوحدة المخدومة لا من الشجرة** — و`vite dev` يخدم
`src/main.tsx` بينما `preview` يخدم `assets/index-*.js`، **فيُقرأ من الذي
يحمله في الحالين**.

### وقِيس بالنقض لا بالخضرة وحدَها

**أُعيد العطبُ عمداً** (`TAXO_DEV_API_BASE=http://127.0.0.1:8001` ثمّ إقلاعُ
الحاوية)، **فسقط الحارسُ بنصّه**:

    ✗ أيصلح للهاتف؟   http://127.0.0.1:8001
    ✗ العنوانُ المخدومُ عبر النفق **عنوانُ هذا الحاسوب** — ولا يصل هاتفاً.

**ثمّ أُعيدت الحاويةُ إلى حالها** (`https://dev-api.tajora.ly`) فعاد أخضر.
**فالأحمرُ مقيسٌ كما الأخضر** — وهو الشرطُ الذي يفرّق الحارسَ من الزينة.

## ونداءٌ يعلّق **قبل أن يبدأ `ssh`** لا تحرسه خياراتُ `ssh` (قِيس ٢٠٢٦-٠٩-٢١)

**`ConnectTimeout` و`ServerAliveInterval` و`ServerAliveCountMax` خياراتٌ يقرؤها
`ssh` بعد أن يعمل.** فما يعلّق **قبل** أن يبدأ — في المشغِّل الذي يستدعيه —
**لا يحرسه شيءٌ منها**. **والسطرُ لا يفضح ذلك**: يحمل الخياراتِ كاملةً ويبدو
محروساً، والحراسةُ لم تبدأ.

**ووقع مقيساً**: `deploy.sh` على WSL يشغّل `ssh.exe` الخاصَّ بويندوز عبر `/init`
— **ولا خيارَ غيرُه**: `~/.ssh/taxo-contabo` بعبارةِ مرورٍ مفتوحةٍ في وكيل
ويندوز وحدَه، و`taxo-deploy` مفتاحُ أمرٍ مقيَّدٍ لا يفعل إلا `pull-release`.
**فعلّق نداءٌ واحدٌ — `git rev-parse HEAD`، قراءةٌ محضة — ستّاً وسبعين دقيقةً**
في البوّابة الأولى، و`ConnectTimeout=20` مكتوبٌ في سطره ولم يفعل شيئاً: لم يبلغ
`ssh` نقطةَ قراءته.

**وأخطرُ ما فيه أنه يُقرأ عطبَ شبكةٍ أو مفتاح.** من يشخّص يذهب إلى `ufw` أو
`authorized_keys` — **والسببُ أن المشغِّلَ لم يُسلِّم الأمرَ أصلاً**. وهو من
عائلة «**الطريقُ خطأ لا المفتاحُ مرفوض**» المكتوبةِ في `deploy.sh`،
**ووجهُها الثاني وأسوأُ منها**: هناك يُرفض في ثانيةٍ بنصّ، **وهنا لا يُرفض ولا
يُقبل** — ولا نصَّ يُقرأ أصلاً.

### ولا يُنفى بتجربةٍ واحدةٍ ناجحة

**النداءُ نفسُه أُعيد من صدفةٍ أماميةٍ فأجاب في ثانية**، وبصمةُ وكيل ويندوز
بصمةُ المفتاح نفسِها. **فالجسرُ متقطِّعٌ لا معطوب.** وفي الرفعة التالية علّق
نداءٌ ثانٍ **سبعَ دقائقَ ثمّ انفكَّ من تلقائه** ومضت الرفعةُ إلى آخرها خضراء.

**فخُضرةُ محاولةٍ واحدةٍ لا تنفي التقطُّع، وحمرتُها لا تثبت العطب** — وهذا هو
ما يجعل التشخيصَ بالإعادة وحدَها كاذباً هنا.

### والحارسُ سقفٌ من فوقه لا خيارٌ من داخله

**ما لا يملك `ssh` أن يحرسه يُحرَس من فوقه**: غلافٌ يُصرَّح في `TAXO_SSH` فيه
`timeout -k 10 900` — خمسَ عشرةَ دقيقةً سخيّةً لأطولِ نداءٍ حقيقيٍّ (`pg_dump`)
وضيّقةً على تعليقٍ أبديّ. **و`timeout` يخرج بـ١٢٤ فتسقط البوّابةُ بنصٍّ** —
ولا تُقرأ نجاحاً، ولا تبقى معلَّقةً بلا حكم.

**ولمَ غلافٌ خارجيٌّ لا تعديلٌ في `deploy.sh`**: السقفُ يخصّ **هذه البيئةَ**
وحدَها — WSL يستدعي ثنائيَّ ويندوز — ولا يخصّ من يشغّل السكربتَ من لينكس أصيل.
**وشرطٌ بيئيٌّ يُدفن في سكربتٍ عامٍّ يصير عبئاً على من لا علّةَ له عنده.**

**وحدُّه مكتوبٌ لا مسكوتٌ عنه**: **لا يمنع التعليقَ، يحدّه.** ورفعةٌ تسقط في
بوّابتها أرخصُ من رفعةٍ تعلّق فيها بلا نهاية — **وهذا كلُّ ما يشتريه، ولا
يُقرأ أكثرَ منه.**
## وقتلُ الأبِ لا يقتل الابن — **رفعان يعملان معاً على خادمٍ واحد** (قِيس ٢٠٢٦-٠٩-٢١)

**وقع مقيساً**: مُنسِّقٌ عالقٌ في انتظار CI بدأ رفعاً في `07:59:00Z`، وبعد
ثوانٍ أُنهي بـ`pkill -f 'orchestrate.sh'` — **فمات الأبُ وبقي `deploy.sh`
حيّاً يتيماً**، ثمّ أُطلق رفعٌ ثانٍ فوقه. **فصار على الخادم رفعان في آنٍ.**

**والنمطُ لا يبلغ الابن**: `pkill -f` يطابق سطرَ أمرِ الأب وحدَه، وابنُه سطرُه
`bash scripts/deploy.sh` لا يذكر المُنسِّقَ بحرف. **ومن قتل بالنمط ظنَّ أنه
قتل الشجرة**، والشجرةُ تُقتل بمجموعة عمليّاتها (`setsid` يفصلها) أو
**بمُعرِّفاتها واحداً واحداً من الورقة إلى الجذر**.

**وأُمسك في ~٣٠ ثانية، ولم يبلغ الثاني الخادمَ بأثر**: وقف عند البوّابة الأولى
(`d46461cc ← d46461cc`، صفرُ التزامات) **وهي قراءةٌ محضة** — لا نسخةَ ولا سحبَ
ولا حاوية. **والفضلُ فيه للترتيب لا لليقظة**: البوّابةُ الأولى تقرأ ولا تكتب،
فلو كان الترتيبُ غيرَ ذلك لَكتب اثنان على شيءٍ واحد.

### والحارسُ `flock` لا `pgrep`

**فحصٌ ثمّ إطلاقٌ فيه فجوةُ سباقٍ بعينها**: اثنان يفحصان في اللحظة نفسِها
فيجدان الساحةَ خالية، ويُطلقان. **و`flock` يفحص ويحجز في فعلٍ واحد** فلا فجوة.

**ويبقى `pgrep` معه لا بدلاً منه** — لأن **قفلاً لا يعرف اليتامى**: عمليةٌ نجت
من قتل أبيها لا تحمل قفلَنا، فيمرّ الجديدُ فوقها. فالحارسان يمسكان حالين
مختلفتين: القفلُ يمنع **المتسابق**، والمسحُ يمنع **الناجي**.

**وقِيست الأربعُ**: لا مانعَ ⇒ يمرّ · `deploy.sh` طليقٌ ⇒ يرفض بـ٢١ · قفلٌ
محجوزٌ ⇒ يرفض بـ٢٠ · وبعد زوالهما ⇒ يمرّ ثانيةً. **وبذرةُ `TAXO_DRY` هي ما
جعل الرفضتين تُقاسان بلا رفع** — وحارسٌ لا يُقاس رفضُه دعوى.

## وخطأٌ عابرٌ يُعامَل حالاً يدور إلى الأبد (قِيس ٢٠٢٦-٠٩-٢١)

**انتهى CI `success` في `07:48:23Z`، وبدأ نداءُ الحالة يُجيب ما لا يُفكّ في
`07:55` — فظلّ المُنسِّقُ يدور ولم يرَ خُضرةً وقعت.** والرفعُ لم يبدأ.

**والعلّةُ أن «لم أستطع القراءة» كان يُعامَل كحالٍ من أحوال التشغيل**: يُطبع
ويُنام ثلاثين ثانيةً، **بلا عدٍّ ولا تصعيدٍ ولا ذكرِ رمز HTTP** — فلا يُعرف
أعطبٌ هو أم شبكة، ولا متى يكفي.

**والفرقُ الذي لم يكن مرسوماً**: «لا تشغيلَ بعد» (`absent`) **انتظارٌ مشروع**
يدوم ما دام، و«ردٌّ لا يُفكّ» **خبرٌ عن القناة لا عن التشغيل**. فصار الثاني
يُحصى، ويُذكر رمزُه، **ويُوقِف بعد تواترٍ متّصل** بدل أن يصمت إلى المهلة.

**وقِيس المُفكِّكُ بأصنافه**: صفحةُ HTML وردٌّ فارغٌ وJSON مبتورٌ ⇒ **عابرٌ
يُحصى**؛ و`{"workflow_runs":[]}` ⇒ **انتظارٌ مشروع**؛ والأخضرُ والأحمرُ
يمرّان حكمين. **وحدُّ المعدّل كان `5000/5000`** — فالسببُ لم يُعرف، ونداءٌ
لاحقٌ بالإعدادات نفسِها أجاب سليماً. **ويُكتب أنه لم يُعرف.**
<!--/جديد-->
