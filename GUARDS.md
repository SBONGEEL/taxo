...
## وحارسٌ خارجَ البابِ الواحد يعيش أحمرَ ولا يعلم به أحد (قِيس 2026-09-03)

**`check:sheet` بقي أحمرَ على `master` عبر إيداعٍ كامل** — من `ebe87c7` (البند
٨) إلى أن كُشف بالصدفة أثناء نقل الشجرة.

**والعلّةُ بنيويةٌ لا سهو**: هو مُصرَّحٌ في `customer-app/package.json` ويعمل
داخل `npm run build` **لذلك التطبيق وحدَه**، **وليس في `scripts/guards.sh`** —
فلا يمرّ عليه خطّافُ الإيداع. **وCI لا ينقذ**: المشروعُ لا يدفع، فبوّابةٌ خلف
بابٍ لا يُفتح ليست بوّابة. **ولا أحدَ بنى تطبيقَ الراكب منذ ذلك الإيداع**،
فبقي الأحمرُ غيرَ مرئيٍّ لا مسكوتاً عنه.

**وهو عينُ درس `check:money-math`** المكتوبِ في رأس `scripts/guards.sh` —
**تكرّر بحارسٍ آخر بعد يومين**. فالقاعدةُ ليست «أصلِح الحارس»، هي: **كلُّ حارسٍ
ساكنٍ يدخل البابَ الواحد يومَ يُكتب**، وإلا فهو مكتوبٌ لا مُشغَّل.

> **وأُدخل `check:sheet` في `guards.sh`** (2026-09-03)، **والحلقةُ تتخطّى ما لا
> يملكه التطبيق** بقراءة `package.json` — فلا يُخترع حارسٌ لتطبيقٍ لا يعرفه.

## وأخضرُ يعتمد ملفَّ حالةٍ غيرَ متعقَّبٍ ليس أخضرَ الشيفرة (قِيس 2026-09-03)

**`tsc -b` تخطّى الفحصَ لأن `tsconfig.app.tsbuildinfo` قال «محدَّث»** — والملفُّ
غيرُ متعقَّبٍ في git. **فبقي خطأُ ترجمةٍ حقيقيٌّ مستوراً في تطبيقين**:
`AppVersion` مستعملٌ في `src/api/endpoints.ts` بلا استيراد (الراكب والكبتن؛
واللوحةُ وحدَها تستورده). **وأوّلُ بناءٍ من شجرةٍ نظيفةٍ كشفه فوراً**:
`error TS2304: Cannot find name 'AppVersion'`.

**والدرسُ ليس عن نظام تشغيل**: **بناءٌ فوق ذاكرةٍ يقيس أقلَّ ممّا يقيس بناءٌ من
الصفر**، وأيُّ خُضرةٍ تعتمد ملفَّ حالةٍ غيرَ متعقَّبٍ **تشهد لجهازٍ واحدٍ لا
للشيفرة** — وهي أختُ «خُضرةٌ كاذبةٌ على جهاز المطوّر أخطرُ منها في CI».

> **وما يمسكه هذا**: `npm run build` في CI يبدأ من استنساخٍ نظيفٍ بلا
> `tsbuildinfo`، **فكان سيمسكه لو دُفع**. **ولم يُدفع** — وهو الشرطُ البشريُّ
> نفسُه الذي أبقى `check:sheet` أحمر.
<!--/جديد-->
