 ...
 *
 * - **أثرَ المكدَّس كاملاً** — يُفتح بزرٍّ ولا يُقصّ بارتفاعٍ ثابت.
 ...
 *
 * **كُتب هنا تلوينٌ يُخفِّت «إطارَ المكتبة»** بنمطٍ يطابق `node_modules`
 * و`vendor-` و`chunk-` و`react-dom`. **ثمّ قِيست حزمةُ الإنتاج فلم يطابق
 * شيئاً**: حزمُ الراكب `index-*.js` و`mapbox-*.js` و`web-*.js`، **ولا حزمةَ
 * `vendor-*` أصلاً وReact داخلَ `index-*`**.
 *
 * **فكان يلوّن إطارَ mapbox حقيقيّاً على أنه إطارُنا** — وهو «حارسٌ صادقٌ
 * تُوسَّع دعواه فوق نطاقه»، **وتمييزٌ يخطئ أسوأُ من لا تمييز**: من يقرأ الأثرَ
 * يبني على اللون فيذهب يفتّش في شيفرةٍ لا عطبَ فيها.
 *
 * **ولا سبيلَ صحيحاً بلا خرائطِ مصدر**: الأسماءُ مُصغَّرةٌ ولا شيءَ في السطر
 * يقول لمن هو. **فنُزع، وكُتب في الشاشة نفسِها أنها لا تعرف** — ويُعاد النظرُ
 * يومَ توجد خرائطُ المصدر، لا قبله.
 *
 * > **⚠ والفُتاتُ فارغٌ حتى تصل حزمةٌ تجمعه**: الخلفيةُ تقبله
 * > (`scrub_breadcrumbs`) والشاشةُ ترسمه، **والجمعُ بُني في
 * > `lib/breadcrumbs.ts`** — فما يسبقه من أحداثٍ لا فُتاتَ فيه بحقّ،
 * > ولا يُقرأ سكوتُه «لم يقع شيءٌ قبل العطب».
 */
...
function StackBlock({ label, stack }: { label: string; stack: string }) {
  ...
  return (
    <div>
      ...
      {/* **حدُّ الشاشة مكتوبٌ فيها لا مسكوتٌ عنه** — انظر رأسَ الملفّ */}
