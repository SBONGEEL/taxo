 ...
 *
 * **ونُسخ إلى اللوحة يومَ نُشرت** (2026-08-21). كان إعفاؤها مكتوباً وله سببُه:
 * «تُخدَم على `127.0.0.1:5175` بالتصميم»، فلا حزمةَ تسافر ولا هدفَ يُخطأ. وقد
 * **زال السببُ في اللحظة التي صارت فيها منشورة** — فالإعفاءُ الذي يُنسى بعد
 * زوال علّته هو نفسُه «العذرُ البائد» الذي نرفضه في الحرّاس.
 *
...
function declared() {
  ...
  if (process.env.DEV_BUILD === "1") {
    console.log("✓ بناءٌ محليٌّ مُصرَّحٌ به (DEV_BUILD=1)");
    return true;
  }
  return false;
}
