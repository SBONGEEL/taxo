...
const DELIBERATE = new Map([
  ...
  // —————— كشفَها الحارسُ نفسُه حين قوي (2026-08-19) ——————
  // **ولا تُبنى قبل عرضها**: قاعدةُ المالك «لا تصلح عطباً تجده قبل أن تعرضه».
  // وكلُّها من صنفٍ واحد: **مصرَّحٌ به في `endpoints.ts` ولا ينادِيه أحد** —
  // وهو ما لم يره المسحُ الأولُ لأنه عدَّ التصريحَ زرّاً.
  [
    "PATCH /admin/campaigns/{campaign_id}",
    "تعديلُ حملةٍ مجدولة — مكتشَفٌ 2026-08-19، ينتظر قرارَ المالك (SPEC §18.2).",
  ],
  [
    "GET /admin/subscription-offers/{offer_id}/grants",
    "منحُ عرضٍ لجمهور `manual` — مسجَّلٌ في SPEC §18.2 من قبل، وهذا شقُّه القارئ.",
  ],
  [
    "POST /admin/subscription-offers/{offer_id}/grants",
    "منحُ عرضٍ لجمهور `manual` — مسجَّلٌ في SPEC §18.2، ولا يُبنى نصفُه.",
  ],
  [
    "PUT /admin/drivers/{driver_id}/advance-cap",
    "سقفُ سلفةِ كبتنٍ بعينه — مكتشَفٌ 2026-08-19، وهو مالٌ يُقرض فينتظر قرارَ المالك (SPEC §18.2).",
  ],
]);
