  ...
  fetch("./downloads/manifest.json", { cache: "no-store" })
    ...
    .then((manifest) => {
      for (const key of ["rider", "driver"]) {
        ...
        // **ومعها إيداعُها** (شرطُ المالك 2026-08-21): البصمةُ تفرّق حزمتين
        // **ولا تقول من أين جاءت أيٌّ منهما** — فشكوى «هذه تفعل كذا» لا
        // تُردّ إلى سطر. فمن حمّل يعرف ما حمّل، ونحن نعرف من أيِّ شجرة.
        //
        // **والوسمُ حين يوجد**: هو ما يسحبه الخادمُ، فيصير الثلاثةُ — ما
        // تخدمه الصفحةُ وما بناه CI ووسمُه — **مقيسةً لا مكتوبة**.
        const from = manifest?.commit ? ` · إيداع ${manifest.commit.slice(0, 8)}` : "";
        const tag = manifest?.tag ? ` · ${manifest.tag}` : "";
        meta.textContent =
          `${app.version_name ?? "1.0"} · بناء ${app.sha256.slice(0, 8)}${from}${tag} · ` +
