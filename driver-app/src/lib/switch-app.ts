 ...
 */
export async function switchToRider(): Promise<boolean> {
  ...
  const { value } = await AppLauncher.canOpenUrl({ url: "ly.tajora.rider" });
  if (!value) return false; // غيرُ مثبَّت — والصفحةُ تتولّاه

  await AppLauncher.openUrl({
    url: `taxo-rider://handoff?t=${encodeURIComponent(token)}`,
  });
  return true;
}
