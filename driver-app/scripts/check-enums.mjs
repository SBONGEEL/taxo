...
const UI_UNIONS = new Set([
  ...
  // «الطلبُ الوارد» لحظةٌ يسمعها الكبتن، لا حالُ صفٍّ في القاعدة
  "Cue",
  // مآلُ ضغط صورة الوثيقة (`lib/shrink.ts`): وصفٌ لما وقع **في المتصفح** قبل
  // أن يخرج شيءٌ على السلك — لا صفَّ في القاعدة ولا حقلَ في أيِّ حمولة.
  // **ويُعلَن هنا لأنه اتحادٌ مختلط**: `failed` قيمةٌ تعرفها الخلفيةُ في تعدادٍ
  // آخر، والثلاثةُ الباقيةُ لا — وهو بعينه الشكلُ الذي بُني الحارسُ له
